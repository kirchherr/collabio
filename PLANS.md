...
115. [ ] Complete native Office find and replace: literal Unicode-safe positions, case and whole-word options, complete match counts, current/all replacement as one reversible local edit, bounded content and preserved structure/formatting. Keep read-only and historical documents searchable, reuse explicit confirmed CAS saves, and verify desktop/tablet/mobile plus the existing regression matrix.
