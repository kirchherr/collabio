...
- Native document authoring, version history, comparison, historical takeover, find/replace, contextual table editing, version-bound discussions, explicit saved-text suggestions, saved-version browser printing and independent document reuse are complete through Roadmap 259. Preserve all 180 checks (145 browser cases and 35 model cases), immutable review/suggestion history, atomic accepted versions and nonempty recovery. Continue user-visible native Office development before CRM expansion; a bounded next candidate is server-side title search and paginated document discovery beyond the current 200-entry list, preserving current ACL checks and dirty drafts. Continuous tracked changes and live collaboration remain future work. DOCX interchange remains a separate Quick Edit/fidelity path; real Word/GenOffice results, calibrated thresholds and human review remain prerequisites, and DOCX saves/WOPI retain their gates. Do not resume Word/account/firewall interventions on the original workstation.
