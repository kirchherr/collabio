...
| DOCX Quick Edit Engine | Selektiv evaluierter GenOffice `docx-engine` hinter `OfficeEditAdapter.v1` | internal development OSS admission prepared; signatures pending | Interessante byte-erhaltende Patch-Architektur; 23 Komponenten exakt inventarisiert und ohne Findings gescannt, npm-Signatur/SLSA/Fulcio/Rekor verifiziert sowie 21 Runtime-Archive und 42 Rechtsdateien hashgebunden. Ein deterministisches Drittanbieter-NOTICE und ein internes, zweifach signiertes Development-Gate ersetzen die nicht verfuegbare externe Pruefung, ohne Hosted Service, On-Prem oder Produktion freizugeben. |
