...
No LMS migrations exist yet.

The first migration must create only the metadata needed for `lms.course` and `lms.enrollment`, with RLS, no hard delete, required metadata, KMS references, audit-chain references, and no training content body columns.
