...
`POST /v1/admin/kb/articles/write-approvals/execute` may commit approved edit writes after approved evidence, guard decision, refresh preview, execution plan hash, and human confirmation match. It must refresh source-version evidence and restore evidence immediately after the write and must keep source text out of audit metadata and responses. It must not enable embeddings, search indexing, or RAG index state.

Migration `0023_knowledge_base_write_approval_evidence.sql` adds the append-only write-approval ledger. Migration `0024_knowledge_base_write_approval_transition_lineage.sql` adds approval-transition lineage. The dry-run path now persists ledger evidence before source-object writes are possible. `KnowledgeBaseSourceObjectWriteGuard` verifies that ledger evidence, the expected current version, proposed source-version evidence, current restore evidence, retention policy, and Legal Hold state before future approved writes. The refresh preview makes the future source/restore evidence update auditable before writes exist. The execution skeleton binds that evidence to human confirmation while still blocking persistence. Execute refreshes source-version plus restore evidence after approved edit writes. Future PostgreSQL write paths must make the article/source/evidence update transactional.
