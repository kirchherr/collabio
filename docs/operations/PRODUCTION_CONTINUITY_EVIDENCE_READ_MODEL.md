...
The requirements response is derived from the current `backup_failover_policy.v3`. It exposes only:
...
`production_continuity_gate_status.v1` uses five states:
...
3. The evidence bundle contains only SHA-256 references and the strict fields defined by
   ...
   added.
4. An operator mounts the reviewed bundle read-only into `production-continuity-deployment-gate`.
5. The no-network gate validates the bundle and writes a metadata-only report.
6. A security administrator reads the gate-status endpoint. Missing, invalid, stale or blocked evidence remains
   fail-closed.
7. Live pilot traffic still requires the separate real-user nomination, privacy, workforce, four-eyes and start chain.
