...
The `collabio-provider` development stack on `dev001` now supplies private Ceph RGW and OpenBao endpoints. Its separately confirmed `protocol-probe` performs only authenticated metadata reads and establishes application-to-provider TLS and identity wiring without writing an object, signing, deleting or exposing bucket names. This report is development evidence only. The mutating acceptance above remains blocked until a dedicated empty Object-Lock proof bucket, one synthetic bundle, isolated PostgreSQL restore evidence and a separate exact-version-delete-denial authorization are prepared. Neither the three-node single-host simulation nor MinIO is accepted as productive WORM or HA evidence.
