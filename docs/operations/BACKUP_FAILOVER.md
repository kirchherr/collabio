...
The metadata-only `productivity_pilot_preflight_gate.v1` binds the business release hash to selected tenant module states, required/forbidden feature flags, the seven-operation route contract, five monitoring controls and four non-destructive rollback controls. Ready reports are persisted for tenant-scoped admission. The append-only `productivity_pilot_traffic_scope_enforcement.v1` is created by migration 0062. Migrations 0061 and 0062 plus the PostgreSQL restore drill verify preflight, admission, and traffic-scope tables, forced RLS, exact role grants, append-only policies, mutation-rejecting triggers, and exact row recovery. Traffic scope is default-deny: approved pilot routes return `423` and out-of-scope CRM/Tasks/Time routes return `403` until a separate start authorization exists. See `docs/operations/PRODUCTIVITY_PILOT_PREFLIGHT.md`, `docs/operations/PRODUCTIVITY_PILOT_ADMISSION.md`, and `docs/operations/PRODUCTIVITY_PILOT_TRAFFIC_SCOPE.md`.
