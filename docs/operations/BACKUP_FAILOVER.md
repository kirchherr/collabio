...
The metadata-only `productivity_pilot_preflight_gate.v1` binds the business release hash to selected tenant module states, required/forbidden feature flags, the seven-operation route contract, five monitoring controls and four non-destructive rollback controls. Ready reports are persisted for tenant-scoped admission. The append-only `productivity_pilot_traffic_scope_enforcement.v1` and time-bounded four-eyes `productivity_pilot_start_authorization.v1` are created by migrations 0062 and 0063. Migrations 0061 through 0063 plus the PostgreSQL restore drill verify preflight, admission, traffic-scope, and start-authorization tables, forced RLS, exact role grants, append-only policies, mutation-rejecting triggers, and exact row recovery. Traffic remains default-deny until an active authorization exists and `SUITE_PRODUCTIVITY_PILOT_RUNTIME_ENABLED=1`; the deployment switch defaults closed, can stop managed traffic immediately, and a restored authorization never opens traffic by itself. See `docs/operations/PRODUCTIVITY_PILOT_PREFLIGHT.md`, `docs/operations/PRODUCTIVITY_PILOT_ADMISSION.md`, `docs/operations/PRODUCTIVITY_PILOT_TRAFFIC_SCOPE.md`, and `docs/operations/PRODUCTIVITY_PILOT_START_AUTHORIZATION.md`.
