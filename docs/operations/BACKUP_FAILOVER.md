...
Before a Tickets & Incidents pilot, call `GET /v1/platform/modules/families/tickets-incidents/storage-migration-evidence` and `GET /v1/platform/modules/families/tickets-incidents/restore-drill-evidence`. The restore evidence now covers migrations `0051` through `0053`, `tickets.ticket_items`, immutable `tickets.ticket_events`, and append-only `tickets.activation_dry_run_execution_approval_records`, including RLS, tenant isolation, event ordering/foreign keys, uniqueness, retention, Legal Hold, KMS, audit references, SLA state, and no-content-payload checks.

The explicit human execution approval is recorded through `POST /v1/platform/modules/families/tickets-incidents/activation-dry-run-execution-approval-records` as hashes and references only. The record does not install, provision, dispatch, execute, or activate anything. After a restore, all `/v1/tickets*` routes must remain closed until the module package, tenant lifecycle state, migration manifest, and feature flags have been restored and validated. Restore evidence must include ticket/event/approval row counts, the append-only event chain, approval uniqueness, active Legal Hold behavior, and the approval-record evidence hash before the controlled pilot can resume.
