...
The metadata-only `productivity_pilot_preflight_gate.v1` binds the business release hash to selected tenant module states, required/forbidden feature flags, the seven-operation route contract, five monitoring controls and four non-destructive rollback controls. Ready reports are persisted for tenant-scoped admission. The append-only `productivity_pilot_traffic_scope_enforcement.v1` and time-bounded four-eyes `productivity_pilot_start_authorization.v1` are created by migrations 0062 and 0063. Migration 0064 adds `productivity_pilot_runtime_window.v1` and `productivity_pilot_runtime_observation.v1`; migration 0065 adds the metadata-only `productivity_pilot_closure_report.v1`. Migrations 0061 through 0065 plus the PostgreSQL restore drill verify preflight, admission, traffic-scope, start-authorization, runtime-window, runtime-observation, and closure-report tables, forced RLS, exact role grants, append-only policies, mutation-rejecting triggers, and exact row recovery. Traffic remains default-deny until an active authorization and designated-user runtime window exist and `SUITE_PRODUCTIVITY_PILOT_RUNTIME_ENABLED=1`; the deployment switch defaults closed, can stop managed traffic immediately, and restored start or closure records never open traffic by themselves. See `docs/operations/PRODUCTIVITY_PILOT_PREFLIGHT.md`, `docs/operations/PRODUCTIVITY_PILOT_ADMISSION.md`, `docs/operations/PRODUCTIVITY_PILOT_TRAFFIC_SCOPE.md`, `docs/operations/PRODUCTIVITY_PILOT_START_AUTHORIZATION.md`, `docs/operations/PRODUCTIVITY_PILOT_RUNTIME_WINDOW.md`, and `docs/operations/PRODUCTIVITY_PILOT_CLOSURE_REPORT.md`.

Migration 0066 extends the mandatory continuity contract with append-only
`productivity_pilot_real_user_nomination.v1` and `productivity_pilot_real_user_admission.v1`
records. The restore drill verifies both tenant-RLS tables, no-update/no-delete policies,
mutation-rejecting triggers, exact `collabio_authz_admin` grants, and exact row recovery. These
...
`productivity_pilot_real_user_readiness.v1` adds no durable state. It is rebuilt after restart,
restore, or failover from the authoritative real-user nomination, admission, start, runtime,
observation, and closure ledgers. Every read revalidates hashes and current-cycle bindings; stale
...
Operators must read `GET /v1/platform/modules/families/tickets-incidents/controlled-pilot/status` before each pilot transition and continue only with its reported next stage. The status read model adds no durable state. Tenant activation-readiness approvals are stored tenant-scoped and append-only in PostgreSQL by migration `0074`; raw confirmation text is excluded. The status recomputes the execution-approval boundary from tenant ID plus the durable tenant-admin approval, restore, and feature-manifest hashes, validates the persisted receipt chain and module state, and fails closed at `evidence_invalid`. A restore must reproduce the same trusted boundary and receipt-chain status before business API access resumes.
...
30. For tenants with active Knowledge Base production runtime evidence, run `docker compose run --rm kb-runtime-reconciler`.
    For CRM account onboarding state, verify all four business rows, four ACL grants, one immutable receipt, matching audit-chain references, Forced RLS, minimal role grants, and exact restored row counts.
    For Tasks & Activities creation state, verify the task, linked initial activity, creator/assignee ACL grants, immutable metadata receipt, shared audit-chain reference, Forced RLS, minimal role grants, and exact restored row counts.
...
- Which CRM account onboarding receipt hashes and atomic write control result were checked?
- Which Tasks & Activities creation receipt hashes and write-control result were checked?
