...
from office_recovery_proof import (
    TENANT_ID,
    require_office_recovery_environment,
    restored_document_inventory,
    restored_version_inventory,
    verify_restored_paragraph_versions,
    verify_restored_reviews,
)
...
def recovery_documents(*, count: int = 2, versions: int = 2) -> RecoveryDocuments:
    ...
    users = tuple(
        UserContext(tenant_id=TENANT_ID, user_id=f"recovery-author-{index}", role_ids={"office-editor"})
    ...
    for index in range(count):
        ...
        command = OfficeDocumentCreateCommand(
            title=f"Recovery {index}", document=content, mutation_reference=f"document-{index}", human_confirmation=True
        )
        ...
        if index == 0:
            ...
            for number in range(1, versions):
                result = service.save(
                    user_context=user,
                    object_id=history_object_id,
                    write_enabled=True,
                    command=OfficeDocumentSaveCommand(
                        **{**command.model_dump(), "mutation_reference": f"version-{number}"},
                        expected_current_version_id=result.version.version_id,
                    ),
                )
    service.writes_available = False
    return RecoveryDocuments(
        service, repository, users, [record.model_dump() for record in repository.documents.values()], history_object_id
    )


def test_recovery_reads_complete_document_and_version_pages_with_separate_current_acl_principals(
    ...
    documents, readers = restored_document_inventory(
        documents=fixture.service, users=fixture.users, expected_documents=fixture.inventory
    )
    ...
    versions = restored_version_inventory(
        documents=fixture.service, user=readers[document.object_id], document=document
    )
...
def test_recovery_rejects_broken_or_truncated_immutable_history(monkeypatch: pytest.MonkeyPatch, tamper: str) -> None:
    ...
    if tamper in {"missing-link", "cycle"}:
        ...
        fixture.repository.saved_versions[(TENANT_ID, document.object_id, version.version_id)] = version.model_copy(
            update={"previous_version_id": "missing" if tamper == "missing-link" else document.current_version_id}
        )
    else:
        if tamper == "head":
...
def test_review_and_suggestion_recovery_select_authorized_object_reader_without_granting_default_actor() -> None:
    ...
    review_report = verify_restored_reviews(
        documents=reviews.documents,
        reviews=reviews.reviews,
        sources=reviews.sources,
        receipts=reviews.receipts,
        user=denied,
        users_by_object={reviews.object_id: reviews.user},
        object_ids=(reviews.object_id,),
        expected_thread_ids={reviews.thread_id},
        expected_event_ids=reviews.event_ids,
    )
    ...
    suggestion_report = verify_restored_suggestions(
        ...
        receipts=suggestions.receipts,
        user=denied,
        users_by_object={suggestions.object_id: suggestions.user},
        object_ids=(suggestions.object_id,),
        expected_suggestion_ids=suggestions.suggestion_ids,
        expected_decision_ids=suggestions.decision_ids,
    )
...
def test_recovery_binds_designated_legacy_and_formatted_sources_without_body_evidence(tamper: str | None) -> None:
    ...
    if tamper is not None:
        ...
        report = verify_restored_paragraph_versions(
