...
STYLE_RECOVERY_TITLE = "Synthetic named style recovery"


def style_recovery_document(number: int) -> dict[str, Any]:
    ...
    def visit(node: dict[str, Any]) -> None:
        if node["type"] in {"paragraph", "heading"} and number > 1:
            node.setdefault("attrs", {})["styleId"] = "body"
...
def seed_synthetic_office_styles(
    *, environment: Mapping[str, str], client: Boto3S3CompatibleObjectStoreClient
    ...
    for number in (1, 2, 3):
        ...
        fields: dict[str, Any] = dict(
            ...
            document=style_recovery_document(number),
