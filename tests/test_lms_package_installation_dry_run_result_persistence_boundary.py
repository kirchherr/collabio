...
from suite.platform.lms_package_installation_dry_run_result_persistence_boundary import (
    ...
    build_lms_package_installation_dry_run_result_persistence_boundary_hash,
...
def _dry_run_result_persistence_boundary_command(
    *,
    ...
    return LmsPackageInstallationDryRunResultPersistenceBoundaryCommand(
        dry_run_plan_evidence_hash=dry_run_plan_hash,
        dry_run_execution_boundary_evidence_hash=dry_run_execution_boundary_hash,
        dry_run_execution_skeleton_evidence_hash=dry_run_execution_skeleton_hash,
        dry_run_executor_implementation_review_evidence_hash=dry_run_executor_implementation_review_hash,
...
def test_lms_package_installation_dry_run_result_persistence_boundary_is_metadata_only_after_request_boundary() -> None:
    ...
    response = build_lms_package_installation_dry_run_result_persistence_boundary_response(
    ...
    assert response.schema_version == LMS_PACKAGE_INSTALLATION_DRY_RUN_RESULT_PERSISTENCE_BOUNDARY_SCHEMA_VERSION
    assert response.endpoint == LMS_PACKAGE_INSTALLATION_DRY_RUN_RESULT_PERSISTENCE_BOUNDARY_ENDPOINT
    assert response.result_contract == LMS_PACKAGE_INSTALLATION_DRY_RUN_RESULT_PERSISTENCE_BOUNDARY_RESULT_CONTRACT
    ...
    assert response.dry_run_result_persistence_boundary_statement_hash.startswith("sha256:")
    assert response.preparer_role_allowed is True
    assert response.dry_run_result_persistence_boundary_ready is True
    assert response.future_dry_run_execution_activation_boundary_required is True
    ...
    assert (
        ...
        in response.dry_run_result_persistence_boundary_steps
    )
    assert "define_no_worker_or_scheduler_activation_flags" in response.dry_run_result_persistence_boundary_steps
    assert (
        "future_dry_run_execution_activation_boundary_required"
        in response.required_dry_run_result_persistence_boundary_evidence
    )
    assert response.blocking_reasons == ()
    assert response.evidence_hash == build_lms_package_installation_dry_run_result_persistence_boundary_hash(response)
...
def test_lms_dry_run_result_persistence_boundary_blocks_execution_request_without_side_effects() -> None:
    module_registry = default_module_registry()
    ...
    assert response.dry_run_result_persistence_boundary_ready is False
    ...
    assert response.destructive_actions_allowed is False
    assert (
        response.next_action == "prepare_lms_package_installation_dry_run_result_persistence_boundary_without_execution"
    )
    assert module_registry.get_tenant_module_or_none(tenant_id="tenant-demo", module_id="lms") is None
