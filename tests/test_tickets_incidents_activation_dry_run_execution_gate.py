...
from suite.platform.tickets_incidents_activation_dry_run_execution_gate import (
    ...
    TicketsIncidentsActivationDryRunExecutionGateCommand,
    build_tickets_incidents_activation_dry_run_execution_gate_hash,
    build_tickets_incidents_activation_dry_run_execution_gate_response,
)
...
def _approval_command(
    ...
    return TicketsIncidentsTenantAdminActivationApprovalRecordCommand(
        ...
        approval_record_ref="tickets-approval:dry-run-execution-gate-demo",
        approval_ticket_ref="ticket:tickets-dry-run-execution-gate-demo",
        human_confirmation_reference="confirmation:tickets-dry-run-execution-gate-demo",
        human_confirmation_statement=TICKETS_INCIDENTS_TENANT_ADMIN_ACTIVATION_APPROVAL_RECORD_CONFIRMATION_STATEMENT,
        change_request_ref="change:tickets-dry-run-execution-gate-demo",
        idempotency_key_ref="idempotency:tickets-dry-run-execution-gate-demo",
        approved_at_utc=datetime(2026, 7, 9, 8, 0, tzinfo=UTC),
        audit_chain_ref="audit:tickets-dry-run-execution-gate-demo",
    )
...
def _dry_run_execution_gate_command(
    *,
    ...
    return TicketsIncidentsActivationDryRunExecutionGateCommand(
        ...
        activation_dry_run_execution_gate_ref="tickets-activation-dry-run-execution-gate:demo",
        change_request_ref="change:tickets-activation-dry-run-execution-gate-demo",
        idempotency_key_ref="idempotency:tickets-activation-dry-run-execution-gate-demo",
        prepared_at_utc=datetime(2026, 7, 9, 8, 40, tzinfo=UTC),
        audit_chain_ref="audit:tickets-activation-dry-run-execution-gate-demo",
        activation_dry_run_execution_gate_statement=TICKETS_INCIDENTS_ACTIVATION_DRY_RUN_EXECUTION_GATE_STATEMENT,
...
def test_tickets_incidents_activation_dry_run_execution_gate_is_metadata_only_after_result_contract() -> None:
    ...
    response = build_tickets_incidents_activation_dry_run_execution_gate_response(
        command=_dry_run_execution_gate_command(
            ...
            activation_execution_boundary_hash=_fake_hash("6"),
            activation_executor_skeleton_hash=_fake_hash("7"),
    ...
    assert response.schema_version == TICKETS_INCIDENTS_ACTIVATION_DRY_RUN_EXECUTION_GATE_SCHEMA_VERSION
    assert response.endpoint == TICKETS_INCIDENTS_ACTIVATION_DRY_RUN_EXECUTION_GATE_ENDPOINT
    assert response.result_contract == TICKETS_INCIDENTS_ACTIVATION_DRY_RUN_EXECUTION_GATE_RESULT_CONTRACT
    ...
    assert response.activation_execution_boundary_evidence_hash == _fake_hash("6")
    assert response.activation_executor_skeleton_evidence_hash == _fake_hash("7")
    ...
    assert response.activation_dry_run_execution_gate_statement_hash.startswith("sha256:")
    assert response.preparer_role_allowed is True
    assert response.activation_dry_run_execution_gate_requested is True
    assert response.activation_dry_run_execution_gate_ready is True
    assert response.future_activation_dry_run_execution_request_boundary_required is True
    ...
    assert (
        "define_activation_dry_run_execution_request_boundary_ref" in response.activation_dry_run_execution_gate_steps
    )
    assert "define_no_write_execution_gate_flags" in response.activation_dry_run_execution_gate_steps
    assert "future_activation_dry_run_execution_request_boundary_required" in (
        response.required_activation_dry_run_execution_gate_evidence
    )
    assert response.blocking_reasons == ()
    assert response.evidence_hash == build_tickets_incidents_activation_dry_run_execution_gate_hash(response)
    assert (
...
def test_tickets_incidents_activation_dry_run_execution_gate_blocks_execution_request() -> None:
    module_registry = default_module_registry()
    ...
    assert response.activation_dry_run_execution_gate_ready is False
