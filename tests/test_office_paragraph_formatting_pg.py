...
from test_office_paragraph_formatting import formatted_document


def test_pg_formatted_versions_preserve_legacy_sources_receipts_cas_and_acl(database: Database) -> None:
    ...
    first_content = formatted_document()
    first = service.save(user_context=user, object_id=object_id, write_enabled=True, command=OfficeDocumentSaveCommand(**{**command("format-one").model_dump(), "document": first_content}, expected_current_version_id=created.version.version_id))
    second_content = deepcopy(first_content)
    second_content["content"][0]["attrs"] = {"textAlign": "left", "lineSpacing": "2", "spacingBefore": 0, "spacingAfter": 24}
    second_command = OfficeDocumentSaveCommand(**{**command("format-two").model_dump(), "document": second_content}, expected_current_version_id=first.version.version_id)
    ...
    for result, expected in ((created, command("legacy").document), (first, first_content), (second, second_content)):
        ...
        record = service.source_repository.get(tenant_id=user.tenant_id, object_id=object_id, version_id=version.version_id)
        ...
        assert receipt.content_hash == record.metadata.content_hash == version.content_hash == stable_hash(canonical_json(expected))
    ...
    with pytest.raises(OfficeDocumentConflictError):
        service.save(user_context=user, object_id=object_id, write_enabled=True, command=second_command.model_copy(update={"mutation_reference": "stale-format"}))
    ...
    with pytest.raises(OfficeDocumentPermissionError):
        service.save(user_context=reader, object_id=object_id, write_enabled=True, command=second_command.model_copy(update={"expected_current_version_id": second.version.version_id, "mutation_reference": "reader-format"}))
