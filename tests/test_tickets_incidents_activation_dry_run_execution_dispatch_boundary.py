...
from suite.platform.tickets_incidents_activation_dry_run_execution_dispatch_boundary import (
    ...
    TicketsIncidentsActivationDryRunExecutionDispatchBoundaryCommand,
    build_tickets_incidents_activation_dry_run_execution_dispatch_boundary_hash,
    build_tickets_incidents_activation_dry_run_execution_dispatch_boundary_response,
)
...
def _approval_command(
    ...
    return TicketsIncidentsTenantAdminActivationApprovalRecordCommand(
        ...
        approval_record_ref="tickets-approval:dry-run-execution-dispatch-boundary-demo",
        approval_ticket_ref="ticket:tickets-dry-run-execution-dispatch-boundary-demo",
        human_confirmation_reference="confirmation:tickets-dry-run-execution-dispatch-boundary-demo",
        human_confirmation_statement=TICKETS_INCIDENTS_TENANT_ADMIN_ACTIVATION_APPROVAL_RECORD_CONFIRMATION_STATEMENT,
        change_request_ref="change:tickets-dry-run-execution-dispatch-boundary-demo",
        idempotency_key_ref="idempotency:tickets-dry-run-execution-dispatch-boundary-demo",
        approved_at_utc=datetime(2026, 7, 9, 9, 20, tzinfo=UTC),
        audit_chain_ref="audit:tickets-dry-run-execution-dispatch-boundary-demo",
    )


def _dry_run_execution_dispatch_boundary_command(
    *,
    ...
    return TicketsIncidentsActivationDryRunExecutionDispatchBoundaryCommand(
        ...
        activation_dry_run_execution_dispatch_boundary_ref=(
            "tickets-activation-dry-run-execution-dispatch-boundary:demo"
        ),
        change_request_ref="change:tickets-activation-dry-run-execution-dispatch-boundary-demo",
        idempotency_key_ref="idempotency:tickets-activation-dry-run-execution-dispatch-boundary-demo",
        prepared_at_utc=datetime(2026, 7, 9, 9, 25, tzinfo=UTC),
        audit_chain_ref="audit:tickets-activation-dry-run-execution-dispatch-boundary-demo",
...
def test_tickets_incidents_activation_dry_run_execution_dispatch_boundary_is_metadata_only() -> None:
    ...
    response = build_tickets_incidents_activation_dry_run_execution_dispatch_boundary_response(
        command=_dry_run_execution_dispatch_boundary_command(
    ...
    assert response.schema_version == TICKETS_INCIDENTS_ACTIVATION_DRY_RUN_EXECUTION_DISPATCH_BOUNDARY_SCHEMA_VERSION
    assert response.endpoint == TICKETS_INCIDENTS_ACTIVATION_DRY_RUN_EXECUTION_DISPATCH_BOUNDARY_ENDPOINT
    assert response.result_contract == TICKETS_INCIDENTS_ACTIVATION_DRY_RUN_EXECUTION_DISPATCH_BOUNDARY_RESULT_CONTRACT
    ...
    assert response.activation_dry_run_execution_dispatch_boundary_statement_hash.startswith("sha256:")
    assert response.preparer_role_allowed is True
    assert response.activation_dry_run_execution_dispatch_boundary_requested is True
    assert response.activation_dry_run_execution_dispatch_boundary_ready is True
    assert response.future_activation_dry_run_execution_worker_boundary_required is True
    ...
    assert "bind_tickets_activation_dry_run_execution_start_boundary_hash" in (
        response.activation_dry_run_execution_dispatch_boundary_steps
    )
    assert "define_execution_worker_boundary_required_before_any_worker_dispatch" in (
        response.activation_dry_run_execution_dispatch_boundary_steps
    )
    assert "future_activation_dry_run_execution_worker_boundary_required" in (
        response.required_activation_dry_run_execution_dispatch_boundary_evidence
    )
    assert response.blocking_reasons == ()
    assert response.evidence_hash == build_tickets_incidents_activation_dry_run_execution_dispatch_boundary_hash(
        response
    )
    assert (
...
def test_tickets_incidents_activation_dry_run_execution_dispatch_boundary_blocks_worker_dispatch() -> None:
    module_registry = default_module_registry()
    ...
    assert response.activation_dry_run_execution_dispatch_boundary_ready is False
