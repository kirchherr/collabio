...
from suite.platform.tickets_incidents_activation_dry_run_plan import (
    ...
    TICKETS_INCIDENTS_ACTIVATION_DRY_RUN_PLAN_SCHEMA_VERSION,
    TICKETS_INCIDENTS_ACTIVATION_DRY_RUN_PLAN_STATEMENT,
    TicketsIncidentsActivationDryRunPlanCommand,
    build_tickets_incidents_activation_dry_run_plan_hash,
...
def test_tickets_incidents_activation_dry_run_plan_is_metadata_only_after_skeleton() -> None:
    ...
    response = build_tickets_incidents_activation_dry_run_plan_response(
    ...
    assert response.schema_version == TICKETS_INCIDENTS_ACTIVATION_DRY_RUN_PLAN_SCHEMA_VERSION
    assert response.endpoint == TICKETS_INCIDENTS_ACTIVATION_DRY_RUN_PLAN_ENDPOINT
    assert response.result_contract == TICKETS_INCIDENTS_ACTIVATION_DRY_RUN_PLAN_RESULT_CONTRACT
    ...
    assert response.activation_dry_run_plan_statement_hash.startswith("sha256:")
    assert response.planner_role_allowed is True
    assert response.activation_dry_run_plan_ready is True
    assert response.activation_dry_run_execution_boundary_required is True
    ...
    assert "verify_activation_executor_skeleton_hash_bound" in response.dry_run_checks
    assert "future_activation_dry_run_execution_boundary_required" in response.required_dry_run_plan_evidence
    assert response.blocking_reasons == ()
    assert response.evidence_hash == build_tickets_incidents_activation_dry_run_plan_hash(response)
...
def test_tickets_incidents_activation_dry_run_plan_blocks_without_admin_or_skeleton() -> None:
    module_registry = default_module_registry()
    ...
    assert response.activation_dry_run_plan_ready is False
