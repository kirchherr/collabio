...
from suite.platform.office_suggestions import OfficeSuggestionService, SuggestionCreateCommand, SuggestionDecisionCommand
...
class SuggestionRecoveryFixture:
    ...
    def verify(self) -> dict[str, Any]:
        return verify_restored_suggestions(suggestions=self.service, sources=self.sources, receipts=self.receipts,
            user=self.user, object_ids=(self.object_id,), expected_suggestion_ids=self.suggestion_ids,
...
def suggestion_recovery_fixture(*, count: int = 2) -> SuggestionRecoveryFixture:
    ...
    documents = OfficeDocumentService(repository=document_repository, source_repository=sources, audit=InMemoryAuditLogger())
    ...
    saved = documents.create(user_context=user, write_enabled=True, command=OfficeDocumentCreateCommand(
        title="Synthetic suggestion recovery", document={"type": "doc", "content": [
            {"type": "paragraph", "content": [{"type": "text", "text": "😀 private anchor"}]}]},
        mutation_reference="recovery-suggestion-doc", human_confirmation=True))
    ...
    for index in range(count):
        created = service.mutate(user_context=user, object_id=object_id, write_enabled=True,
            command=SuggestionCreateCommand.model_validate({"anchor_version_id": saved.version.version_id,
                "expected_current_version_id": saved.version.version_id, "anchor": {"from": 1, "to": 3},
                "replacement_text": f"private replacement {index}", "mutation_reference": f"suggestion-{index}",
                "human_confirmation": True}))
        ids.append(created.suggestion.suggestion_id)
    decisions: set[str] = set()
    for index, suggestion_id in enumerate(ids[:2]):
        decision = service.mutate(user_context=user, object_id=object_id, suggestion_id=suggestion_id, write_enabled=True,
            command=SuggestionDecisionCommand.model_validate({"operation": "accept" if index == 0 else "reject",
                "expected_revision": 1, "expected_current_version_id": saved.version.version_id if index == 0 else None,
                "mutation_reference": f"decision-{index}", "human_confirmation": True}))
        assert decision.decision is not None
        decisions.add(decision.decision.decision_id)
    documents.writes_available = False
    return SuggestionRecoveryFixture(service, repository, document_repository, sources, receipts, user, object_id,
                                     set(ids), decisions)
...
@pytest.mark.parametrize("tamper", ("missing_proposal", "missing_decision", "anchor", "source", "receipt", "result",
                                    "rejected_result", "acl", "inventory", "writable"))
def test_suggestion_recovery_denies_incomplete_content_lineage_or_permissions(tamper: str) -> None:
    ...
    if tamper == "missing_proposal":
        ...
        fixture.receipts._receipts[key] = fixture.receipts._receipts[key].model_copy(update={"parent_object_id": "wrong"})
    elif tamper == "result":
