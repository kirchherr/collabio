...
from suite.platform.lms_package_installation_dry_run_result_contract import (
    ...
    LMS_PACKAGE_INSTALLATION_DRY_RUN_RESULT_CONTRACT_SCHEMA_VERSION,
    LMS_PACKAGE_INSTALLATION_DRY_RUN_RESULT_CONTRACT_STATEMENT,
    LmsPackageInstallationDryRunResultContractCommand,
    build_lms_package_installation_dry_run_result_contract_hash,
...
def test_lms_package_installation_dry_run_result_contract_is_metadata_only_after_review() -> None:
    ...
    response = build_lms_package_installation_dry_run_result_contract_response(
    ...
    assert response.schema_version == LMS_PACKAGE_INSTALLATION_DRY_RUN_RESULT_CONTRACT_SCHEMA_VERSION
    assert response.endpoint == LMS_PACKAGE_INSTALLATION_DRY_RUN_RESULT_CONTRACT_ENDPOINT
    assert response.result_contract == LMS_PACKAGE_INSTALLATION_DRY_RUN_RESULT_CONTRACT_RESULT_CONTRACT
    ...
    assert response.dry_run_result_contract_statement_hash.startswith("sha256:")
    assert response.preparer_role_allowed is True
    assert response.dry_run_result_contract_ready is True
    ...
    assert "define_dry_run_result_receipt_schema_version" in response.dry_run_result_contract_steps
    assert "define_no_write_check_status_fields" in response.dry_run_result_contract_steps
    assert "future_dry_run_execution_gate_required" in response.required_dry_run_result_contract_evidence
    assert response.blocking_reasons == ()
    assert response.evidence_hash == build_lms_package_installation_dry_run_result_contract_hash(response)
...
def test_lms_dry_run_result_contract_blocks_execution_request_without_side_effects() -> None:
    module_registry = default_module_registry()
    ...
    assert response.dry_run_result_contract_ready is False
