...
from suite.platform.lms_package_installation_dry_run_execution_scheduler_boundary import (
    ...
    build_lms_package_installation_dry_run_execution_scheduler_boundary_hash,
...
def test_lms_dry_run_execution_scheduler_boundary_is_metadata_only_and_non_activating() -> None:
    module_registry = default_module_registry()
    migration_manifest = load_migration_manifest()
    ...
    response = build_lms_package_installation_dry_run_execution_scheduler_boundary_response(
    ...
    assert response.schema_version == LMS_PACKAGE_INSTALLATION_DRY_RUN_EXECUTION_SCHEDULER_BOUNDARY_SCHEMA_VERSION
    assert response.endpoint == LMS_PACKAGE_INSTALLATION_DRY_RUN_EXECUTION_SCHEDULER_BOUNDARY_ENDPOINT
    assert response.result_contract == LMS_PACKAGE_INSTALLATION_DRY_RUN_EXECUTION_SCHEDULER_BOUNDARY_RESULT_CONTRACT
    ...
    assert response.scheduler_activation_boundary_ref == "scheduler-activation-boundary:lms-dry-run-demo"
    assert response.command_hash.startswith("sha256:")
    assert response.idempotency_key_hash.startswith("sha256:")
    assert response.dry_run_execution_scheduler_boundary_statement_hash.startswith("sha256:")
    assert response.preparer_role_allowed is True
    assert response.dry_run_execution_scheduler_boundary_requested is True
    assert response.dry_run_execution_scheduler_boundary_ready is True
    ...
    assert response.future_worker_image_boundary_required is True
    ...
    assert "bind_scheduler_policy_ref_without_activation" in response.dry_run_execution_scheduler_boundary_steps
    assert "scheduler_not_activated" in response.required_dry_run_execution_scheduler_boundary_evidence
    assert "scheduler_job_not_created" in response.required_dry_run_execution_scheduler_boundary_evidence
    assert response.blocking_reasons == ()
    assert response.evidence_hash == build_lms_package_installation_dry_run_execution_scheduler_boundary_hash(response)
...
def test_lms_dry_run_execution_scheduler_boundary_blocks_missing_evidence_and_runtime_flags() -> None:
    module_registry = default_module_registry()
    ...
    assert response.dry_run_execution_scheduler_boundary_ready is False
