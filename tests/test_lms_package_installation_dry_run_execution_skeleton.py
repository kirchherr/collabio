...
from suite.platform.lms_package_installation_dry_run_execution_skeleton import (
    ...
    build_lms_package_installation_dry_run_execution_skeleton_hash,
...
def test_lms_package_installation_dry_run_execution_skeleton_is_metadata_only_after_boundary() -> None:
    ...
    response = build_lms_package_installation_dry_run_execution_skeleton_response(
    ...
    assert response.schema_version == LMS_PACKAGE_INSTALLATION_DRY_RUN_EXECUTION_SKELETON_SCHEMA_VERSION
    assert response.endpoint == LMS_PACKAGE_INSTALLATION_DRY_RUN_EXECUTION_SKELETON_ENDPOINT
    assert response.result_contract == LMS_PACKAGE_INSTALLATION_DRY_RUN_EXECUTION_SKELETON_RESULT_CONTRACT
    ...
    assert response.dry_run_execution_skeleton_preparation_statement_hash.startswith("sha256:")
    assert response.preparer_role_allowed is True
    assert response.dry_run_execution_skeleton_prepared is True
    ...
    assert "bind_lms_package_installation_dry_run_execution_boundary_hash" in response.dry_run_execution_skeleton_steps
    assert "future_dry_run_executor_implementation_required" in response.required_dry_run_execution_skeleton_evidence
    assert response.blocking_reasons == ()
    assert response.evidence_hash == build_lms_package_installation_dry_run_execution_skeleton_hash(response)
...
def test_lms_dry_run_execution_skeleton_blocks_execution_request_without_side_effects() -> None:
    module_registry = default_module_registry()
    ...
    assert response.dry_run_execution_skeleton_prepared is False
