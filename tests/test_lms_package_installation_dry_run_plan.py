...
from suite.platform.lms_package_installation_dry_run_plan import (
    LMS_PACKAGE_INSTALLATION_DRY_RUN_PLAN_ENDPOINT,
    LMS_PACKAGE_INSTALLATION_DRY_RUN_PLAN_RESULT_CONTRACT,
    LMS_PACKAGE_INSTALLATION_DRY_RUN_PLAN_SCHEMA_VERSION,
    LMS_PACKAGE_INSTALLATION_DRY_RUN_PLAN_STATEMENT,
    LmsPackageInstallationDryRunPlanCommand,
    build_lms_package_installation_dry_run_plan_hash,
...
def test_lms_package_installation_dry_run_plan_is_metadata_only_after_skeleton() -> None:
    ...
    response = build_lms_package_installation_dry_run_plan_response(
    ...
    assert response.schema_version == LMS_PACKAGE_INSTALLATION_DRY_RUN_PLAN_SCHEMA_VERSION
    assert response.endpoint == LMS_PACKAGE_INSTALLATION_DRY_RUN_PLAN_ENDPOINT
    assert response.result_contract == LMS_PACKAGE_INSTALLATION_DRY_RUN_PLAN_RESULT_CONTRACT
    ...
    assert response.dry_run_plan_statement_hash.startswith("sha256:")
    assert response.planner_role_allowed is True
    assert response.dry_run_plan_ready is True
    assert response.dry_run_execution_boundary_required is True
    ...
    assert "verify_executor_skeleton_hash_bound" in response.dry_run_checks
    assert "future_dry_run_execution_boundary_required" in response.required_dry_run_plan_evidence
    assert response.blocking_reasons == ()
    assert response.evidence_hash == build_lms_package_installation_dry_run_plan_hash(response)
...
def test_lms_dry_run_plan_blocks_missing_admin_and_dry_run_execution_request_without_side_effects() -> None:
    module_registry = default_module_registry()
    ...
    assert response.dry_run_plan_ready is False
    ...
    assert response.tenant_module_state_created is False
    assert response.destructive_actions_allowed is False
    assert response.next_action == "prepare_lms_package_installation_dry_run_plan_without_tenant_activation"
    assert module_registry.get_tenant_module_or_none(tenant_id="tenant-demo", module_id="lms") is None
