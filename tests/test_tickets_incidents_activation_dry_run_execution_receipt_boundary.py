...
from suite.platform.tickets_incidents_activation_dry_run_execution_receipt_boundary import (
    ...
    build_tickets_incidents_activation_dry_run_execution_receipt_boundary_hash,
...
def _approval_command(
    ...
    return TicketsIncidentsTenantAdminActivationApprovalRecordCommand(
        ...
        approval_record_ref="tickets-approval:dry-run-execution-receipt-boundary-demo",
        approval_ticket_ref="ticket:tickets-dry-run-execution-receipt-boundary-demo",
        human_confirmation_reference="confirmation:tickets-dry-run-execution-receipt-boundary-demo",
        human_confirmation_statement=TICKETS_INCIDENTS_TENANT_ADMIN_ACTIVATION_APPROVAL_RECORD_CONFIRMATION_STATEMENT,
        change_request_ref="change:tickets-dry-run-execution-receipt-boundary-demo",
        idempotency_key_ref="idempotency:tickets-dry-run-execution-receipt-boundary-demo",
        approved_at_utc=datetime(2026, 7, 9, 8, 0, tzinfo=UTC),
        audit_chain_ref="audit:tickets-dry-run-execution-receipt-boundary-demo",
    )
    ...
    *,
    ...
    return TicketsIncidentsActivationDryRunExecutionReceiptBoundaryCommand(
        ...
        activation_dry_run_execution_receipt_boundary_ref=(
            "tickets-activation-dry-run-execution-receipt-boundary:demo"
        ),
        change_request_ref="change:tickets-activation-dry-run-execution-receipt-boundary-demo",
        idempotency_key_ref="idempotency:tickets-activation-dry-run-execution-receipt-boundary-demo",
        prepared_at_utc=datetime(2026, 7, 9, 8, 55, tzinfo=UTC),
        audit_chain_ref="audit:tickets-activation-dry-run-execution-receipt-boundary-demo",
...
def test_tickets_incidents_activation_dry_run_execution_receipt_boundary_is_metadata_only_after_preflight() -> None:
    ...
    response = build_tickets_incidents_activation_dry_run_execution_receipt_boundary_response(
    ...
    assert response.schema_version == TICKETS_INCIDENTS_ACTIVATION_DRY_RUN_EXECUTION_RECEIPT_BOUNDARY_SCHEMA_VERSION
    assert response.endpoint == TICKETS_INCIDENTS_ACTIVATION_DRY_RUN_EXECUTION_RECEIPT_BOUNDARY_ENDPOINT
    assert response.result_contract == TICKETS_INCIDENTS_ACTIVATION_DRY_RUN_EXECUTION_RECEIPT_BOUNDARY_RESULT_CONTRACT
    ...
    assert response.activation_dry_run_execution_receipt_boundary_statement_hash.startswith("sha256:")
    assert response.preparer_role_allowed is True
    assert response.activation_dry_run_execution_receipt_boundary_requested is True
    assert response.activation_dry_run_execution_receipt_boundary_ready is True
    assert response.future_activation_dry_run_result_persistence_boundary_required is True
    ...
    assert "bind_tickets_activation_dry_run_execution_preflight_hash" in (
        response.activation_dry_run_execution_receipt_boundary_steps
    )
    assert "define_result_persistence_boundary_required" in response.activation_dry_run_execution_receipt_boundary_steps
    assert "future_activation_dry_run_result_persistence_boundary_required" in (
        response.required_activation_dry_run_execution_receipt_boundary_evidence
    )
    assert response.blocking_reasons == ()
    assert response.evidence_hash == build_tickets_incidents_activation_dry_run_execution_receipt_boundary_hash(
        response
    )
    assert (
...
def test_tickets_incidents_activation_dry_run_execution_receipt_boundary_blocks_execution_request() -> None:
    module_registry = default_module_registry()
    ...
    assert response.activation_dry_run_execution_receipt_boundary_ready is False
