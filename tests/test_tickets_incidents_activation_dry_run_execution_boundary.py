...
from suite.platform.tickets_incidents_activation_dry_run_execution_boundary import (
    ...
    build_tickets_incidents_activation_dry_run_execution_boundary_hash,
...
def test_tickets_incidents_activation_dry_run_execution_boundary_is_metadata_only_after_plan() -> None:
    ...
    assert response.schema_version == TICKETS_INCIDENTS_ACTIVATION_DRY_RUN_EXECUTION_BOUNDARY_SCHEMA_VERSION
    assert response.endpoint == TICKETS_INCIDENTS_ACTIVATION_DRY_RUN_EXECUTION_BOUNDARY_ENDPOINT
    assert response.result_contract == TICKETS_INCIDENTS_ACTIVATION_DRY_RUN_EXECUTION_BOUNDARY_RESULT_CONTRACT
    ...
    assert response.activation_dry_run_execution_boundary_review_statement_hash.startswith("sha256:")
    assert response.reviewer_role_allowed is True
    assert response.activation_dry_run_execution_boundary_review_ready is True
    assert response.activation_dry_run_execution_boundary_ready is True
    assert response.future_activation_dry_run_execution_skeleton_required is True
    ...
    assert "activation_dry_run_plan_hash" in response.required_dry_run_execution_boundary_evidence
    assert (
        "future_activation_dry_run_execution_skeleton_required" in response.required_dry_run_execution_boundary_evidence
    )
    assert response.blocking_reasons == ()
    assert response.evidence_hash == build_tickets_incidents_activation_dry_run_execution_boundary_hash(response)
    assert (
...
def test_tickets_incidents_activation_dry_run_execution_boundary_blocks_execution_request() -> None:
    module_registry = default_module_registry()
    ...
    assert response.activation_dry_run_execution_boundary_review_ready is False
    assert response.activation_dry_run_execution_boundary_ready is False
    ...
    assert response.tenant_module_state_created is False
    assert response.destructive_actions_allowed is False
    assert response.next_action == "review_tickets_incidents_activation_dry_run_execution_boundary"
    assert module_registry.get_tenant_module_or_none(tenant_id="tenant-demo", module_id="tickets_incidents") is None
