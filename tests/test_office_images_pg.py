...
from suite.platform.office_documents import OfficeDocumentCreateCommand, OfficeDocumentInvalidContentError, OfficeDocumentNotFoundError, OfficeDocumentSaveCommand
...
def test_pg_images_save_history_copy_and_replay_are_exact_and_independently_owned(database: Database) -> None:
    ...
    saved = service.save(user_context=user, object_id=object_id, write_enabled=True, command=OfficeDocumentSaveCommand(
        **{**command("with-image").model_dump(), "document": document}, expected_current_version_id=created.version.version_id))
    ...
    removed = service.save(user_context=user, object_id=object_id, write_enabled=True, command=OfficeDocumentSaveCommand(
    ...
    assert service.read_content(user_context=user, object_id=object_id, version_id=saved.version.version_id).content == document
    with psycopg.connect(database.admin_dsn) as connection:
        set_tenant(connection, user.tenant_id)
        connection.execute("UPDATE collabio.object_acl_entries SET status = 'revoked' WHERE tenant_id = %s AND object_id = %s",
...
def test_pg_wrong_owner_hash_and_foreign_tenant_images_fail_without_document_write(database: Database) -> None:
    ...
    repository = service.repository
    assert isinstance(repository, PgOfficeDocumentRepository)
    attrs = store_uploaded_image(repository, user, first.document.object_id, png_from_pixels(1, 1, b"\0\0\xff\xff"), 1, 1)
    ...
    for changes in ({}, {"documentId": second.document.object_id}, {"contentHash": "sha256:" + "0" * 64}):
        ...
        with pytest.raises((OfficeDocumentInvalidContentError, OfficeDocumentNotFoundError)):
            service.save(user_context=user, object_id=second.document.object_id, write_enabled=True, command=OfficeDocumentSaveCommand(
                **{**command("wrong-image").model_dump(), "document": document}, expected_current_version_id=second.version.version_id))
