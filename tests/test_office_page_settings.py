...
def page_settings() -> dict[str, Any]:
    return {"paper": "letter", "orientation": "landscape", "margins": {"top": 12, "right": 25, "bottom": 30, "left": 40}}
...
def test_page_settings_fail_before_commit_without_content_echo(
    ...
    response = office_api.client.post(BASE, headers=office_api.headers, json={**create_payload("bad-page"), "document": document})
...
def test_page_settings_api_versions_replay_reset_history_conflict_and_access(office_api: OfficeApiHarness) -> None:
    ...
    command = {**payload, "document": reset, "mutation_reference": "page-reset", "expected_current_version_id": first["version"]["version_id"]}
    ...
    assert office_api.client.post(path, headers=office_api.headers, json={**command, "mutation_reference": "stale-page"}).status_code == 409
