...
from suite.platform.lms_package_installation_dry_run_execution_boundary import (
    ...
    build_lms_package_installation_dry_run_execution_boundary_hash,
...
def test_lms_package_installation_dry_run_execution_boundary_is_metadata_only_after_plan() -> None:
    ...
    response = build_lms_package_installation_dry_run_execution_boundary_response(
    ...
    assert response.schema_version == LMS_PACKAGE_INSTALLATION_DRY_RUN_EXECUTION_BOUNDARY_SCHEMA_VERSION
    assert response.endpoint == LMS_PACKAGE_INSTALLATION_DRY_RUN_EXECUTION_BOUNDARY_ENDPOINT
    assert response.result_contract == LMS_PACKAGE_INSTALLATION_DRY_RUN_EXECUTION_BOUNDARY_RESULT_CONTRACT
    ...
    assert response.dry_run_execution_boundary_review_statement_hash.startswith("sha256:")
    assert response.reviewer_role_allowed is True
    assert response.dry_run_execution_boundary_review_ready is True
    assert response.package_installation_dry_run_execution_boundary_ready is True
    assert response.future_dry_run_execution_skeleton_required is True
    ...
    assert "package_installation_dry_run_plan_hash" in response.required_dry_run_execution_boundary_evidence
    assert "future_dry_run_execution_skeleton_required" in response.required_dry_run_execution_boundary_evidence
    assert response.blocking_reasons == ()
    assert response.evidence_hash == build_lms_package_installation_dry_run_execution_boundary_hash(response)
    assert (
        response.next_action == "prepare_lms_package_installation_dry_run_execution_skeleton_without_tenant_activation"
    )
    assert module_registry.get_tenant_module_or_none(tenant_id="tenant-demo", module_id="lms") is None


def test_lms_dry_run_execution_boundary_blocks_execution_request_without_side_effects() -> None:
    module_registry = default_module_registry()
    ...
    assert response.dry_run_execution_boundary_review_ready is False
    assert response.package_installation_dry_run_execution_boundary_ready is False
    ...
    assert response.tenant_module_state_created is False
    assert response.destructive_actions_allowed is False
    assert response.next_action == "review_lms_package_installation_dry_run_execution_boundary"
    assert module_registry.get_tenant_module_or_none(tenant_id="tenant-demo", module_id="lms") is None
