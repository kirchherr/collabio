...
from suite.platform.lms_package_installation_dry_run_execution_preflight import (
    ...
    build_lms_package_installation_dry_run_execution_preflight_hash,
...
def test_lms_package_installation_dry_run_execution_preflight_is_metadata_only_after_request_boundary() -> None:
    ...
    response = build_lms_package_installation_dry_run_execution_preflight_response(
    ...
    assert response.schema_version == LMS_PACKAGE_INSTALLATION_DRY_RUN_EXECUTION_PREFLIGHT_SCHEMA_VERSION
    assert response.endpoint == LMS_PACKAGE_INSTALLATION_DRY_RUN_EXECUTION_PREFLIGHT_ENDPOINT
    assert response.result_contract == LMS_PACKAGE_INSTALLATION_DRY_RUN_EXECUTION_PREFLIGHT_RESULT_CONTRACT
    ...
    assert response.dry_run_execution_preflight_statement_hash.startswith("sha256:")
    assert response.preparer_role_allowed is True
    assert response.dry_run_execution_preflight_ready is True
    assert response.future_dry_run_execution_receipt_boundary_required is True
    ...
    assert "define_preflight_idempotency_and_readiness_checks" in response.dry_run_execution_preflight_steps
    assert "define_no_worker_or_scheduler_activation_flags" in response.dry_run_execution_preflight_steps
    assert (
        "future_dry_run_execution_receipt_boundary_required" in response.required_dry_run_execution_preflight_evidence
    )
    assert response.blocking_reasons == ()
    assert response.evidence_hash == build_lms_package_installation_dry_run_execution_preflight_hash(response)
...
def test_lms_dry_run_execution_preflight_blocks_execution_request_without_side_effects() -> None:
    module_registry = default_module_registry()
    ...
    assert response.dry_run_execution_preflight_ready is False
