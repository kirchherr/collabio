...
from suite.platform.tickets_incidents_activation_dry_run_executor_runtime_boundary import (
    ...
    TicketsIncidentsActivationDryRunExecutorRuntimeBoundaryCommand,
    build_tickets_incidents_activation_dry_run_executor_runtime_boundary_hash,
    build_tickets_incidents_activation_dry_run_executor_runtime_boundary_response,
)
...
def _approval_command(
    ...
    return TicketsIncidentsTenantAdminActivationApprovalRecordCommand(
        ...
        approval_record_ref="tickets-approval:dry-run-executor-runtime-boundary-demo",
        approval_ticket_ref="ticket:tickets-dry-run-executor-runtime-boundary-demo",
        human_confirmation_reference="confirmation:tickets-dry-run-executor-runtime-boundary-demo",
        human_confirmation_statement=TICKETS_INCIDENTS_TENANT_ADMIN_ACTIVATION_APPROVAL_RECORD_CONFIRMATION_STATEMENT,
        change_request_ref="change:tickets-dry-run-executor-runtime-boundary-demo",
        idempotency_key_ref="idempotency:tickets-dry-run-executor-runtime-boundary-demo",
        approved_at_utc=datetime(2026, 7, 9, 8, 0, tzinfo=UTC),
        audit_chain_ref="audit:tickets-dry-run-executor-runtime-boundary-demo",
    )
...
def _dry_run_executor_runtime_boundary_command(
    *,
    ...
    return TicketsIncidentsActivationDryRunExecutorRuntimeBoundaryCommand(
        ...
        activation_dry_run_executor_runtime_boundary_ref="tickets-activation-dry-run-executor-runtime-boundary:demo",
        change_request_ref="change:tickets-activation-dry-run-executor-runtime-boundary-demo",
        idempotency_key_ref="idempotency:tickets-activation-dry-run-executor-runtime-boundary-demo",
        prepared_at_utc=datetime(2026, 7, 9, 8, 50, tzinfo=UTC),
        audit_chain_ref="audit:tickets-activation-dry-run-executor-runtime-boundary-demo",
        activation_dry_run_executor_runtime_boundary_statement=TICKETS_INCIDENTS_ACTIVATION_DRY_RUN_EXECUTOR_RUNTIME_BOUNDARY_STATEMENT,
...
def test_tickets_incidents_activation_dry_run_executor_runtime_boundary_is_metadata_only_after_request_boundary() -> (
    None
):
    ...
    response = build_tickets_incidents_activation_dry_run_executor_runtime_boundary_response(
        command=_dry_run_executor_runtime_boundary_command(
            ...
            activation_execution_boundary_hash=_fake_hash("8"),
            activation_executor_skeleton_hash=_fake_hash("9"),
    ...
    assert response.schema_version == TICKETS_INCIDENTS_ACTIVATION_DRY_RUN_EXECUTOR_RUNTIME_BOUNDARY_SCHEMA_VERSION
    assert response.endpoint == TICKETS_INCIDENTS_ACTIVATION_DRY_RUN_EXECUTOR_RUNTIME_BOUNDARY_ENDPOINT
    assert response.result_contract == TICKETS_INCIDENTS_ACTIVATION_DRY_RUN_EXECUTOR_RUNTIME_BOUNDARY_RESULT_CONTRACT
    ...
    assert response.activation_execution_boundary_evidence_hash == _fake_hash("8")
    assert response.activation_executor_skeleton_evidence_hash == _fake_hash("9")
    ...
    assert response.activation_dry_run_executor_runtime_boundary_statement_hash.startswith("sha256:")
    assert response.preparer_role_allowed is True
    assert response.activation_dry_run_executor_runtime_boundary_requested is True
    assert response.activation_dry_run_executor_runtime_boundary_ready is True
    assert response.future_activation_dry_run_execution_preflight_required is True
    ...
    assert (
        ...
        in response.activation_dry_run_executor_runtime_boundary_steps
    )
    assert (
        "define_no_worker_or_scheduler_activation_flags" in response.activation_dry_run_executor_runtime_boundary_steps
    )
    assert "future_activation_dry_run_execution_preflight_required" in (
        response.required_activation_dry_run_executor_runtime_boundary_evidence
    )
    assert response.blocking_reasons == ()
    assert response.evidence_hash == build_tickets_incidents_activation_dry_run_executor_runtime_boundary_hash(response)
    assert response.next_action == "prepare_tickets_incidents_activation_dry_run_execution_preflight_without_execution"
    assert module_registry.get_tenant_module_or_none(tenant_id="tenant-demo", module_id="tickets_incidents") is None


def test_tickets_incidents_activation_dry_run_executor_runtime_boundary_blocks_execution_request() -> None:
    module_registry = default_module_registry()
    ...
    assert response.activation_dry_run_executor_runtime_boundary_ready is False
