...
def test_native_document_supports_bounded_rich_structure_without_active_content() -> None:
    ...
    rich = {"type": "doc", "content": [
        ...
        {"type": "paragraph", "content": [
            {"type": "text", "text": "<script>literal</script>", "marks": [{"type": "bold"}, {"type": "underline"}]},
            {"type": "hardBreak"},
        ]},
        ...
        {"type": "table", "content": [{"type": "tableRow", "content": [
            {"type": "tableHeader", "attrs": {"colspan": 1, "rowspan": 1, "colwidth": None}, "content": [paragraph]},
...
@pytest.mark.parametrize("node", [
    ...
    {"type": "paragraph", "content": [{"type": "text", "text": "x", "marks": [{"type": "link", "attrs": {"href": "x"}}]}]},
    ...
    {"type": "table", "content": [{"type": "tableRow", "content": [{"type": "tableCell", "attrs": {"rowspan": 2}, "content": [{"type": "paragraph"}]}]}]},
...
def test_native_create_save_history_and_actor_bound_idempotency(office: Any) -> None:
    ...
    saved = service.save(user_context=user, object_id=object_id, write_enabled=True, command=OfficeDocumentSaveCommand(
        title="Revised title", document=document_text("New text"), mutation_reference="save-1", human_confirmation=True,
    ...
    with pytest.raises(OfficeDocumentConflictError):
        service.create(user_context=user, command=create_command().model_copy(update={"title": "Changed"}), write_enabled=True)
...
def test_native_write_gates_readonly_acl_revoke_and_foreign_tenant(office: Any) -> None:
    ...
    object_id = created.document.object_id
    user.readable_object_ids.add(object_id)
    command = OfficeDocumentSaveCommand(**create_command("save").model_dump(), expected_current_version_id=created.version.version_id)
...
def test_stale_save_and_missing_confirmation_have_no_source_or_receipt_side_effect(office: Any) -> None:
    ...
    with pytest.raises(ValidationError):
        service.create(user_context=user, command=create_command().model_copy(update={"human_confirmation": False}), write_enabled=True)
    with pytest.raises(ValidationError):
...
@pytest.mark.parametrize("field,value", [
    ...
    ("content_byte_length", 400_001), ("created_by", "another-user"), ("acl_version", 99),
])
def test_source_metadata_corruption_is_rejected_before_content_read(office: Any, monkeypatch: Any, field: str, value: Any) -> None:
    service, repository, user = office
    created = service.create(user_context=user, command=create_command(), write_enabled=True)
    object_id = created.document.object_id
    user.readable_object_ids.add(object_id)
    metadata = repository.source_repository.get_metadata(tenant_id=user.tenant_id, object_id=object_id, version_id=created.version.version_id)
    monkeypatch.setattr(repository.source_repository, "get_metadata", lambda **_: metadata.model_copy(update={field: value}))
    monkeypatch.setattr(repository.source_repository, "get", lambda **_: pytest.fail("preflight must block content access"))
    with pytest.raises(OfficeDocumentInvalidContentError):
...
def test_loaded_bytes_must_match_exact_manifest_and_canonical_document(office: Any, monkeypatch: Any) -> None:
    ...
    object_id = created.document.object_id
    user.readable_object_ids.add(object_id)
    record = repository.source_repository.get(tenant_id=user.tenant_id, object_id=object_id, version_id=created.version.version_id)
