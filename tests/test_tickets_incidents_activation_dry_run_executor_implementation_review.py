...
from suite.platform.tickets_incidents_activation_dry_run_executor_implementation_review import (
    ...
    build_tickets_incidents_activation_dry_run_executor_implementation_review_hash,
    ...
    *,
    ...
    return TicketsIncidentsActivationDryRunExecutorImplementationReviewCommand(
        ...
        activation_dry_run_executor_implementation_review_ref=(
            "tickets-activation-dry-run-executor-implementation-review:demo"
        ),
        change_request_ref="change:tickets-activation-dry-run-executor-implementation-review-demo",
        idempotency_key_ref="idempotency:tickets-activation-dry-run-executor-implementation-review-demo",
        reviewed_at_utc=datetime(2026, 7, 9, 8, 30, tzinfo=UTC),
        audit_chain_ref="audit:tickets-activation-dry-run-executor-implementation-review-demo",
...
def test_tickets_incidents_activation_dry_run_executor_implementation_review_is_metadata_only_after_skeleton() -> None:
    ...
    response = build_tickets_incidents_activation_dry_run_executor_implementation_review_response(
    ...
    assert response.endpoint == TICKETS_INCIDENTS_ACTIVATION_DRY_RUN_EXECUTOR_IMPLEMENTATION_REVIEW_ENDPOINT
    assert (
        response.result_contract == TICKETS_INCIDENTS_ACTIVATION_DRY_RUN_EXECUTOR_IMPLEMENTATION_REVIEW_RESULT_CONTRACT
    )
    ...
    assert response.activation_dry_run_executor_implementation_review_statement_hash.startswith("sha256:")
    assert response.reviewer_role_allowed is True
    assert response.activation_dry_run_executor_implementation_review_ready is True
    ...
    assert (
        ...
        in response.activation_dry_run_executor_implementation_review_steps
    )
    assert (
        "future_activation_dry_run_result_contract_required"
        in response.required_activation_dry_run_executor_implementation_review_evidence
    )
    assert response.blocking_reasons == ()
    assert response.evidence_hash == build_tickets_incidents_activation_dry_run_executor_implementation_review_hash(
        response
    )
...
def test_tickets_incidents_activation_dry_run_executor_implementation_review_blocks_execution_request() -> None:
    module_registry = default_module_registry()
    ...
    assert response.activation_dry_run_executor_implementation_review_ready is False
