...
def _result_metadata_command(
    *,
    ...
    worker_ref: str = "lease-consumer:lms-dry-run-unit",
    worker_receipt_idempotency_key_ref: str = "idempotency:lms-dry-run-worker-receipt-unit",
    worker_result_stub_idempotency_key_ref: str = "idempotency:lms-dry-run-worker-result-stub-unit",
    **overrides: object,
) -> LmsPackageInstallationDryRunExecutionOutboxResultMetadataCommand:
    payload: dict[str, object] = {
        "worker_idempotency_key_hash": worker_idempotency_key_hash,
        ...
        "worker_result_stub_idempotency_key_ref": worker_result_stub_idempotency_key_ref,
        "recorded_at_utc": datetime(2026, 6, 30, 12, 40, tzinfo=UTC),
        "idempotency_key_ref": "idempotency:lms-dry-run-result-metadata-unit",
        "audit_chain_ref": "audit:lms-dry-run-result-metadata-unit",
        "result_metadata_statement": LMS_PACKAGE_INSTALLATION_DRY_RUN_EXECUTION_OUTBOX_RESULT_METADATA_STATEMENT,
    }
    payload.update(overrides)
    return LmsPackageInstallationDryRunExecutionOutboxResultMetadataCommand.model_validate(payload)
...
def test_lms_dry_run_execution_result_metadata_store_registers_stub_bound_metadata_without_business_writes() -> None:
    ...
    assert response.schema_version == "lms_package_installation_dry_run_execution_outbox_result_metadata_store.v1"
    assert response.result_metadata_store_ready is True
    assert response.result_metadata_record_requested is True
    assert response.result_metadata_record_registered is True
    assert response.worker_receipt_ref_validated is True
    assert response.worker_result_stub_ref_validated is True
    assert response.worker_status_observed is True
    assert response.lease_validated is True
    assert response.result_metadata_ref.startswith("result-metadata:sha256:")
    assert response.worker_receipt_ref == receipt_response.worker_receipt_ref
    assert response.expected_worker_receipt_ref == receipt_response.worker_receipt_ref
    assert response.worker_result_stub_ref == result_stub_response.worker_result_stub_ref
    assert response.expected_worker_result_stub_ref == result_stub_response.worker_result_stub_ref
    assert response.result_metadata_record is not None
    assert response.result_metadata_record.tenant_id == "tenant-demo"
    assert response.result_metadata_record.worker_job_ref == leased.worker_job_ref
    assert response.result_metadata_record.result_metadata_ref == response.result_metadata_ref
    assert response.result_metadata_record.dry_run_result_payload_included is False
    assert response.received_job == leased
    assert response.received_job_queue_status == LmsPackageInstallationDryRunExecutionJobStatus.LEASED
    assert response.outbox_state_mutated is False
    assert response.business_writes_executed is False
    ...
    assert response.summary.lease_validated_job_count == 1
    assert response.summary.receipt_ref_validated_job_count == 1
    assert response.summary.worker_result_stub_ref_validated_job_count == 1
    assert response.summary.status_observed_job_count == 1
    assert response.summary.blocking_reason_count == 0
    assert response.evidence_hash.startswith("sha256:")
    assert len(result_metadata_store.list_records(tenant_id="tenant-demo")) == 1
    persisted = job_store.get(tenant_id="tenant-demo", worker_idempotency_key_hash=leased.worker_idempotency_key_hash)
    assert persisted == leased


def test_lms_dry_run_execution_result_metadata_store_blocks_payload_and_business_write_requests() -> None:
    job_store = InMemoryLmsPackageInstallationDryRunExecutionJobOutboxStore((_job(),))
    result_metadata_store = InMemoryLmsPackageInstallationDryRunExecutionResultMetadataStore()
    leased = job_store.lease_next(
        tenant_id="tenant-demo",
        lease_owner="lease-consumer:lms-dry-run-unit",
        now=datetime(2026, 6, 30, 12, 0, 1, tzinfo=UTC),
    )
    ...
    response = build_lms_package_installation_dry_run_execution_outbox_result_metadata_store_response(
        command=_result_metadata_command(
            ...
            lease_id=leased.lease_id,
            worker_receipt_ref=receipt_response.worker_receipt_ref,
            worker_result_stub_ref="worker-result-stub:sha256:ffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff",
            worker_execution_requested=True,
            dry_run_result_persistence_requested=True,
            dry_run_result_payload_included=True,
            business_write_requested=True,
        ),
        tenant_id="tenant-demo",
        user_role_ids={"tenant-admin"},
    ...
    assert response.result_metadata_store_ready is False
    assert response.result_metadata_record_registered is False
    assert response.result_metadata_record is None
    assert response.worker_receipt_ref_validated is True
    assert response.worker_result_stub_ref_validated is False
    assert response.worker_status_observed is True
    assert response.lease_validated is True
    assert response.summary.result_metadata_record_count == 0
    assert response.summary.worker_result_stub_ref_validated_job_count == 0
    assert "lms_dry_run_execution_result_metadata_worker_result_stub_ref_mismatch" in response.blocking_reasons
    assert "worker_execution_forbidden_in_lms_dry_run_execution_result_metadata" in response.blocking_reasons
    assert "dry_run_result_persistence_forbidden_in_lms_dry_run_execution_result_metadata" in response.blocking_reasons
    assert "dry_run_result_payload_forbidden_in_lms_dry_run_execution_result_metadata" in response.blocking_reasons
...
def test_lms_dry_run_execution_result_reconciliation_gate_reconciles_read_model_without_business_writes() -> None:
    job_store = InMemoryLmsPackageInstallationDryRunExecutionJobOutboxStore((_job(),))
    result_metadata_store = InMemoryLmsPackageInstallationDryRunExecutionResultMetadataStore()
    lease_response = build_lms_package_installation_dry_run_execution_outbox_lease_consumer_response(
        command=_lease_command(),
    ...
    response = build_lms_package_installation_dry_run_execution_outbox_result_reconciliation_gate_response(
        ...
        job_store=job_store,
        result_metadata_store=result_metadata_store,
        generated_at_utc=datetime(2026, 6, 30, 12, 50, tzinfo=UTC),
    )

    assert response.schema_version == "lms_package_installation_dry_run_execution_outbox_result_reconciliation_gate.v1"
    assert response.result_reconciliation_gate_ready is True
    assert response.reader_role_allowed is True
    assert response.result_read_model_ready is True
    assert response.result_read_model_evidence_hash.startswith("sha256:")
    assert response.result_metadata_refs == (metadata_response.result_metadata_ref,)
    assert response.reconciled_result_metadata_refs == (metadata_response.result_metadata_ref,)
    assert response.missing_metadata_record_refs == ()
    assert response.missing_read_model_entry_refs == ()
    assert response.mismatched_result_metadata_refs == ()
    assert response.missing_job_reference_refs == ()
    assert len(response.reconciliation_entries) == 1
    entry = response.reconciliation_entries[0]
    assert entry.tenant_id == "tenant-demo"
    assert entry.result_metadata_ref == metadata_response.result_metadata_ref
    assert entry.worker_job_ref == leased.worker_job_ref
    assert entry.metadata_record_found is True
    assert entry.read_model_entry_found is True
    assert entry.job_reference_found is True
    assert entry.metadata_record_matches_read_model is True
    assert entry.received_job_queue_status == LmsPackageInstallationDryRunExecutionJobStatus.LEASED
    assert entry.dry_run_result_payload_included is False
    assert entry.business_writes_executed is False
    assert response.outbox_state_mutated is False
    ...
    assert response.summary.blocking_reason_count == 0
    assert response.evidence_hash.startswith("sha256:")
    assert response.next_action == "seal_lms_dry_run_execution_foundation_and_return_to_cross_module_backend_slices"


def test_lms_dry_run_execution_result_reconciliation_gate_blocks_missing_job_references() -> None:
    source_job_store = InMemoryLmsPackageInstallationDryRunExecutionJobOutboxStore((_job(),))
    source_metadata_store = InMemoryLmsPackageInstallationDryRunExecutionResultMetadataStore()
    leased = source_job_store.lease_next(
        tenant_id="tenant-demo",
    ...
    assert response.summary.missing_job_reference_count == 1
    assert response.missing_job_reference_refs == (metadata_response.result_metadata_ref,)
    assert "lms_dry_run_execution_result_reconciliation_gate_requires_ready_read_model" in response.blocking_reasons
    assert "lms_dry_run_execution_result_reconciliation_gate_requires_job_references" in response.blocking_reasons
