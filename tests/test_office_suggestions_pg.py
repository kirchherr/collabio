    ...
    create = SuggestionCreateCommand.model_validate(
        {
            ...
            "mutation_reference": "create-suggestion",
            "human_confirmation": True,
        }
    )
    created = suggestions.mutate(
        user_context=user, object_id=saved.document.object_id, command=create, write_enabled=True
    )
    ...
    with psycopg.connect(database.app_dsn) as connection:
        ...
        for table in (
            ...
            result = connection.execute(
                f"SELECT count(*) FROM {table} WHERE tenant_id=%s", (user.tenant_id,)
            ).fetchone()
...
def test_pg_suggestion_accept_is_one_reopenable_exact_version_and_decision(database: Database) -> None:
    ...
    assert counts(database, user) == (2, 1, 1, 4, 4)
    assert (
        docs.read_content(user_context=user, object_id=saved.document.object_id).version.version_id
        == result.suggestion.result_version_id
    )
    with psycopg.connect(database.admin_dsn) as connection:
        set_tenant(connection, user.tenant_id)
        connection.execute(
            "UPDATE collabio.object_acl_entries SET permission='read' WHERE tenant_id=%s AND object_id=%s",
...
def test_pg_suggestion_racing_accept_and_reject_have_only_one_result_and_no_loser_put(database: Database) -> None:
    ...
    def decide(operation: str) -> str:
        service = build_office_suggestion_service(
            document_service=service_for(database, store), audit=InMemoryAuditLogger()
        )
        ...
        try:
            service.mutate(
                ...
                object_id=saved.document.object_id,
                suggestion_id=created.suggestion.suggestion_id,
                command=decision_command(operation, saved.version.version_id, operation),
                write_enabled=True,
            )
...
def test_pg_suggestion_stale_accept_has_no_put_and_reject_preserves_head(database: Database) -> None:
    ...
    latest = docs.save(
        ...
        command=OfficeDocumentSaveCommand(
            **command("later", "later content").model_dump(), expected_current_version_id=saved.version.version_id
        ),
    )
    ...
    with pytest.raises(OfficeDocumentConflictError):
        service.mutate(
            ...
            object_id=saved.document.object_id,
            suggestion_id=created.suggestion.suggestion_id,
            command=decision_command("accept", latest.version.version_id),
            write_enabled=True,
        )
    ...
    rejected = service.mutate(
        ...
        object_id=saved.document.object_id,
        suggestion_id=created.suggestion.suggestion_id,
        command=decision_command("reject", latest.version.version_id),
        write_enabled=True,
    )
