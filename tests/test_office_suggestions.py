...
from suite.platform.office_suggestions import (
    OfficeSuggestionService, SuggestionCreateCommand, SuggestionDecisionCommand, replace_suggestion_text,
)
...
class SuggestionHarness:
    ...
    def command(self, reference: str = "suggest-create", replacement: str = "REPLACEMENT") -> SuggestionCreateCommand:
        return SuggestionCreateCommand.model_validate({
            "anchor_version_id": self.version_id, "expected_current_version_id": self.version_id,
            "anchor": {"from": 5, "to": 9}, "replacement_text": replacement,
    ...
    def create(self, reference: str = "suggest-create", replacement: str = "REPLACEMENT") -> Any:
        return self.service.mutate(user_context=self.user, object_id=self.object_id,
                                   command=self.command(reference, replacement), write_enabled=True)

    def decide(self, suggestion_id: str, operation: str = "accept", reference: str = "decide") -> Any:
        return self.service.mutate(
            user_context=self.user, object_id=self.object_id, suggestion_id=suggestion_id, write_enabled=True,
...
def decision_command(operation: str, version_id: str, reference: str = "decide") -> SuggestionDecisionCommand:
    return SuggestionDecisionCommand.model_validate({
        "operation": operation, "expected_revision": 1,
...
def suggestions() -> SuggestionHarness:
    sources = InMemorySourceObjectRepository()
    documents = OfficeDocumentService(
        repository=InMemoryOfficeDocumentRepository(source_repository=sources), source_repository=sources,
        audit=InMemoryAuditLogger(),
    )
    user = UserContext(tenant_id="tenant-suggestions", user_id="editor", role_ids={"office-editor"})
    saved = documents.create(user_context=user, write_enabled=True, command=OfficeDocumentCreateCommand(
        title="Private title", document=text_document(), mutation_reference="initial", human_confirmation=True,
    ))
...
def test_suggestion_accept_saves_exact_new_version_and_terminal_decision(suggestions: SuggestionHarness) -> None:
    ...
    assert len(suggestions.documents.history(user_context=suggestions.user, object_id=suggestions.object_id).versions) == 1
    ...
    historical = suggestions.documents.read_content(user_context=suggestions.user, object_id=suggestions.object_id,
                                                     version_id=suggestions.version_id)
    assert historical.content == text_document()
    detail = suggestions.service.detail(user_context=suggestions.user, object_id=suggestions.object_id,
                                         suggestion_id=created.suggestion.suggestion_id, write_enabled=True)
    ...
    assert len(suggestions.documents.history(user_context=suggestions.user, object_id=suggestions.object_id).versions) == 2
...
def test_suggestion_stale_accept_denied_but_reject_is_durable(suggestions: SuggestionHarness) -> None:
    created = suggestions.create()
    suggestions.documents.save(user_context=suggestions.user, object_id=suggestions.object_id, write_enabled=True,
        command=OfficeDocumentSaveCommand(title="Later", document=text_document("Other content"),
            expected_current_version_id=suggestions.version_id, mutation_reference="later", human_confirmation=True))
    with pytest.raises(OfficeDocumentConflictError):
...
def test_suggestion_exact_retry_is_actor_bound_and_rechecks_acl(suggestions: SuggestionHarness) -> None:
    ...
    accepted = suggestions.decide(created.suggestion.suggestion_id)
    suggestions.documents.save(user_context=suggestions.user, object_id=suggestions.object_id, write_enabled=True,
        command=OfficeDocumentSaveCommand(title="Later", document=text_document("Later content"),
            expected_current_version_id=accepted.suggestion.result_version_id, mutation_reference="later", human_confirmation=True))
...
def test_suggestion_access_fails_before_source_reads(suggestions: SuggestionHarness, monkeypatch: pytest.MonkeyPatch, change: str) -> None:
    created = suggestions.create()
    ...
    with pytest.raises((OfficeDocumentPermissionError, OfficeDocumentNotFoundError)):
        suggestions.service.mutate(user_context=user, object_id=suggestions.object_id,
...
def test_suggestion_writes_require_feature_and_durable_service(suggestions: SuggestionHarness, available: bool, enabled: bool) -> None:
    suggestions.documents.writes_available = available
    assert not suggestions.service.list_suggestions(user_context=suggestions.user, object_id=suggestions.object_id,
                                                     write_enabled=enabled).can_create
    with pytest.raises(OfficeDocumentPermissionError):
        suggestions.service.mutate(user_context=suggestions.user, object_id=suggestions.object_id,
                                    command=suggestions.command(), write_enabled=enabled)
...
def test_suggestion_pagination_is_complete_and_historical_filter_validated(suggestions: SuggestionHarness) -> None:
    ...
    after, seen = None, set()
    while True:
        page = suggestions.service.list_suggestions(user_context=suggestions.user, object_id=suggestions.object_id,
                                                     after=after, limit=3)
    ...
    with pytest.raises(OfficeDocumentNotFoundError):
        suggestions.service.list_suggestions(user_context=suggestions.user, object_id=suggestions.object_id,
                                              anchor_version_id="missing")
...
def test_suggestion_replacement_preserves_cross_mark_structure_and_following_positions() -> None:
    ...
    assert result["content"][0]["content"] == [
        ...
        {"type": "text", "text": "XYZ", "marks": [{"type": "bold"}]}, {"type": "text", "text": "ef"},
    ]
...
def test_suggestion_rejects_noop_cross_break_and_output_limits() -> None:
    ...
    broken = {"type": "doc", "content": [{"type": "paragraph", "content": [
...
def test_suggestion_corruption_and_outage_fail_closed(suggestions: SuggestionHarness, monkeypatch: pytest.MonkeyPatch) -> None:
    created = suggestions.create()
...
@pytest.mark.parametrize("overrides", ({"human_confirmation": False}, {"expected_revision": True}, {"expected_revision": 2}))
def test_suggestion_decisions_require_exact_confirmation_and_revision(overrides: dict[str, Any]) -> None:
    with pytest.raises(ValidationError):
        SuggestionDecisionCommand.model_validate({
            "operation": "reject", "expected_revision": 1, "mutation_reference": "x", "human_confirmation": True,
            **overrides,
        })
