...
from suite.platform.tickets_incidents_activation_dry_run_execution_preflight import (
    ...
    build_tickets_incidents_activation_dry_run_execution_preflight_hash,
...
def _approval_command(
    ...
    return TicketsIncidentsTenantAdminActivationApprovalRecordCommand(
        ...
        approval_record_ref="tickets-approval:dry-run-execution-preflight-demo",
        approval_ticket_ref="ticket:tickets-dry-run-execution-preflight-demo",
        human_confirmation_reference="confirmation:tickets-dry-run-execution-preflight-demo",
        human_confirmation_statement=TICKETS_INCIDENTS_TENANT_ADMIN_ACTIVATION_APPROVAL_RECORD_CONFIRMATION_STATEMENT,
        change_request_ref="change:tickets-dry-run-execution-preflight-demo",
        idempotency_key_ref="idempotency:tickets-dry-run-execution-preflight-demo",
        approved_at_utc=datetime(2026, 7, 9, 8, 0, tzinfo=UTC),
        audit_chain_ref="audit:tickets-dry-run-execution-preflight-demo",
    )
...
def _dry_run_execution_preflight_command(
    *,
    ...
    return TicketsIncidentsActivationDryRunExecutionPreflightCommand(
        ...
        activation_dry_run_execution_preflight_ref="tickets-activation-dry-run-execution-preflight:demo",
        change_request_ref="change:tickets-activation-dry-run-execution-preflight-demo",
        idempotency_key_ref="idempotency:tickets-activation-dry-run-execution-preflight-demo",
        prepared_at_utc=datetime(2026, 7, 9, 8, 50, tzinfo=UTC),
        audit_chain_ref="audit:tickets-activation-dry-run-execution-preflight-demo",
...
def test_tickets_incidents_activation_dry_run_execution_preflight_is_metadata_only_after_runtime_boundary() -> None:
    ...
    response = build_tickets_incidents_activation_dry_run_execution_preflight_response(
    ...
    assert response.schema_version == TICKETS_INCIDENTS_ACTIVATION_DRY_RUN_EXECUTION_PREFLIGHT_SCHEMA_VERSION
    assert response.endpoint == TICKETS_INCIDENTS_ACTIVATION_DRY_RUN_EXECUTION_PREFLIGHT_ENDPOINT
    assert response.result_contract == TICKETS_INCIDENTS_ACTIVATION_DRY_RUN_EXECUTION_PREFLIGHT_RESULT_CONTRACT
    ...
    assert response.activation_dry_run_execution_preflight_statement_hash.startswith("sha256:")
    assert response.preparer_role_allowed is True
    assert response.activation_dry_run_execution_preflight_requested is True
    assert response.activation_dry_run_execution_preflight_ready is True
    assert response.future_activation_dry_run_execution_receipt_boundary_required is True
    ...
    assert (
        ...
        in response.activation_dry_run_execution_preflight_steps
    )
    assert (
        "define_receipt_boundary_required_before_dry_run_execution"
        in response.activation_dry_run_execution_preflight_steps
    )
    assert "future_activation_dry_run_execution_receipt_boundary_required" in (
        response.required_activation_dry_run_execution_preflight_evidence
    )
    assert response.blocking_reasons == ()
    assert response.evidence_hash == build_tickets_incidents_activation_dry_run_execution_preflight_hash(response)
    assert (
...
def test_tickets_incidents_activation_dry_run_execution_preflight_blocks_execution_request() -> None:
    module_registry = default_module_registry()
    ...
    assert response.activation_dry_run_execution_preflight_ready is False
