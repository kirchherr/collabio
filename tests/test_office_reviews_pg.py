...
from suite.platform.office_documents import OfficeDocumentConflictError, OfficeDocumentNotFoundError, OfficeDocumentPermissionError
...
def prepared(database: Database) -> tuple[Any, ...]:
    ...
    create = ReviewCreateCommand.model_validate({"anchor_version_id": saved.version.version_id,
        "expected_current_version_id": saved.version.version_id, "anchor": {"from": 9, "to": 13},
        "body": "PRIVATE review body", "mutation_reference": "review-create", "human_confirmation": True})
...
def review_counts(database: Database, user: Any) -> tuple[int, ...]:
    with psycopg.connect(database.app_dsn) as connection:
        ...
        result = []
        for table in ("office.review_threads", "office.review_events", "collabio.source_object_metadata", "collabio.source_object_write_receipts"):
...
def test_pg_review_complete_loop_is_reopenable_after_reconstructing_service(database: Database) -> None:
    ...
    for revision, operation in enumerate(("reply", "resolve", "reopen"), start=1):
        reviews = build_office_review_service(document_service=service_for(database, store), audit=InMemoryAuditLogger())
        reviews.mutate(user_context=user, object_id=object_id, thread_id=created.thread.thread_id, write_enabled=True,
            command=ReviewEventCommand(operation=operation, expected_revision=revision, mutation_reference=operation,
                human_confirmation=True, body="reply content" if operation == "reply" else None))
    detail = reviews.detail(user_context=user, object_id=object_id, thread_id=created.thread.thread_id, write_enabled=True)
...
def test_pg_review_current_parent_acl_precedes_storage_and_replay(database: Database, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert not reviews.detail(user_context=reader, object_id=object_id, thread_id=created.thread.thread_id, write_enabled=True).can_comment
    ...
    with psycopg.connect(database.admin_dsn) as connection:
        ...
        connection.execute("UPDATE collabio.object_acl_entries SET status='revoked', revoked_at_utc=now() "
            "WHERE tenant_id=%s AND object_id=%s AND acl_subject_id=%s", (user.tenant_id,object_id,user.user_id))
...
def test_pg_competing_review_revision_has_one_commit_and_no_loser_side_effect(database: Database) -> None:
    ...
    def save(index: int) -> str:
        ...
        try:
            worker.mutate(user_context=user, object_id=object_id, thread_id=created.thread.thread_id, write_enabled=True,
                command=ReviewEventCommand(operation="reply", expected_revision=1, body=f"Reply {index}",
                    mutation_reference=f"concurrent-{index}", human_confirmation=True))
...
def test_pg_review_pre_put_outage_rolls_back_thread_and_receipt(database: Database, monkeypatch: pytest.MonkeyPatch) -> None:
    store, _, user, object_id, reviews, create = prepared(database)
...
def test_pg_review_post_put_binding_failure_rolls_back_and_exposes_orphan(database: Database, monkeypatch: pytest.MonkeyPatch) -> None:
...
def test_pg_review_rls_immutable_anchor_events_and_head_cannot_rewind(database: Database) -> None:
    ...
    assert reviews.detail(user_context=user,object_id=object_id,thread_id=created.thread.thread_id).thread.revision == 1
