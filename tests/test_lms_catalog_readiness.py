...
from suite.platform.lms_catalog_readiness import (
    LMS_CATALOG_READINESS_ENDPOINT,
    LMS_CATALOG_READINESS_RESULT_CONTRACT,
    LMS_CATALOG_READINESS_SCHEMA_VERSION,
...
def test_lms_catalog_readiness_declares_metadata_only_registration_boundary() -> None:
    ...
    assert response.schema_version == LMS_CATALOG_READINESS_SCHEMA_VERSION
    assert response.endpoint == LMS_CATALOG_READINESS_ENDPOINT
    assert response.result_contract == LMS_CATALOG_READINESS_RESULT_CONTRACT
    ...
    assert response.summary.object_type_count == 3
...
def test_lms_catalog_readiness_is_tenant_scoped_without_catalog_side_effects() -> None:
    ...
    first = build_lms_catalog_readiness_response(
        user_context=UserContext(tenant_id="tenant-a", user_id="u-1", role_ids={"admin"}),
        module_registry=module_registry,
    )
    second = build_lms_catalog_readiness_response(
