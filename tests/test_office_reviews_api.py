...
def test_review_api_create_reply_resolve_reopen_are_noncacheable_and_literal(
    review_api: ReviewApiHarness, caplog: pytest.LogCaptureFixture
) -> None:
    ...
    for revision, operation in enumerate(("reply", "resolve", "reopen"), start=1):
        response = review_api.office.client.post(
            f"{thread_url}/events",
            headers=review_api.office.headers,
            json={
                "operation": operation,
                "expected_revision": revision,
                "body": REVIEW_BODY if operation == "reply" else None,
...
def test_review_api_gate_changes_precede_mutation_and_cached_capabilities(
    review_api: ReviewApiHarness, monkeypatch: pytest.MonkeyPatch, block: str
) -> None:
    created = review_api.create()
    headers = review_api.office.headers
    if block == "module":
    ...
    event = review_api.office.client.post(
        f"{review_api.base}/{created['thread']['thread_id']}/events",
        headers=headers,
        json={
            "operation": "resolve",
            "expected_revision": 1,
            "mutation_reference": "blocked-state",
            "human_confirmation": True,
        },
    )
...
@pytest.mark.parametrize(
    ...
    [
        ...
        {"body": "x" * 4001},
        {"body": "secret\x00"},
        {"anchor": {"from": True, "to": 3}},
        {"quote": "forged quote"},
    ],
)
def test_review_api_invalid_requests_never_echo_bodies(
    review_api: ReviewApiHarness, update: dict[str, Any], caplog: pytest.LogCaptureFixture
) -> None:
    response = review_api.office.client.post(
        review_api.base, headers=review_api.office.headers, json={**review_api.payload(), **update}
    )
...
def test_review_api_current_acl_revocation_denies_before_source_reads(
    review_api: ReviewApiHarness, monkeypatch: pytest.MonkeyPatch
) -> None:
    created = review_api.create()
    ...
    del review_api.office.repository.grants[
        ("tenant-demo", review_api.object_id, review_api.office.headers["X-User-Id"])
    ]
    for path in (review_api.base, url):
        response = review_api.office.client.get(path, headers=review_api.office.headers)
        assert response.status_code == 404 and response.json() == {"detail": "Document not found"}
    replay = review_api.office.client.post(
        review_api.base, headers=review_api.office.headers, json=review_api.payload()
    )
...
def test_review_api_storage_failures_are_safe503_without_body_logs(
    review_api: ReviewApiHarness, monkeypatch: pytest.MonkeyPatch, operation: str, caplog: pytest.LogCaptureFixture
) -> None:
    created = review_api.create()
    if operation == "read":
        monkeypatch.setattr(
            review_api.service.source_repository, "get", Mock(side_effect=SourceObjectStorageError(REVIEW_BODY))
        )
        response = review_api.office.client.get(
            f"{review_api.base}/{created['thread']['thread_id']}", headers=review_api.office.headers
        )
    elif operation == "write":
        monkeypatch.setattr(
            review_api.service.source_repository, "add", Mock(side_effect=SourceObjectStorageError(REVIEW_BODY))
        )
        response = review_api.office.client.post(
            review_api.base, headers=review_api.office.headers, json=review_api.payload("failed")
        )
    else:
        monkeypatch.setattr(
            review_api.service.repository, "list_threads", Mock(side_effect=psycopg.OperationalError(REVIEW_BODY))
        )
...
def test_review_api_jwt_does_not_accept_forged_parent_grants(
    review_api: ReviewApiHarness, monkeypatch: pytest.MonkeyPatch
) -> None:
    created = review_api.create()
    ...
    payload = {
        ...
        "exp": int(time()) + 120,
        "roles": ["tenant-admin"],
        "readable_object_ids": [review_api.object_id],
    }
    segments = [
...
def test_review_api_memory_runtime_factory_cannot_persist_review(
    review_api: ReviewApiHarness, monkeypatch: pytest.MonkeyPatch
) -> None:
