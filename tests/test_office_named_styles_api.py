...
def test_api_styles_versions_replay_history_and_conflicts(office_api: OfficeApiHarness) -> None:
    ...
    command = {**payload, "document": changed, "mutation_reference": "style-update", "expected_current_version_id": first["version"]["version_id"]}
    ...
    old = office_api.client.get(f"{BASE}/{object_id}/content", headers=office_api.headers, params={"version_id": first["version"]["version_id"]})
    assert old.json()["content"] == payload["document"] and old.headers["cache-control"] == "no-store"
    stale = office_api.client.post(f"{BASE}/{object_id}/versions", headers=office_api.headers, json={**command, "mutation_reference": "style-stale"})
...
def test_api_invalid_styles_do_not_commit_or_echo(office_api: OfficeApiHarness, monkeypatch: pytest.MonkeyPatch, tamper: str) -> None:
    enable_office()
    ...
    response = office_api.client.post(BASE, headers=office_api.headers, json={**create_payload("invalid-style"), "document": document})
