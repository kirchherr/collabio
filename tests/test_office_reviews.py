...
from suite.platform.office_documents import (
    OfficeDocumentConflictError, OfficeDocumentCreateCommand, OfficeDocumentInvalidContentError,
    OfficeDocumentNotFoundError, OfficeDocumentPermissionError, OfficeDocumentSaveCommand, OfficeDocumentService,
)
...
from suite.platform.office_reviews import (
    OfficeReviewService, ReviewAnchor, ReviewCreateCommand, ReviewEventCommand, derive_review_quote,
)
...
class ReviewHarness:
    ...
    def command(self, reference: str = "review-create", body: str = "PRIVATE COMMENT", anchor: dict[str, int] | None = None) -> ReviewCreateCommand:
        return ReviewCreateCommand.model_validate({"anchor_version_id": self.version_id,
            "expected_current_version_id": self.version_id, "anchor": anchor, "body": body,
    ...
    def create(self, reference: str = "review-create", anchor: dict[str, int] | None = None) -> Any:
        return self.service.mutate(user_context=self.user, object_id=self.object_id, command=self.command(reference, anchor=anchor), write_enabled=True)


@pytest.fixture
def reviews() -> ReviewHarness:
    source = InMemorySourceObjectRepository()
    docs = OfficeDocumentService(repository=InMemoryOfficeDocumentRepository(source_repository=source), source_repository=source, audit=InMemoryAuditLogger())
    user = UserContext(tenant_id="tenant-review", user_id="editor", role_ids={"office-editor"})
    saved = docs.create(user_context=user, write_enabled=True, command=OfficeDocumentCreateCommand(title="Private title",
        document=text_document(), mutation_reference="document-create", human_confirmation=True))
    ...
    return ReviewHarness(service, docs, service.repository, source, user, saved.document.object_id, saved.version.version_id)


def event_command(operation: str, revision: int, reference: str, body: str | None = None) -> ReviewEventCommand:
    return ReviewEventCommand.model_validate({"operation": operation, "expected_revision": revision, "body": body,
        "mutation_reference": reference, "human_confirmation": True})
...
def test_review_lifecycle_is_durable_source_backed_without_document_versions(reviews: ReviewHarness) -> None:
    ...
    for revision, operation in enumerate(("reply", "resolve", "reopen"), start=1):
        changed = reviews.service.mutate(user_context=reviews.user, object_id=reviews.object_id, thread_id=thread_id,
            command=event_command(operation, revision, operation, "Reply <script>literal</script>" if operation == "reply" else None), write_enabled=True)
        assert changed.applied_revision == revision + 1
    detail = reviews.service.detail(user_context=reviews.user, object_id=reviews.object_id, thread_id=thread_id, write_enabled=True)
...
def test_review_replay_is_actor_bound_and_requires_current_acl_and_confirmation(reviews: ReviewHarness) -> None:
    ...
    reviews.service.mutate(user_context=reviews.user, object_id=reviews.object_id, thread_id=created.thread.thread_id,
        command=event_command("resolve", 1, "resolve"), write_enabled=True)
    ...
    with pytest.raises(OfficeDocumentConflictError):
        reviews.service.mutate(user_context=reviews.user, object_id=reviews.object_id,
            command=reviews.command(body="different"), write_enabled=True)
    with pytest.raises(ValidationError):
        reviews.service.mutate(user_context=reviews.user, object_id=reviews.object_id,
            command=reviews.command().model_copy(update={"human_confirmation": False}), write_enabled=True)
...
def test_review_historical_discussion_survives_new_document_save(reviews: ReviewHarness) -> None:
    ...
    reviews.documents.save(user_context=reviews.user, object_id=reviews.object_id, write_enabled=True,
        command=OfficeDocumentSaveCommand(title="New title", document=text_document("New contents"), mutation_reference="document-save",
            human_confirmation=True, expected_current_version_id=reviews.version_id))
    listed = reviews.service.list_threads(user_context=reviews.user, object_id=reviews.object_id, anchor_version_id=reviews.version_id, write_enabled=True)
    assert not listed.can_create and listed.can_comment and listed.threads[0].anchor_version_id == reviews.version_id
    changed = reviews.service.mutate(user_context=reviews.user, object_id=reviews.object_id, thread_id=thread.thread_id,
        command=event_command("reply", 1, "old-version-reply", "Still relevant"), write_enabled=True)
    assert changed.thread.anchor_version_id == reviews.version_id
    with pytest.raises(OfficeDocumentConflictError):
        reviews.create("stale-new-thread")
    with pytest.raises(OfficeDocumentNotFoundError):
        reviews.service.list_threads(user_context=reviews.user, object_id=reviews.object_id, anchor_version_id="missing")


def test_review_denies_before_reading_storage_and_read_does_not_grant_write(reviews: ReviewHarness, monkeypatch: pytest.MonkeyPatch) -> None:
    thread = reviews.create().thread
    ...
    reviews.repository.documents.grants[(reviews.user.tenant_id, reviews.object_id, reviews.user.user_id)] = "read"
    detail = reviews.service.detail(user_context=reviews.user, object_id=reviews.object_id, thread_id=thread.thread_id, write_enabled=True)
    assert not detail.can_comment and not detail.can_resolve and detail.events[0].body == "PRIVATE COMMENT"
    with pytest.raises(OfficeDocumentPermissionError):
        reviews.service.mutate(user_context=reviews.user, object_id=reviews.object_id, thread_id=thread.thread_id,
            command=event_command("resolve", 1, "denied-resolve"), write_enabled=True)
...
def test_review_pagination_is_complete_and_state_cas_is_independent(reviews: ReviewHarness) -> None:
    ...
    second = reviews.service.list_threads(user_context=reviews.user, object_id=reviews.object_id, limit=1, after=first.next_cursor)
    assert first.next_cursor and not second.next_cursor and first.threads[0].thread_id != second.threads[0].thread_id
    reviews.service.mutate(user_context=reviews.user, object_id=reviews.object_id, thread_id=thread.thread_id,
        command=event_command("reply", 1, "reply", "another"), write_enabled=True)
    with pytest.raises(OfficeDocumentConflictError):
        reviews.service.mutate(user_context=reviews.user, object_id=reviews.object_id, thread_id=thread.thread_id,
            command=event_command("resolve", 1, "stale"), write_enabled=True)
    page = reviews.service.detail(user_context=reviews.user, object_id=reviews.object_id, thread_id=thread.thread_id, limit=1)
    assert page.next_after_revision == 1
    last = reviews.service.detail(user_context=reviews.user, object_id=reviews.object_id, thread_id=thread.thread_id, after_revision=1, limit=1)
...
def test_anchor_rejects_split_surrogate_or_outside_saved_content(anchor: dict[str, int], reviews: ReviewHarness) -> None:
...
def test_anchor_spans_marks_but_never_hard_break_or_paragraphs() -> None:
    doc = {"type": "doc", "content": [{"type": "paragraph", "content": [
        {"type": "text", "text": "Ca", "marks": [{"type": "bold"}]}, {"type": "text", "text": "fé"},
        {"type": "hardBreak"}, {"type": "text", "text": "other"}]}, text_document("last")["content"][0]]}
...
def test_anchor_nested_table_positions_and_boundaries() -> None:
    doc = {"type": "doc", "content": [{"type": "table", "content": [{"type": "tableRow", "content": [
...
def test_maximum_unicode_body_and_quote_fit_canonical_resource_bound(reviews: ReviewHarness) -> None:
    saved = reviews.documents.save(user_context=reviews.user, object_id=reviews.object_id, write_enabled=True,
        command=OfficeDocumentSaveCommand(title="Unicode", document=text_document("😀" * 2000), mutation_reference="unicode-save",
            human_confirmation=True, expected_current_version_id=reviews.version_id))
    reviews.version_id = saved.version.version_id
    created = reviews.service.mutate(user_context=reviews.user, object_id=reviews.object_id,
        command=reviews.command(body="😀" * 4000, anchor={"from": 1, "to": 4001}), write_enabled=True)
...
def test_storage_outage_and_corrupt_content_fail_closed(reviews: ReviewHarness, monkeypatch: pytest.MonkeyPatch) -> None:
    created = reviews.create()
    ...
    with pytest.raises(SourceObjectStorageError):
        reviews.service.detail(user_context=reviews.user, object_id=reviews.object_id, thread_id=created.thread.thread_id)
    record = original_get(tenant_id=reviews.user.tenant_id, object_id=created.thread.thread_id, version_id=created.event.event_id)
...
def test_write_feature_and_durable_factory_gate_precede_mutation(reviews: ReviewHarness, monkeypatch: pytest.MonkeyPatch) -> None:
