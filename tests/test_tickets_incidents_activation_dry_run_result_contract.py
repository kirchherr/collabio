...
from suite.platform.tickets_incidents_activation_dry_run_executor_implementation_review import (
    TICKETS_INCIDENTS_ACTIVATION_DRY_RUN_EXECUTOR_IMPLEMENTATION_REVIEW_STATEMENT,
    TicketsIncidentsActivationDryRunExecutorImplementationReviewCommand,
    build_tickets_incidents_activation_dry_run_executor_implementation_review_response,
)
...
from suite.platform.tickets_incidents_activation_dry_run_result_contract import (
    ...
    TicketsIncidentsActivationDryRunResultContractCommand,
    build_tickets_incidents_activation_dry_run_result_contract_hash,
    build_tickets_incidents_activation_dry_run_result_contract_response,
)
...
def _approval_command(
    ...
    return TicketsIncidentsTenantAdminActivationApprovalRecordCommand(
        ...
        approval_record_ref="tickets-approval:result-contract-demo",
        approval_ticket_ref="ticket:tickets-result-contract-demo",
        human_confirmation_reference="confirmation:tickets-result-contract-demo",
        human_confirmation_statement=TICKETS_INCIDENTS_TENANT_ADMIN_ACTIVATION_APPROVAL_RECORD_CONFIRMATION_STATEMENT,
        change_request_ref="change:tickets-result-contract-demo",
        idempotency_key_ref="idempotency:tickets-result-contract-demo",
        approved_at_utc=datetime(2026, 7, 9, 8, 0, tzinfo=UTC),
        audit_chain_ref="audit:tickets-result-contract-demo",
    )
...
def _boundary_command(
    *,
    ...
    return TicketsIncidentsActivationExecutionBoundaryCommand(
        ...
        activation_execution_boundary_ref="tickets-activation-boundary:result-contract-demo",
        change_request_ref="change:tickets-activation-boundary-result-contract-demo",
        idempotency_key_ref="idempotency:tickets-activation-boundary-result-contract-demo",
        reviewed_at_utc=datetime(2026, 7, 9, 8, 5, tzinfo=UTC),
        audit_chain_ref="audit:tickets-activation-boundary-result-contract-demo",
...
def _skeleton_command(
    *,
    ...
    return TicketsIncidentsActivationExecutorSkeletonCommand(
        ...
        tenant_admin_approval_gate_hash=approval_gate_hash,
        tenant_admin_approval_record_hash=approval_record_hash,
        activation_executor_skeleton_ref="tickets-activation-executor-skeleton:result-contract-demo",
        change_request_ref="change:tickets-activation-executor-skeleton-result-contract-demo",
        idempotency_key_ref="idempotency:tickets-activation-executor-skeleton-result-contract-demo",
        prepared_at_utc=datetime(2026, 7, 9, 8, 10, tzinfo=UTC),
        audit_chain_ref="audit:tickets-activation-executor-skeleton-result-contract-demo",
...
def _dry_run_plan_command(
    *,
    ...
    return TicketsIncidentsActivationDryRunPlanCommand(
        ...
        tenant_admin_approval_gate_hash=approval_gate_hash,
        tenant_admin_approval_record_hash=approval_record_hash,
        activation_dry_run_plan_ref="tickets-activation-dry-run-plan:result-contract-demo",
        change_request_ref="change:tickets-activation-dry-run-plan-result-contract-demo",
        idempotency_key_ref="idempotency:tickets-activation-dry-run-plan-result-contract-demo",
        planned_at_utc=datetime(2026, 7, 9, 8, 15, tzinfo=UTC),
        audit_chain_ref="audit:tickets-activation-dry-run-plan-result-contract-demo",
    ...
    *,
    ...
    return TicketsIncidentsActivationDryRunExecutionBoundaryCommand(
        ...
        activation_dry_run_execution_boundary_ref=(
            "tickets-activation-dry-run-execution-boundary:result-contract-demo"
        ),
        change_request_ref="change:tickets-activation-dry-run-execution-boundary-result-contract-demo",
        idempotency_key_ref="idempotency:tickets-activation-dry-run-execution-boundary-result-contract-demo",
        reviewed_at_utc=datetime(2026, 7, 9, 8, 20, tzinfo=UTC),
        audit_chain_ref="audit:tickets-activation-dry-run-execution-boundary-result-contract-demo",
    ...
    *,
    ...
    return TicketsIncidentsActivationDryRunExecutionSkeletonCommand(
        ...
        activation_dry_run_execution_skeleton_ref=(
            "tickets-activation-dry-run-execution-skeleton:result-contract-demo"
        ),
        change_request_ref="change:tickets-activation-dry-run-execution-skeleton-result-contract-demo",
        idempotency_key_ref="idempotency:tickets-activation-dry-run-execution-skeleton-result-contract-demo",
        prepared_at_utc=datetime(2026, 7, 9, 8, 25, tzinfo=UTC),
        audit_chain_ref="audit:tickets-activation-dry-run-execution-skeleton-result-contract-demo",
...
def _dry_run_executor_implementation_review_command(
    *,
    ...
    return TicketsIncidentsActivationDryRunExecutorImplementationReviewCommand(
        activation_dry_run_plan_evidence_hash=activation_dry_run_plan_hash,
        activation_dry_run_execution_boundary_evidence_hash=activation_dry_run_execution_boundary_hash,
        ...
        activation_dry_run_executor_implementation_review_ref=(
            "tickets-activation-dry-run-executor-implementation-review:result-contract-demo"
        ),
        change_request_ref="change:tickets-activation-dry-run-executor-implementation-review-result-contract-demo",
        idempotency_key_ref="idempotency:tickets-activation-dry-run-executor-implementation-review-result-contract-demo",
        reviewed_at_utc=datetime(2026, 7, 9, 8, 30, tzinfo=UTC),
        audit_chain_ref="audit:tickets-activation-dry-run-executor-implementation-review-result-contract-demo",
...
def _dry_run_result_contract_command(
    *,
    ...
    return TicketsIncidentsActivationDryRunResultContractCommand(
        activation_dry_run_plan_evidence_hash=activation_dry_run_plan_hash,
        activation_dry_run_execution_boundary_evidence_hash=activation_dry_run_execution_boundary_hash,
        activation_dry_run_execution_skeleton_evidence_hash=activation_dry_run_execution_skeleton_hash,
        ...
        tenant_admin_approval_gate_hash=approval_gate_hash,
        tenant_admin_approval_record_hash=approval_record_hash,
        activation_dry_run_result_contract_ref="tickets-activation-dry-run-result-contract:demo",
        change_request_ref="change:tickets-activation-dry-run-result-contract-demo",
        idempotency_key_ref="idempotency:tickets-activation-dry-run-result-contract-demo",
        prepared_at_utc=datetime(2026, 7, 9, 8, 35, tzinfo=UTC),
        audit_chain_ref="audit:tickets-activation-dry-run-result-contract-demo",
        activation_dry_run_result_contract_statement=TICKETS_INCIDENTS_ACTIVATION_DRY_RUN_RESULT_CONTRACT_STATEMENT,
...
def test_tickets_incidents_activation_dry_run_result_contract_is_metadata_only_after_review() -> None:
    ...
    activation_dry_run_execution_skeleton = build_tickets_incidents_activation_dry_run_execution_skeleton_response(
    ...
    implementation_review = build_tickets_incidents_activation_dry_run_executor_implementation_review_response(
        command=_dry_run_executor_implementation_review_command(
            activation_dry_run_plan_hash=activation_dry_run_plan.evidence_hash,
            activation_dry_run_execution_boundary_hash=activation_dry_run_execution_boundary.evidence_hash,
            activation_dry_run_execution_skeleton_hash=activation_dry_run_execution_skeleton.evidence_hash,
            activation_execution_boundary_hash=activation_boundary.evidence_hash,
            activation_executor_skeleton_hash=activation_executor_skeleton.evidence_hash,
            approval_gate_hash=approval_gate.evidence_hash,
            approval_record_hash=approval_record.evidence_hash,
    ...
    response = build_tickets_incidents_activation_dry_run_result_contract_response(
        command=_dry_run_result_contract_command(
            activation_dry_run_plan_hash=activation_dry_run_plan.evidence_hash,
            activation_dry_run_execution_boundary_hash=activation_dry_run_execution_boundary.evidence_hash,
            activation_dry_run_execution_skeleton_hash=activation_dry_run_execution_skeleton.evidence_hash,
            activation_dry_run_executor_implementation_review_hash=implementation_review.evidence_hash,
            activation_execution_boundary_hash=activation_boundary.evidence_hash,
            activation_executor_skeleton_hash=activation_executor_skeleton.evidence_hash,
            approval_gate_hash=approval_gate.evidence_hash,
            approval_record_hash=approval_record.evidence_hash,
    ...
    assert response.schema_version == TICKETS_INCIDENTS_ACTIVATION_DRY_RUN_RESULT_CONTRACT_SCHEMA_VERSION
    assert response.endpoint == TICKETS_INCIDENTS_ACTIVATION_DRY_RUN_RESULT_CONTRACT_ENDPOINT
    assert response.result_contract == TICKETS_INCIDENTS_ACTIVATION_DRY_RUN_RESULT_CONTRACT_RESULT_CONTRACT
    ...
    assert (
        response.activation_dry_run_execution_skeleton_evidence_hash
        == activation_dry_run_execution_skeleton.evidence_hash
    )
    assert (
        response.activation_dry_run_executor_implementation_review_evidence_hash == implementation_review.evidence_hash
    )
    ...
    assert response.activation_dry_run_result_contract_statement_hash.startswith("sha256:")
    assert response.preparer_role_allowed is True
    assert response.activation_dry_run_result_contract_requested is True
    assert response.activation_dry_run_result_contract_ready is True
    assert response.future_dry_run_execution_gate_required is True
    ...
    assert "define_activation_dry_run_result_receipt_schema_version" in (
        response.activation_dry_run_result_contract_steps
    )
    assert "future_activation_dry_run_execution_gate_required" in (
        response.required_activation_dry_run_result_contract_evidence
    )
    assert response.blocking_reasons == ()
    assert response.evidence_hash == build_tickets_incidents_activation_dry_run_result_contract_hash(response)
    assert response.next_action == "prepare_tickets_incidents_activation_dry_run_execution_gate_without_execution"
    assert module_registry.get_tenant_module_or_none(tenant_id="tenant-demo", module_id="tickets_incidents") is None


def test_tickets_incidents_activation_dry_run_result_contract_blocks_execution_request() -> None:
    module_registry = default_module_registry()
    ...
    assert response.activation_dry_run_result_contract_ready is False
    ...
    assert response.dry_run_result_persistence_allowed is False
    assert response.destructive_actions_allowed is False
    assert response.next_action == "prepare_tickets_incidents_activation_dry_run_result_contract_without_execution"
    assert module_registry.get_tenant_module_or_none(tenant_id="tenant-demo", module_id="tickets_incidents") is None
