...
def test_api_character_versions_exact_retry_legacy_reads_and_feature_denial(office_api: OfficeApiHarness) -> None:
    ...
    command = {**payload, "document": character_recovery_document(2), "mutation_reference": "character-save",
    ...
    old = office_api.client.get(f"{BASE}/{object_id}/content", headers=office_api.headers,
    ...
    stale = office_api.client.post(f"{BASE}/{object_id}/versions", headers=office_api.headers,
        json={**command, "mutation_reference": "stale"})
...
@pytest.mark.parametrize("attrs", [{}, {"fontSize": True}, {"fontSize": 18.0}, {"textColor": "SECRET"},
    {"fontSize": 12, "style": "SECRET"}, {"textColor": None}])
def test_api_invalid_character_attributes_never_commit_or_echo(
    ...
    response = office_api.client.post(BASE, headers=office_api.headers,
        json={**create_payload("character-invalid"), "document": content})
