...
from suite.platform.lms_package_installation_dry_run_execution_plan import (
    ...
    LMS_PACKAGE_INSTALLATION_DRY_RUN_EXECUTION_PLAN_SCHEMA_VERSION,
    LMS_PACKAGE_INSTALLATION_DRY_RUN_EXECUTION_PLAN_STATEMENT,
    LmsPackageInstallationDryRunExecutionPlanCommand,
    build_lms_package_installation_dry_run_execution_plan_hash,
...
def test_lms_dry_run_execution_plan_is_metadata_only_and_binds_operator_controls() -> None:
    module_registry = default_module_registry()
    ...
    response = build_lms_package_installation_dry_run_execution_plan_response(
        command=_execution_plan_command(approval_record_hash=execution_approval_record_hash),
        user_context=user_context,
        module_registry=module_registry,
    ...
    assert response.schema_version == LMS_PACKAGE_INSTALLATION_DRY_RUN_EXECUTION_PLAN_SCHEMA_VERSION
    assert response.endpoint == LMS_PACKAGE_INSTALLATION_DRY_RUN_EXECUTION_PLAN_ENDPOINT
    assert response.result_contract == LMS_PACKAGE_INSTALLATION_DRY_RUN_EXECUTION_PLAN_RESULT_CONTRACT
    ...
    assert response.dry_run_execution_plan_statement_hash.startswith("sha256:")
    assert response.preparer_role_allowed is True
    assert response.dry_run_execution_plan_requested is True
    assert response.dry_run_execution_plan_ready is True
    ...
    assert response.future_dry_run_execution_plan_review_required is True
    ...
    assert "bind_execution_window_ref" in response.dry_run_execution_plan_steps
    assert "scheduler_not_activated" in response.required_dry_run_execution_plan_evidence
    assert "worker_image_resolution_disabled" in response.required_dry_run_execution_plan_evidence
    assert response.blocking_reasons == ()
    assert response.evidence_hash == build_lms_package_installation_dry_run_execution_plan_hash(response)
...
def test_lms_dry_run_execution_plan_blocks_missing_evidence_and_runtime_flags() -> None:
    module_registry = default_module_registry()
    user_context = UserContext(tenant_id="tenant-demo", user_id="reader-1", role_ids={"knowledge-worker"})

    response = build_lms_package_installation_dry_run_execution_plan_response(
        command=_execution_plan_command(
    ...
    assert response.dry_run_execution_plan_ready is False
