...
from office_recovery_proof import require_office_recovery_environment, verify_restored_style_versions
...
def test_style_seed_rejects_normal_environment_before_database_access() -> None:
    ...
    with pytest.raises(RuntimeError):
        seed_synthetic_office_styles(environment=environment, client=client)
    assert not client.mock_calls


def test_style_restore_target_requires_matching_fixed_pair() -> None:
    env = recovery_environment()
    for key in ("SUITE_POSTGRES_RESTORE_TARGET_DSN", "SUITE_OFFICE_RECOVERY_TARGET_DSN"):
        env[key] = env[key].replace("/collabio_work_e2e_restore", "/collabio_work_e2e_267_restore")
...
def test_recovery_binds_designated_legacy_and_formatted_sources_without_body_evidence(tamper: str | None) -> None:
    ...
    created = fixture.service.create(
        ...
        command=OfficeDocumentCreateCommand(
            title=STYLE_RECOVERY_TITLE,
            document=style_recovery_document(1),
            mutation_reference="work-e2e-style-recovery-1",
    ...
    current = created
    for number in (2, 3):
        content = style_recovery_document(number)
        if tamper == "format" and number == 2:
            content["attrs"]["styles"][0]["character"]["textColor"] = "red"
        current = fixture.service.save(
            ...
            command=OfficeDocumentSaveCommand(
                title=STYLE_RECOVERY_TITLE,
                document=content,
                mutation_reference=f"work-e2e-style-recovery-{number}",
                human_confirmation=True,
