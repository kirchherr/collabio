-- 0064_productivity_pilot_runtime_window.sql
-- Designated-principal runtime windows and metadata-only authorization observations.

CREATE TABLE IF NOT EXISTS collabio.productivity_pilot_runtime_windows (
    ...
    designated_principal_ids jsonb NOT NULL CHECK (
        jsonb_typeof(designated_principal_ids) = 'array'
        AND jsonb_array_length(designated_principal_ids) BETWEEN 1 AND 25
    ),
    ...
    allowed_api_operations jsonb NOT NULL CHECK (
        ...
        AND jsonb_array_length(allowed_api_operations) = 7
    ),
    ...
    window_record jsonb NOT NULL CHECK (
        ...
        AND window_record ->> 'schema_version' = 'productivity_pilot_runtime_window.v1'
        ...
        AND window_record -> 'designated_principal_ids' = designated_principal_ids
    ...
    schema_version text NOT NULL DEFAULT 'productivity_pilot_runtime_window.v1' CHECK (
        schema_version = 'productivity_pilot_runtime_window.v1'
    ),
...
COMMENT ON TABLE collabio.productivity_pilot_runtime_windows IS
    'Append-only runtime windows that restrict controlled pilot traffic to designated tenant principals.';

ALTER TABLE collabio.productivity_pilot_runtime_windows ENABLE ROW LEVEL SECURITY;
ALTER TABLE collabio.productivity_pilot_runtime_windows FORCE ROW LEVEL SECURITY;

CREATE POLICY productivity_pilot_runtime_windows_tenant_select
    ON collabio.productivity_pilot_runtime_windows
    FOR SELECT USING (tenant_id = collabio.current_tenant_id());

CREATE POLICY productivity_pilot_runtime_windows_tenant_insert
    ON collabio.productivity_pilot_runtime_windows
    FOR INSERT WITH CHECK (tenant_id = collabio.current_tenant_id());

CREATE POLICY productivity_pilot_runtime_windows_no_update
    ON collabio.productivity_pilot_runtime_windows
    FOR UPDATE USING (false);

CREATE POLICY productivity_pilot_runtime_windows_no_hard_delete
    ON collabio.productivity_pilot_runtime_windows
    FOR DELETE USING (false);

CREATE TRIGGER productivity_pilot_runtime_windows_append_only
BEFORE UPDATE OR DELETE ON collabio.productivity_pilot_runtime_windows
FOR EACH ROW EXECUTE FUNCTION collabio.reject_productivity_pilot_evidence_mutation();

CREATE TABLE IF NOT EXISTS collabio.productivity_pilot_runtime_observations (
    tenant_id text NOT NULL CHECK (tenant_id <> ''),
    ...
    operation text NOT NULL CHECK (operation ~ '^(GET|POST|PUT|PATCH|DELETE) /v1/[a-z0-9_./{}-]+$'),
    observed_at_utc timestamptz NOT NULL,
    observation_record jsonb NOT NULL CHECK (
        jsonb_typeof(observation_record) = 'object'
        AND observation_record ->> 'schema_version' = 'productivity_pilot_runtime_observation.v1'
    ...
    evidence_hash text NOT NULL CHECK (evidence_hash ~ '^sha256:[a-f0-9]{64}$'),
    created_at_utc timestamptz NOT NULL DEFAULT now(),
    schema_version text NOT NULL DEFAULT 'productivity_pilot_runtime_observation.v1' CHECK (
        schema_version = 'productivity_pilot_runtime_observation.v1'
    ),
...
COMMENT ON TABLE collabio.productivity_pilot_runtime_observations IS
    'Append-only metadata-only observations of designated-principal authorization at the pilot route boundary.';

ALTER TABLE collabio.productivity_pilot_runtime_observations ENABLE ROW LEVEL SECURITY;
ALTER TABLE collabio.productivity_pilot_runtime_observations FORCE ROW LEVEL SECURITY;

CREATE POLICY productivity_pilot_runtime_observations_tenant_select
    ON collabio.productivity_pilot_runtime_observations
    FOR SELECT USING (tenant_id = collabio.current_tenant_id());

CREATE POLICY productivity_pilot_runtime_observations_tenant_insert
    ON collabio.productivity_pilot_runtime_observations
    FOR INSERT WITH CHECK (tenant_id = collabio.current_tenant_id());

CREATE POLICY productivity_pilot_runtime_observations_no_update
    ON collabio.productivity_pilot_runtime_observations
    FOR UPDATE USING (false);

CREATE POLICY productivity_pilot_runtime_observations_no_hard_delete
    ON collabio.productivity_pilot_runtime_observations
    FOR DELETE USING (false);

CREATE TRIGGER productivity_pilot_runtime_observations_append_only
BEFORE UPDATE OR DELETE ON collabio.productivity_pilot_runtime_observations
FOR EACH ROW EXECUTE FUNCTION collabio.reject_productivity_pilot_evidence_mutation();

DO $$
BEGIN
    IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'collabio_authz_admin') THEN
        EXECUTE 'GRANT SELECT, INSERT ON TABLE collabio.productivity_pilot_runtime_windows TO collabio_authz_admin';
        EXECUTE 'GRANT SELECT, INSERT ON TABLE collabio.productivity_pilot_runtime_observations TO collabio_authz_admin';
    END IF;
END
$$;
