-- 0074_tickets_incidents_tenant_activation_approval_records.sql
-- Durable tenant-scoped readiness approval metadata. This table never activates the module.
...
CREATE TABLE IF NOT EXISTS tickets.tenant_admin_activation_approval_records (
    ...
    approval_gate_evidence_hash text NOT NULL CHECK (
        approval_gate_evidence_hash ~ '^sha256:[a-f0-9]{64}$'
    ),
    ...
    approval_record jsonb NOT NULL CHECK (
        ...
        AND (approval_record ->> 'approval_record_created')::boolean = true
        AND (approval_record ->> 'activation_execution_allowed')::boolean = false
        ...
        AND (approval_record ->> 'worker_activation_allowed')::boolean = false
    ),
    evidence_hash text NOT NULL CHECK (evidence_hash ~ '^sha256:[a-f0-9]{64}$'),
    created_at_utc timestamptz NOT NULL DEFAULT now(),
    schema_version text NOT NULL DEFAULT
        'tickets_incidents_tenant_admin_activation_approval_record.v1' CHECK (
            schema_version = 'tickets_incidents_tenant_admin_activation_approval_record.v1'
        ),
...
COMMENT ON TABLE tickets.tenant_admin_activation_approval_records IS
    'Append-only tenant-admin readiness approval metadata. No content, job, worker, provisioning or activation state.';
...
CREATE POLICY tickets_tenant_activation_approval_records_tenant_select
    ON tickets.tenant_admin_activation_approval_records
...
BEGIN
    IF EXISTS (SELECT 1 FROM pg_roles WHERE rolname = 'collabio_app') THEN
        EXECUTE 'GRANT SELECT, INSERT ON TABLE tickets.tenant_admin_activation_approval_records TO collabio_app';
    END IF;
END
$$;

UPDATE collabio.module_catalog
SET required_migration_versions = '["0051", "0052", "0053", "0054", "0074"]'::jsonb
