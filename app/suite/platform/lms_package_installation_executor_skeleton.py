...
LMS_PACKAGE_INSTALLATION_EXECUTOR_SKELETON_SCHEMA_VERSION = "lms_package_installation_executor_skeleton.v1"
LMS_PACKAGE_INSTALLATION_EXECUTOR_SKELETON_RESULT_CONTRACT = (
    "metadata_only_lms_package_installation_executor_skeleton_no_execution"
)
...
LMS_PACKAGE_INSTALLATION_EXECUTOR_SKELETON_PREPARATION_STATEMENT = (
    "I prepare the LMS package installation executor skeleton without executing installation."
)
LMS_PACKAGE_INSTALLATION_EXECUTOR_SKELETON_READY_NEXT_ACTION = (
    "prepare_lms_package_installation_dry_run_without_tenant_activation"
)
LMS_PACKAGE_INSTALLATION_EXECUTOR_SKELETON_RETRY_NEXT_ACTION = (
...
class LmsPackageInstallationExecutorSkeletonCommand(BaseModel):
    model_config = ConfigDict(extra="forbid")

    execution_boundary_evidence_hash: str
    tenant_admin_approval_gate_hash: str
    tenant_admin_approval_record_hash: str
    executor_skeleton_ref: str
    change_request_ref: str
    idempotency_key_ref: str
    prepared_at_utc: datetime
    audit_chain_ref: str
    executor_skeleton_preparation_statement: str
    executor_skeleton_requested: bool = True
    ...
    def require_sha256(cls, value: str) -> str:
        if not SHA256_PATTERN.fullmatch(value):
            raise ValueError("LMS executor skeleton hashes must be sha256 references")
        return value

    @field_validator("executor_skeleton_ref", "change_request_ref", "idempotency_key_ref", "audit_chain_ref")
    @classmethod
    def require_ref(cls, value: str) -> str:
        if not REF_PATTERN.fullmatch(value.strip()):
            raise ValueError("LMS executor skeleton references must use a typed ref prefix")
        return value.strip()

    @field_validator("executor_skeleton_preparation_statement")
    @classmethod
    def require_exact_preparation_statement(cls, value: str) -> str:
        normalized = value.strip()
        if normalized != LMS_PACKAGE_INSTALLATION_EXECUTOR_SKELETON_PREPARATION_STATEMENT:
            raise ValueError("LMS executor skeleton requires the exact preparation statement")
        return normalized
    ...
    def require_timezone_aware_timestamp(cls, value: datetime) -> datetime:
        if value.tzinfo is None or value.utcoffset() is None:
            raise ValueError("LMS executor skeleton prepared_at_utc must include a timezone")
        return value


class LmsPackageInstallationExecutorSkeletonSummary(BaseModel):
    model_config = ConfigDict(extra="forbid")
...
class LmsPackageInstallationExecutorSkeletonResponse(BaseModel):
    model_config = ConfigDict(extra="forbid")

    schema_version: str = LMS_PACKAGE_INSTALLATION_EXECUTOR_SKELETON_SCHEMA_VERSION
    tenant_id: str
    module_id: str = LMS_MODULE_ID
    endpoint: str = LMS_PACKAGE_INSTALLATION_EXECUTOR_SKELETON_ENDPOINT
    result_contract: str = LMS_PACKAGE_INSTALLATION_EXECUTOR_SKELETON_RESULT_CONTRACT
    ...
    executor_skeleton_preparation_statement_hash: str
    executor_skeleton_ref: str
    change_request_ref: str
    audit_chain_ref: str
    prepared_by: str
    ...
    executor_skeleton_requested: bool
    executor_skeleton_prepared: bool
    package_installation_executor_implementation_required: bool = True
    package_installation_dry_run_required: bool = True
    ...
    summary: LmsPackageInstallationExecutorSkeletonSummary
    ...
    def require_non_empty_text(cls, value: str) -> str:
        if not value.strip():
            raise ValueError("LMS executor skeleton text fields must not be empty")
        return value
    ...
    @field_validator(
        ...
        "executor_skeleton_preparation_statement_hash",
        "evidence_hash",
    )
    @classmethod
    ...
    def require_unique_lists(cls, value: tuple[str, ...]) -> tuple[str, ...]:
        if len(set(value)) != len(value):
            raise ValueError("LMS executor skeleton lists must not contain duplicates")
        for item in value:
            if not item.strip():
                raise ValueError("LMS executor skeleton list items must not be empty")
        return value

    @model_validator(mode="after")
    def require_non_executing_skeleton_contract(self) -> LmsPackageInstallationExecutorSkeletonResponse:
        if self.schema_version != LMS_PACKAGE_INSTALLATION_EXECUTOR_SKELETON_SCHEMA_VERSION:
            raise ValueError("LMS executor skeleton schema version is invalid")
        if self.endpoint != LMS_PACKAGE_INSTALLATION_EXECUTOR_SKELETON_ENDPOINT:
            raise ValueError("LMS executor skeleton endpoint is invalid")
        if self.result_contract != LMS_PACKAGE_INSTALLATION_EXECUTOR_SKELETON_RESULT_CONTRACT:
            raise ValueError("LMS executor skeleton result contract is invalid")
        if self.module_id != LMS_MODULE_ID:
            raise ValueError("LMS executor skeleton only applies to lms")
        if self.continuity_domain != LMS_CONTINUITY_DOMAIN:
            raise ValueError("LMS executor skeleton continuity domain is invalid")
        expected_prepared = (
            self.package_installation_ready
            and self.preparer_role_allowed
            and self.executor_skeleton_requested
            and not self.blocking_reasons
        )
        if self.executor_skeleton_prepared != expected_prepared:
            raise ValueError("LMS executor skeleton preparation must match prerequisites")
        if not self.package_installation_executor_implementation_required:
            raise ValueError("LMS executor skeleton must still require a future executor implementation")
        if not self.package_installation_dry_run_required:
            raise ValueError("LMS executor skeleton must require a future dry run")
        if (
            ...
            raise ValueError("LMS executor skeleton must remain metadata-only and non-executing")
        if self.summary.skeleton_step_count != len(self.skeleton_steps):
            raise ValueError("LMS executor skeleton step count must match skeleton steps")
        if self.summary.required_executor_skeleton_evidence_count != len(self.required_executor_skeleton_evidence):
            raise ValueError("LMS executor skeleton evidence count must match evidence list")
        if self.summary.blocking_reason_count != len(self.blocking_reasons):
            raise ValueError("LMS executor skeleton blocking count must match blocking reasons")
        return self


def build_lms_package_installation_executor_skeleton_response(
    *,
    ...
    command_hash = build_lms_package_installation_executor_skeleton_command_hash(command)
    idempotency_key_hash = stable_hash(
        canonical_json(
            {
                "schema_version": "lms_package_installation_executor_skeleton_idempotency_key.v1",
    ...
    preparation_statement_hash = stable_hash(command.executor_skeleton_preparation_statement)
    preparer_role_allowed = bool({"tenant-admin", "tenant_admin"} & user_context.role_ids)
    blocking_reasons = _executor_skeleton_blocking_reasons(
        command=command,
    ...
    skeleton_steps = (
        ...
        "bind_package_installation_execution_boundary_hash",
        "plan_metadata_schema_presence_checks",
        "plan_tenant_module_state_creation_as_future_dry_run_only",
        "defer_lms_business_api_activation",
        "defer_content_payload_creation",
    ...
    required_executor_skeleton_evidence = (
        ...
        "exact_executor_skeleton_preparation_statement_hash",
        "change_request_ref",
        "idempotency_key_hash",
        "audit_chain_ref",
        "future_dry_run_required",
        "no_lms_installation_execution_confirmation",
    )
    draft = LmsPackageInstallationExecutorSkeletonResponse(
        tenant_id=user_context.tenant_id,
        package_installation_ready=readiness.package_installation_ready,
        ...
        executor_skeleton_preparation_statement_hash=preparation_statement_hash,
        executor_skeleton_ref=command.executor_skeleton_ref,
        ...
        preparer_role_allowed=preparer_role_allowed,
        executor_skeleton_requested=command.executor_skeleton_requested,
        executor_skeleton_prepared=skeleton_prepared,
        ...
        next_action=(
            ...
            else LMS_PACKAGE_INSTALLATION_EXECUTOR_SKELETON_RETRY_NEXT_ACTION
        ),
    )
    return draft.model_copy(update={"evidence_hash": build_lms_package_installation_executor_skeleton_hash(draft)})


def build_lms_package_installation_executor_skeleton_command_hash(
    command: LmsPackageInstallationExecutorSkeletonCommand,
) -> str:
    payload = command.model_dump(mode="json", exclude={"executor_skeleton_preparation_statement"})
    payload["executor_skeleton_preparation_statement_hash"] = stable_hash(
        command.executor_skeleton_preparation_statement
    )
    return stable_hash(canonical_json(payload))


def build_lms_package_installation_executor_skeleton_hash(
    response: LmsPackageInstallationExecutorSkeletonResponse,
) -> str:
    return stable_hash(canonical_json(response.model_dump(mode="json", exclude={"evidence_hash"})))


def _executor_skeleton_blocking_reasons(
    *,
    command: LmsPackageInstallationExecutorSkeletonCommand,
    package_installation_ready: bool,
    ...
    if not command.executor_skeleton_requested:
        reasons.append("executor_skeleton_not_requested")
