...
TICKETS_INCIDENTS_ACTIVATION_DRY_RUN_EXECUTION_REQUEST_BOUNDARY_SCHEMA_VERSION = (
    "tickets_incidents_activation_dry_run_execution_request_boundary.v1"
)
TICKETS_INCIDENTS_ACTIVATION_DRY_RUN_EXECUTION_REQUEST_BOUNDARY_RESULT_CONTRACT = (
    "metadata_only_tickets_incidents_activation_dry_run_execution_request_boundary_no_execution"
)
TICKETS_INCIDENTS_ACTIVATION_DRY_RUN_EXECUTION_REQUEST_BOUNDARY_ENDPOINT = (
...
TICKETS_INCIDENTS_ACTIVATION_DRY_RUN_EXECUTION_REQUEST_BOUNDARY_STATEMENT = (
    "I prepare the Tickets & Incidents activation dry-run execution request boundary without executing "
    "dry-run or tenant activation."
)
TICKETS_INCIDENTS_ACTIVATION_DRY_RUN_EXECUTION_REQUEST_BOUNDARY_READY_NEXT_ACTION = (
    "prepare_tickets_incidents_activation_dry_run_executor_runtime_boundary_without_execution"
)
TICKETS_INCIDENTS_ACTIVATION_DRY_RUN_EXECUTION_REQUEST_BOUNDARY_RETRY_NEXT_ACTION = (
...
class TicketsIncidentsActivationDryRunExecutionRequestBoundaryCommand(BaseModel):
    ...
    activation_dry_run_execution_request_boundary_ref: str
    change_request_ref: str
    idempotency_key_ref: str
    prepared_at_utc: datetime
    audit_chain_ref: str
    activation_dry_run_execution_request_boundary_statement: str
    activation_dry_run_execution_request_boundary_requested: bool = True
    ...
    def require_sha256(cls, value: str) -> str:
        if not SHA256_PATTERN.fullmatch(value):
            raise ValueError(
                "Tickets & Incidents activation dry-run execution request boundary hashes must be sha256 references"
            )
        return value
    ...
    def require_ref(cls, value: str) -> str:
        if not REF_PATTERN.fullmatch(value.strip()):
            raise ValueError(
                "Tickets & Incidents activation dry-run execution request boundary references need typed ref prefixes"
            )
        return value.strip()

    @field_validator("activation_dry_run_execution_request_boundary_statement")
    @classmethod
    def require_exact_statement(cls, value: str) -> str:
        normalized = value.strip()
        if normalized != TICKETS_INCIDENTS_ACTIVATION_DRY_RUN_EXECUTION_REQUEST_BOUNDARY_STATEMENT:
            raise ValueError(
                "Tickets & Incidents activation dry-run execution request boundary requires the exact statement"
            )
        return normalized
    ...
    def require_timezone_aware_timestamp(cls, value: datetime) -> datetime:
        if value.tzinfo is None or value.utcoffset() is None:
            raise ValueError(
                "Tickets & Incidents activation dry-run execution request boundary prepared_at_utc needs a timezone"
            )
        return value


class TicketsIncidentsActivationDryRunExecutionRequestBoundarySummary(BaseModel):
    model_config = ConfigDict(extra="forbid")

    activation_dry_run_execution_request_boundary_step_count: int
    required_activation_dry_run_execution_request_boundary_evidence_count: int
    blocking_reason_count: int


class TicketsIncidentsActivationDryRunExecutionRequestBoundaryResponse(BaseModel):
    model_config = ConfigDict(extra="forbid")

    schema_version: str = TICKETS_INCIDENTS_ACTIVATION_DRY_RUN_EXECUTION_REQUEST_BOUNDARY_SCHEMA_VERSION
    tenant_id: str
    module_id: str = TICKETS_INCIDENTS_MODULE_ID
    endpoint: str = TICKETS_INCIDENTS_ACTIVATION_DRY_RUN_EXECUTION_REQUEST_BOUNDARY_ENDPOINT
    result_contract: str = TICKETS_INCIDENTS_ACTIVATION_DRY_RUN_EXECUTION_REQUEST_BOUNDARY_RESULT_CONTRACT
    ...
    activation_dry_run_execution_request_boundary_statement_hash: str
    activation_dry_run_execution_request_boundary_ref: str
    change_request_ref: str
    audit_chain_ref: str
    prepared_by: str
    ...
    activation_dry_run_execution_request_boundary_requested: bool
    activation_dry_run_execution_request_boundary_ready: bool
    future_activation_dry_run_executor_runtime_boundary_required: bool = True
    ...
    activation_dry_run_execution_request_boundary_steps: tuple[str, ...]
    required_activation_dry_run_execution_request_boundary_evidence: tuple[str, ...]
    blocking_reasons: tuple[str, ...]
    summary: TicketsIncidentsActivationDryRunExecutionRequestBoundarySummary
    ...
    def require_non_empty_text(cls, value: str) -> str:
        if not value.strip():
            raise ValueError(
                "Tickets & Incidents activation dry-run execution request boundary text fields must not be empty"
            )
        return value
    ...
    @field_validator(
        ...
        "activation_dry_run_execution_request_boundary_statement_hash",
        "evidence_hash",
    )
    @classmethod
    ...
    @field_validator(
        "activation_dry_run_execution_request_boundary_steps",
        "required_activation_dry_run_execution_request_boundary_evidence",
        "blocking_reasons",
        "evidence_refs",
    )
    @classmethod
    def require_unique_lists(cls, value: tuple[str, ...]) -> tuple[str, ...]:
        if len(set(value)) != len(value):
            raise ValueError(
                "Tickets & Incidents activation dry-run execution request boundary lists must not contain duplicates"
            )
        for item in value:
            if not item.strip():
                raise ValueError(
                    "Tickets & Incidents activation dry-run execution request boundary list items must not be empty"
                )
        return value

    @model_validator(mode="after")
    def require_non_executing_execution_request_boundary(
        self,
    ) -> TicketsIncidentsActivationDryRunExecutionRequestBoundaryResponse:
        if self.schema_version != TICKETS_INCIDENTS_ACTIVATION_DRY_RUN_EXECUTION_REQUEST_BOUNDARY_SCHEMA_VERSION:
            raise ValueError(
                "Tickets & Incidents activation dry-run execution request boundary schema version is invalid"
            )
        if self.endpoint != TICKETS_INCIDENTS_ACTIVATION_DRY_RUN_EXECUTION_REQUEST_BOUNDARY_ENDPOINT:
            raise ValueError("Tickets & Incidents activation dry-run execution request boundary endpoint is invalid")
        if self.result_contract != TICKETS_INCIDENTS_ACTIVATION_DRY_RUN_EXECUTION_REQUEST_BOUNDARY_RESULT_CONTRACT:
            raise ValueError(
                "Tickets & Incidents activation dry-run execution request boundary result contract is invalid"
            )
        if self.module_id != TICKETS_INCIDENTS_MODULE_ID:
            raise ValueError(
                "Tickets & Incidents activation dry-run execution request boundary only applies to tickets_incidents"
            )
        if self.continuity_domain != TICKETS_INCIDENTS_CONTINUITY_DOMAIN:
            raise ValueError(
                "Tickets & Incidents activation dry-run execution request boundary continuity domain is invalid"
            )
        expected_ready = (
            self.approval_gate_ready
            and self.human_approval_ready
            and self.preparer_role_allowed
            and self.activation_dry_run_execution_request_boundary_requested
            and not self.blocking_reasons
        )
        if self.activation_dry_run_execution_request_boundary_ready != expected_ready:
            raise ValueError(
                "Tickets & Incidents activation dry-run execution request boundary readiness is inconsistent"
            )
        if not self.future_activation_dry_run_executor_runtime_boundary_required:
            raise ValueError(
                "Tickets & Incidents activation dry-run execution request boundary must require a future "
        ...
        if (
            ...
            raise ValueError(
                "Tickets & Incidents activation dry-run execution request boundary must remain metadata-only"
            )
        if self.summary.activation_dry_run_execution_request_boundary_step_count != len(
            self.activation_dry_run_execution_request_boundary_steps
        ):
            raise ValueError("Tickets & Incidents activation dry-run execution request boundary step count mismatch")
        if self.summary.required_activation_dry_run_execution_request_boundary_evidence_count != len(
            self.required_activation_dry_run_execution_request_boundary_evidence
        ):
            raise ValueError(
                "Tickets & Incidents activation dry-run execution request boundary evidence count mismatch"
            )
        if self.summary.blocking_reason_count != len(self.blocking_reasons):
            raise ValueError(
                "Tickets & Incidents activation dry-run execution request boundary blocking count mismatch"
            )
        return self


def build_tickets_incidents_activation_dry_run_execution_request_boundary_response(
    *,
    command: TicketsIncidentsActivationDryRunExecutionRequestBoundaryCommand,
    ...
    command_hash = build_tickets_incidents_activation_dry_run_execution_request_boundary_command_hash(command)
    idempotency_key_hash = stable_hash(
        canonical_json(
            {
                "schema_version": "tickets_incidents_activation_dry_run_execution_request_boundary_idempotency_key.v1",
                "tenant_id": user_context.tenant_id,
    ...
    statement_hash = stable_hash(command.activation_dry_run_execution_request_boundary_statement)
    preparer_role_allowed = bool({"tenant-admin", "tenant_admin"} & user_context.role_ids)
    blocking_reasons = _activation_dry_run_execution_request_boundary_blocking_reasons(
        command=command,
    ...
    gate_steps = (
        ...
        "verify_activation_dry_run_execution_gate_schema_bound",
        "define_activation_dry_run_execution_request_idempotency_fields",
        "define_activation_dry_run_execution_request_preconditions",
    ...
    required_evidence = (
        ...
        "tickets_restore_drill_evidence_hash",
        "exact_activation_dry_run_execution_request_boundary_statement_hash",
        "change_request_ref",
        "idempotency_key_hash",
        "audit_chain_ref",
        "future_activation_dry_run_executor_runtime_boundary_required",
        "activation_dry_run_execution_request_boundary_schema_version",
        "activation_dry_run_execution_request_idempotency_fields",
        "activation_dry_run_execution_request_precondition_fields",
        "future_activation_dry_run_executor_runtime_boundary",
    ...
    draft = TicketsIncidentsActivationDryRunExecutionRequestBoundaryResponse(
        tenant_id=user_context.tenant_id,
        approval_gate_ready=approval_gate.approval_gate_ready,
        ...
        activation_dry_run_execution_request_boundary_statement_hash=statement_hash,
        activation_dry_run_execution_request_boundary_ref=command.activation_dry_run_execution_request_boundary_ref,
        ...
        preparer_role_allowed=preparer_role_allowed,
        activation_dry_run_execution_request_boundary_requested=command.activation_dry_run_execution_request_boundary_requested,
        activation_dry_run_execution_request_boundary_ready=gate_ready,
        activation_dry_run_execution_request_boundary_steps=gate_steps,
        required_activation_dry_run_execution_request_boundary_evidence=required_evidence,
        blocking_reasons=blocking_reasons,
        summary=TicketsIncidentsActivationDryRunExecutionRequestBoundarySummary(
            activation_dry_run_execution_request_boundary_step_count=len(gate_steps),
            required_activation_dry_run_execution_request_boundary_evidence_count=len(required_evidence),
        ...
        next_action=(
            TICKETS_INCIDENTS_ACTIVATION_DRY_RUN_EXECUTION_REQUEST_BOUNDARY_READY_NEXT_ACTION
            if gate_ready
            else TICKETS_INCIDENTS_ACTIVATION_DRY_RUN_EXECUTION_REQUEST_BOUNDARY_RETRY_NEXT_ACTION
        ),
    )
    return draft.model_copy(
        update={"evidence_hash": build_tickets_incidents_activation_dry_run_execution_request_boundary_hash(draft)}
    )


def build_tickets_incidents_activation_dry_run_execution_request_boundary_command_hash(
    command: TicketsIncidentsActivationDryRunExecutionRequestBoundaryCommand,
) -> str:
    payload = command.model_dump(mode="json", exclude={"activation_dry_run_execution_request_boundary_statement"})
    payload["activation_dry_run_execution_request_boundary_statement_hash"] = stable_hash(
        command.activation_dry_run_execution_request_boundary_statement
    )
    return stable_hash(canonical_json(payload))


def build_tickets_incidents_activation_dry_run_execution_request_boundary_hash(
    response: TicketsIncidentsActivationDryRunExecutionRequestBoundaryResponse,
) -> str:
    return stable_hash(canonical_json(response.model_dump(mode="json", exclude={"evidence_hash"})))


def _activation_dry_run_execution_request_boundary_blocking_reasons(
    *,
    command: TicketsIncidentsActivationDryRunExecutionRequestBoundaryCommand,
    approval_gate_ready: bool,
    ...
    if not command.activation_dry_run_execution_request_boundary_requested:
        reasons.append("activation_dry_run_execution_request_boundary_not_requested")
