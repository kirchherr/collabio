...
TICKETS_INCIDENTS_ACTIVATION_EXECUTOR_SKELETON_SCHEMA_VERSION = "tickets_incidents_activation_executor_skeleton.v1"
...
TICKETS_INCIDENTS_ACTIVATION_EXECUTOR_SKELETON_PREPARATION_STATEMENT = (
    "I prepare the Tickets & Incidents activation executor skeleton without executing activation."
)
TICKETS_INCIDENTS_ACTIVATION_EXECUTOR_SKELETON_READY_NEXT_ACTION = (
    "prepare_tickets_incidents_activation_dry_run_plan_without_tenant_activation"
)
TICKETS_INCIDENTS_ACTIVATION_EXECUTOR_SKELETON_RETRY_NEXT_ACTION = (
...
class TicketsIncidentsActivationExecutorSkeletonCommand(BaseModel):
    ...
    activation_executor_skeleton_ref: str
    change_request_ref: str
    idempotency_key_ref: str
    prepared_at_utc: datetime
    audit_chain_ref: str
    activation_executor_skeleton_preparation_statement: str
    activation_executor_skeleton_requested: bool = True
    ...
    def require_sha256(cls, value: str) -> str:
        if not SHA256_PATTERN.fullmatch(value):
            raise ValueError("Tickets & Incidents activation executor skeleton hashes must be sha256 references")
        return value
    ...
    def require_ref(cls, value: str) -> str:
        if not REF_PATTERN.fullmatch(value.strip()):
            raise ValueError("Tickets & Incidents activation executor skeleton references need typed ref prefixes")
        return value.strip()

    @field_validator("activation_executor_skeleton_preparation_statement")
    @classmethod
    def require_exact_preparation_statement(cls, value: str) -> str:
        normalized = value.strip()
        if normalized != TICKETS_INCIDENTS_ACTIVATION_EXECUTOR_SKELETON_PREPARATION_STATEMENT:
            raise ValueError("Tickets & Incidents activation executor skeleton requires the exact statement")
        return normalized
    ...
    def require_timezone_aware_timestamp(cls, value: datetime) -> datetime:
        if value.tzinfo is None or value.utcoffset() is None:
            raise ValueError("Tickets & Incidents activation executor skeleton prepared_at_utc needs a timezone")
        return value


class TicketsIncidentsActivationExecutorSkeletonSummary(BaseModel):
    model_config = ConfigDict(extra="forbid")

    skeleton_step_count: int
    required_executor_skeleton_evidence_count: int
    blocking_reason_count: int


class TicketsIncidentsActivationExecutorSkeletonResponse(BaseModel):
    model_config = ConfigDict(extra="forbid")

    schema_version: str = TICKETS_INCIDENTS_ACTIVATION_EXECUTOR_SKELETON_SCHEMA_VERSION
    tenant_id: str
    module_id: str = TICKETS_INCIDENTS_MODULE_ID
    endpoint: str = TICKETS_INCIDENTS_ACTIVATION_EXECUTOR_SKELETON_ENDPOINT
    result_contract: str = TICKETS_INCIDENTS_ACTIVATION_EXECUTOR_SKELETON_RESULT_CONTRACT
    ...
    activation_executor_skeleton_preparation_statement_hash: str
    activation_executor_skeleton_ref: str
    change_request_ref: str
    audit_chain_ref: str
    prepared_by: str
    ...
    activation_executor_skeleton_requested: bool
    activation_executor_skeleton_prepared: bool
    activation_executor_implementation_required: bool = True
    activation_dry_run_required: bool = True
    ...
    skeleton_steps: tuple[str, ...]
    required_executor_skeleton_evidence: tuple[str, ...]
    blocking_reasons: tuple[str, ...]
    summary: TicketsIncidentsActivationExecutorSkeletonSummary
    ...
    def require_non_empty_text(cls, value: str) -> str:
        if not value.strip():
            raise ValueError("Tickets & Incidents activation executor skeleton text fields must not be empty")
        return value
    ...
    @field_validator(
        ...
        "activation_executor_skeleton_preparation_statement_hash",
        "evidence_hash",
    )
    @classmethod
    ...
    @field_validator("skeleton_steps", "required_executor_skeleton_evidence", "blocking_reasons", "evidence_refs")
    @classmethod
    def require_unique_lists(cls, value: tuple[str, ...]) -> tuple[str, ...]:
        if len(set(value)) != len(value):
            raise ValueError("Tickets & Incidents activation executor skeleton lists must not contain duplicates")
        for item in value:
            if not item.strip():
                raise ValueError("Tickets & Incidents activation executor skeleton list items must not be empty")
        return value

    @model_validator(mode="after")
    def require_non_executing_skeleton_contract(self) -> TicketsIncidentsActivationExecutorSkeletonResponse:
        if self.schema_version != TICKETS_INCIDENTS_ACTIVATION_EXECUTOR_SKELETON_SCHEMA_VERSION:
            raise ValueError("Tickets & Incidents activation executor skeleton schema version is invalid")
        if self.endpoint != TICKETS_INCIDENTS_ACTIVATION_EXECUTOR_SKELETON_ENDPOINT:
            raise ValueError("Tickets & Incidents activation executor skeleton endpoint is invalid")
        if self.result_contract != TICKETS_INCIDENTS_ACTIVATION_EXECUTOR_SKELETON_RESULT_CONTRACT:
            raise ValueError("Tickets & Incidents activation executor skeleton result contract is invalid")
        if self.module_id != TICKETS_INCIDENTS_MODULE_ID:
            raise ValueError("Tickets & Incidents activation executor skeleton only applies to tickets_incidents")
        if self.continuity_domain != TICKETS_INCIDENTS_CONTINUITY_DOMAIN:
            raise ValueError("Tickets & Incidents activation executor skeleton continuity domain is invalid")
        expected_prepared = (
            self.approval_gate_ready
            and self.human_approval_ready
            and self.preparer_role_allowed
            and self.activation_executor_skeleton_requested
            and not self.blocking_reasons
        )
        if self.activation_executor_skeleton_prepared != expected_prepared:
            raise ValueError("Tickets & Incidents activation executor skeleton preparation is inconsistent")
        if not self.activation_executor_implementation_required:
            raise ValueError("Tickets & Incidents activation executor skeleton must require a future executor")
        if not self.activation_dry_run_required:
            raise ValueError("Tickets & Incidents activation executor skeleton must require a future dry run")
        if (
            ...
            raise ValueError("Tickets & Incidents activation executor skeleton must remain metadata-only")
        if self.summary.skeleton_step_count != len(self.skeleton_steps):
            raise ValueError("Tickets & Incidents activation executor skeleton step count mismatch")
        if self.summary.required_executor_skeleton_evidence_count != len(self.required_executor_skeleton_evidence):
            raise ValueError("Tickets & Incidents activation executor skeleton evidence count mismatch")
        if self.summary.blocking_reason_count != len(self.blocking_reasons):
            raise ValueError("Tickets & Incidents activation executor skeleton blocking count mismatch")
        return self


def build_tickets_incidents_activation_executor_skeleton_response(
    *,
    command: TicketsIncidentsActivationExecutorSkeletonCommand,
...
) -> TicketsIncidentsActivationExecutorSkeletonResponse:
    ...
    command_hash = build_tickets_incidents_activation_executor_skeleton_command_hash(command)
    idempotency_key_hash = stable_hash(
        canonical_json(
            {
                "schema_version": "tickets_incidents_activation_executor_skeleton_idempotency_key.v1",
                "tenant_id": user_context.tenant_id,
    ...
    preparation_statement_hash = stable_hash(command.activation_executor_skeleton_preparation_statement)
    preparer_role_allowed = bool({"tenant-admin", "tenant_admin"} & user_context.role_ids)
    blocking_reasons = _activation_executor_skeleton_blocking_reasons(
        command=command,
    ...
    skeleton_steps = (
        ...
        "bind_activation_execution_boundary_hash",
        "plan_metadata_schema_presence_checks",
        "plan_tenant_module_state_creation_as_future_dry_run_only",
        "defer_tickets_business_api_activation",
        "defer_ticket_worker_activation",
        "defer_ticket_content_payload_creation",
        "emit_audit_hashes_without_confirmation_or_review_text",
    )
    required_executor_skeleton_evidence = (
        "tenant_admin_role",
        "activation_approval_gate_hash",
        "tenant_admin_activation_approval_record_hash",
        "activation_execution_boundary_hash",
        "tickets_restore_drill_evidence_hash",
        "exact_activation_executor_skeleton_preparation_statement_hash",
        "change_request_ref",
        "idempotency_key_hash",
        "audit_chain_ref",
        "future_activation_dry_run_required",
    ...
    draft = TicketsIncidentsActivationExecutorSkeletonResponse(
        tenant_id=user_context.tenant_id,
        approval_gate_ready=approval_gate.approval_gate_ready,
        ...
        activation_executor_skeleton_preparation_statement_hash=preparation_statement_hash,
        activation_executor_skeleton_ref=command.activation_executor_skeleton_ref,
        ...
        preparer_role_allowed=preparer_role_allowed,
        activation_executor_skeleton_requested=command.activation_executor_skeleton_requested,
        activation_executor_skeleton_prepared=skeleton_prepared,
        skeleton_steps=skeleton_steps,
        required_executor_skeleton_evidence=required_executor_skeleton_evidence,
        blocking_reasons=blocking_reasons,
        summary=TicketsIncidentsActivationExecutorSkeletonSummary(
            skeleton_step_count=len(skeleton_steps),
            required_executor_skeleton_evidence_count=len(required_executor_skeleton_evidence),
        ...
        next_action=(
            ...
            else TICKETS_INCIDENTS_ACTIVATION_EXECUTOR_SKELETON_RETRY_NEXT_ACTION
        ),
    )
    return draft.model_copy(update={"evidence_hash": build_tickets_incidents_activation_executor_skeleton_hash(draft)})


def build_tickets_incidents_activation_executor_skeleton_command_hash(
    command: TicketsIncidentsActivationExecutorSkeletonCommand,
) -> str:
    payload = command.model_dump(mode="json", exclude={"activation_executor_skeleton_preparation_statement"})
    payload["activation_executor_skeleton_preparation_statement_hash"] = stable_hash(
        command.activation_executor_skeleton_preparation_statement
    )
    return stable_hash(canonical_json(payload))


def build_tickets_incidents_activation_executor_skeleton_hash(
    response: TicketsIncidentsActivationExecutorSkeletonResponse,
) -> str:
    return stable_hash(canonical_json(response.model_dump(mode="json", exclude={"evidence_hash"})))


def _activation_executor_skeleton_blocking_reasons(
    *,
    command: TicketsIncidentsActivationExecutorSkeletonCommand,
    approval_gate_ready: bool,
    ...
    if not command.activation_executor_skeleton_requested:
        reasons.append("activation_executor_skeleton_not_requested")
