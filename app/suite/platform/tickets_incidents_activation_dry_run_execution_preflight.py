...
TICKETS_INCIDENTS_ACTIVATION_DRY_RUN_EXECUTION_PREFLIGHT_SCHEMA_VERSION = (
    "tickets_incidents_activation_dry_run_execution_preflight.v1"
)
TICKETS_INCIDENTS_ACTIVATION_DRY_RUN_EXECUTION_PREFLIGHT_RESULT_CONTRACT = (
    "metadata_only_tickets_incidents_activation_dry_run_execution_preflight_no_execution"
)
TICKETS_INCIDENTS_ACTIVATION_DRY_RUN_EXECUTION_PREFLIGHT_ENDPOINT = (
...
TICKETS_INCIDENTS_ACTIVATION_DRY_RUN_EXECUTION_PREFLIGHT_STATEMENT = (
    "I prepare the Tickets & Incidents activation dry-run execution preflight without executing "
    "dry-run or tenant activation."
)
TICKETS_INCIDENTS_ACTIVATION_DRY_RUN_EXECUTION_PREFLIGHT_READY_NEXT_ACTION = (
    "prepare_tickets_incidents_activation_dry_run_execution_receipt_boundary_without_execution"
)
TICKETS_INCIDENTS_ACTIVATION_DRY_RUN_EXECUTION_PREFLIGHT_RETRY_NEXT_ACTION = (
...
class TicketsIncidentsActivationDryRunExecutionPreflightCommand(BaseModel):
    ...
    activation_dry_run_execution_preflight_ref: str
    change_request_ref: str
    idempotency_key_ref: str
    prepared_at_utc: datetime
    audit_chain_ref: str
    activation_dry_run_execution_preflight_statement: str
    activation_dry_run_execution_preflight_requested: bool = True
    ...
    def require_sha256(cls, value: str) -> str:
        if not SHA256_PATTERN.fullmatch(value):
            raise ValueError(
                "Tickets & Incidents activation dry-run execution preflight hashes must be sha256 references"
            )
        return value
    ...
    def require_ref(cls, value: str) -> str:
        if not REF_PATTERN.fullmatch(value.strip()):
            raise ValueError(
                "Tickets & Incidents activation dry-run execution preflight references need typed ref prefixes"
            )
        return value.strip()

    @field_validator("activation_dry_run_execution_preflight_statement")
    @classmethod
    def require_exact_statement(cls, value: str) -> str:
        normalized = value.strip()
        if normalized != TICKETS_INCIDENTS_ACTIVATION_DRY_RUN_EXECUTION_PREFLIGHT_STATEMENT:
            raise ValueError("Tickets & Incidents activation dry-run execution preflight requires the exact statement")
        return normalized
    ...
    def require_timezone_aware_timestamp(cls, value: datetime) -> datetime:
        if value.tzinfo is None or value.utcoffset() is None:
            raise ValueError(
                "Tickets & Incidents activation dry-run execution preflight prepared_at_utc needs a timezone"
            )
        return value


class TicketsIncidentsActivationDryRunExecutionPreflightSummary(BaseModel):
    model_config = ConfigDict(extra="forbid")

    activation_dry_run_execution_preflight_step_count: int
    required_activation_dry_run_execution_preflight_evidence_count: int
    blocking_reason_count: int


class TicketsIncidentsActivationDryRunExecutionPreflightResponse(BaseModel):
    model_config = ConfigDict(extra="forbid")

    schema_version: str = TICKETS_INCIDENTS_ACTIVATION_DRY_RUN_EXECUTION_PREFLIGHT_SCHEMA_VERSION
    tenant_id: str
    module_id: str = TICKETS_INCIDENTS_MODULE_ID
    endpoint: str = TICKETS_INCIDENTS_ACTIVATION_DRY_RUN_EXECUTION_PREFLIGHT_ENDPOINT
    result_contract: str = TICKETS_INCIDENTS_ACTIVATION_DRY_RUN_EXECUTION_PREFLIGHT_RESULT_CONTRACT
    ...
    activation_dry_run_execution_preflight_statement_hash: str
    activation_dry_run_execution_preflight_ref: str
    change_request_ref: str
    audit_chain_ref: str
    prepared_by: str
    ...
    activation_dry_run_execution_preflight_requested: bool
    activation_dry_run_execution_preflight_ready: bool
    future_activation_dry_run_execution_receipt_boundary_required: bool = True
    ...
    activation_dry_run_execution_preflight_steps: tuple[str, ...]
    required_activation_dry_run_execution_preflight_evidence: tuple[str, ...]
    blocking_reasons: tuple[str, ...]
    summary: TicketsIncidentsActivationDryRunExecutionPreflightSummary
    ...
    def require_non_empty_text(cls, value: str) -> str:
        if not value.strip():
            raise ValueError("Tickets & Incidents activation dry-run execution preflight text fields must not be empty")
        return value
    ...
    @field_validator(
        ...
        "activation_dry_run_execution_preflight_statement_hash",
        "evidence_hash",
    )
    @classmethod
    ...
    @field_validator(
        "activation_dry_run_execution_preflight_steps",
        "required_activation_dry_run_execution_preflight_evidence",
        "blocking_reasons",
        "evidence_refs",
    )
    @classmethod
    def require_unique_lists(cls, value: tuple[str, ...]) -> tuple[str, ...]:
        if len(set(value)) != len(value):
            raise ValueError(
                "Tickets & Incidents activation dry-run execution preflight lists must not contain duplicates"
            )
        for item in value:
            if not item.strip():
                raise ValueError(
                    "Tickets & Incidents activation dry-run execution preflight list items must not be empty"
                )
        return value
    ...
    def require_non_executing_execution_request_boundary(
        self,
    ) -> TicketsIncidentsActivationDryRunExecutionPreflightResponse:
        if self.schema_version != TICKETS_INCIDENTS_ACTIVATION_DRY_RUN_EXECUTION_PREFLIGHT_SCHEMA_VERSION:
            raise ValueError("Tickets & Incidents activation dry-run execution preflight schema version is invalid")
        if self.endpoint != TICKETS_INCIDENTS_ACTIVATION_DRY_RUN_EXECUTION_PREFLIGHT_ENDPOINT:
            raise ValueError("Tickets & Incidents activation dry-run execution preflight endpoint is invalid")
        if self.result_contract != TICKETS_INCIDENTS_ACTIVATION_DRY_RUN_EXECUTION_PREFLIGHT_RESULT_CONTRACT:
            raise ValueError("Tickets & Incidents activation dry-run execution preflight result contract is invalid")
        if self.module_id != TICKETS_INCIDENTS_MODULE_ID:
            raise ValueError(
                "Tickets & Incidents activation dry-run execution preflight only applies to tickets_incidents"
            )
        if self.continuity_domain != TICKETS_INCIDENTS_CONTINUITY_DOMAIN:
            raise ValueError("Tickets & Incidents activation dry-run execution preflight continuity domain is invalid")
        expected_ready = (
            self.approval_gate_ready
            and self.human_approval_ready
            and self.preparer_role_allowed
            and self.activation_dry_run_execution_preflight_requested
            and not self.blocking_reasons
        )
        if self.activation_dry_run_execution_preflight_ready != expected_ready:
            raise ValueError("Tickets & Incidents activation dry-run execution preflight readiness is inconsistent")
        if not self.future_activation_dry_run_execution_receipt_boundary_required:
            raise ValueError(
                "Tickets & Incidents activation dry-run execution preflight must require a future "
        ...
        if (
            ...
            raise ValueError("Tickets & Incidents activation dry-run execution preflight must remain metadata-only")
        if self.summary.activation_dry_run_execution_preflight_step_count != len(
            self.activation_dry_run_execution_preflight_steps
        ):
            raise ValueError("Tickets & Incidents activation dry-run execution preflight step count mismatch")
        if self.summary.required_activation_dry_run_execution_preflight_evidence_count != len(
            self.required_activation_dry_run_execution_preflight_evidence
        ):
            raise ValueError("Tickets & Incidents activation dry-run execution preflight evidence count mismatch")
        if self.summary.blocking_reason_count != len(self.blocking_reasons):
            raise ValueError("Tickets & Incidents activation dry-run execution preflight blocking count mismatch")
        return self


def build_tickets_incidents_activation_dry_run_execution_preflight_response(
    *,
    command: TicketsIncidentsActivationDryRunExecutionPreflightCommand,
    ...
    command_hash = build_tickets_incidents_activation_dry_run_execution_preflight_command_hash(command)
    idempotency_key_hash = stable_hash(
        canonical_json(
            {
                "schema_version": "tickets_incidents_activation_dry_run_execution_preflight_idempotency_key.v1",
                "tenant_id": user_context.tenant_id,
    ...
    statement_hash = stable_hash(command.activation_dry_run_execution_preflight_statement)
    preparer_role_allowed = bool({"tenant-admin", "tenant_admin"} & user_context.role_ids)
    blocking_reasons = _activation_dry_run_execution_preflight_blocking_reasons(
        command=command,
    ...
    gate_steps = (
        ...
        "define_dry_run_execution_preflight_invariants",
        "define_dry_run_input_admission_fields",
        "define_no_worker_or_business_api_flags",
    ...
    required_evidence = (
        ...
        "tickets_restore_drill_evidence_hash",
        "exact_activation_dry_run_execution_preflight_statement_hash",
        "change_request_ref",
        "idempotency_key_hash",
        "audit_chain_ref",
        "activation_dry_run_execution_preflight_schema_version",
        "activation_dry_run_executor_runtime_boundary_schema_version",
        "dry_run_execution_preflight_invariant_fields",
        "dry_run_input_admission_fields",
    ...
    draft = TicketsIncidentsActivationDryRunExecutionPreflightResponse(
        tenant_id=user_context.tenant_id,
        approval_gate_ready=approval_gate.approval_gate_ready,
        ...
        activation_dry_run_execution_preflight_statement_hash=statement_hash,
        activation_dry_run_execution_preflight_ref=command.activation_dry_run_execution_preflight_ref,
        ...
        preparer_role_allowed=preparer_role_allowed,
        activation_dry_run_execution_preflight_requested=command.activation_dry_run_execution_preflight_requested,
        activation_dry_run_execution_preflight_ready=gate_ready,
        activation_dry_run_execution_preflight_steps=gate_steps,
        required_activation_dry_run_execution_preflight_evidence=required_evidence,
        blocking_reasons=blocking_reasons,
        summary=TicketsIncidentsActivationDryRunExecutionPreflightSummary(
            activation_dry_run_execution_preflight_step_count=len(gate_steps),
            required_activation_dry_run_execution_preflight_evidence_count=len(required_evidence),
        ...
        next_action=(
            TICKETS_INCIDENTS_ACTIVATION_DRY_RUN_EXECUTION_PREFLIGHT_READY_NEXT_ACTION
            if gate_ready
            else TICKETS_INCIDENTS_ACTIVATION_DRY_RUN_EXECUTION_PREFLIGHT_RETRY_NEXT_ACTION
        ),
    )
    return draft.model_copy(
        update={"evidence_hash": build_tickets_incidents_activation_dry_run_execution_preflight_hash(draft)}
    )


def build_tickets_incidents_activation_dry_run_execution_preflight_command_hash(
    command: TicketsIncidentsActivationDryRunExecutionPreflightCommand,
) -> str:
    payload = command.model_dump(mode="json", exclude={"activation_dry_run_execution_preflight_statement"})
    payload["activation_dry_run_execution_preflight_statement_hash"] = stable_hash(
        command.activation_dry_run_execution_preflight_statement
    )
    return stable_hash(canonical_json(payload))


def build_tickets_incidents_activation_dry_run_execution_preflight_hash(
    response: TicketsIncidentsActivationDryRunExecutionPreflightResponse,
) -> str:
    return stable_hash(canonical_json(response.model_dump(mode="json", exclude={"evidence_hash"})))


def _activation_dry_run_execution_preflight_blocking_reasons(
    *,
    command: TicketsIncidentsActivationDryRunExecutionPreflightCommand,
    approval_gate_ready: bool,
    ...
    if not command.activation_dry_run_execution_preflight_requested:
        reasons.append("activation_dry_run_execution_preflight_not_requested")
