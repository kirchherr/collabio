...
from suite.platform.office_document_repository import (
    InMemoryOfficeDocumentRepository, PgOfficeDocumentRepository, _model_values, _prepare_version,
)
from suite.platform.office_documents import (
    ...
    OfficeDocumentPermissionError, OfficeDocumentRecord, OfficeDocumentSaveCommand, OfficeDocumentService,
    OfficeDocumentVersion, office_document_command_hash,
)
from suite.platform.office_reviews import derive_review_quote
from suite.platform.office_suggestions import (
    MAX_SUGGESTION_BYTES, SUGGESTION_MIME, SUGGESTION_SCHEMA, SUGGESTION_SOURCE_SYSTEM,
    SuggestionCreateCommand, SuggestionDecisionCommand, SuggestionDecisionRecord, SuggestionEvidence,
    SuggestionListSnapshot, SuggestionSnapshot, TextSuggestionRecord, read_suggestion_payload,
    replace_suggestion_text, suggestion_command_hash,
)
...
def _prepare_evidence(
    user: UserContext, document: OfficeDocumentRecord, suggestion_id: str, source_version_id: str,
    payload: dict[str, Any], command_hash: str, mutation_reference: str, acl_rows: list[tuple[Any, ...]],
    ...
    metadata = SourceObjectMetadata(
        tenant_id=user.tenant_id, object_id=suggestion_id, version_id=source_version_id,
        object_type=SourceObjectType.COMMENT, title="Office text suggestion",
        owner_principal_id=document.owner_principal_id, created_by=user.user_id,
        created_at_utc=now, updated_at_utc=now, classification=DataClass.INTERNAL,
        ...
        audit_chain_ref=audit_ref, source_system=SUGGESTION_SOURCE_SYSTEM, schema_version=SUGGESTION_SCHEMA,
        mime_type=SUGGESTION_MIME, lifecycle_state=SourceLifecycleState.SAVED_VERSION,
        ...
        parent_object_id=document.object_id, thread_id=suggestion_id,
    )
    ...
    receipt = build_source_object_write_receipt(
        record=source, receipt_reference=f"office-suggestion-write:{uuid4().hex}",
        audit_chain_ref=audit_ref, captured_at_utc=now,
    )
    evidence = SuggestionEvidence(
        tenant_id=user.tenant_id, object_id=document.object_id, suggestion_id=suggestion_id,
        source_version_id=source_version_id, created_by=user.user_id, created_at_utc=now,
        ...
        mutation_reference=mutation_reference, command_hash=command_hash, audit_chain_ref=audit_ref,
    )
...
class OfficeSuggestionRepositoryAdapter:
    ...
    def _session(self, user: UserContext, *, write: bool = False) -> Iterator[Connection]:
        if isinstance(self.documents, PgOfficeDocumentRepository):
            ...
            with self.documents._lock:
                stores = [
                    (self.documents, "documents"), (self.documents, "saved_versions"),
    ...
    def _document(self, connection: Connection, user: UserContext, object_id: str, *, write: bool = False) -> OfficeDocumentRecord:
        if isinstance(self.documents, PgOfficeDocumentRepository):
    ...
    def _version(self, connection: Connection, user: UserContext, object_id: str, version_id: str) -> OfficeDocumentVersion:
        if isinstance(self.documents, PgOfficeDocumentRepository):
            assert connection is not None
            return self.documents._version(connection, tenant_id=user.tenant_id, object_id=object_id, version_id=version_id)
        return self.documents.get_version(user_context=user, object_id=object_id, version_id=version_id)

    def _rows(
        self, connection: Connection, table: str, user: UserContext, *, after: str | None = None,
        limit: int = 1, **filters: str,
    ) -> list[dict[str, Any]]:
        if table not in TABLE_MODELS:
            raise ValueError("Invalid suggestion table")
        filters = {"tenant_id": user.tenant_id, **filters}
        if connection is None:
            rows = [row for row in self._memory[table].values() if all(row[key] == value for key, value in filters.items())]
            return sorted(
                (row for row in rows if after is None or row["suggestion_id"] > after), key=lambda row: row["suggestion_id"]
            )[:limit]
    ...
    def _suggestion(self, connection: Connection, user: UserContext, object_id: str, suggestion_id: str) -> TextSuggestionRecord:
    ...
    def _decision(self, connection: Connection, user: UserContext, suggestion_id: str) -> SuggestionDecisionRecord | None:
    ...
    def list_suggestions(
        ...
        with self._session(user) as connection:
            ...
            return SuggestionListSnapshot(
                document=document, can_write=writable, has_more=len(rows) > limit,
                entries=tuple(SuggestionSnapshot(
                    document=document, suggestion=TextSuggestionRecord.model_validate(row),
                    decision=self._decision(connection, user, row["suggestion_id"]), can_write=writable,
    ...
    def detail(self, *, user: UserContext, object_id: str, suggestion_id: str) -> SuggestionSnapshot:
        with self._session(user) as connection:
            ...
            return SuggestionSnapshot(
                document=document, suggestion=self._suggestion(connection, user, object_id, suggestion_id),
                decision=self._decision(connection, user, suggestion_id),
                can_write=self._can_write(connection, user, object_id),
            )

    def _store(
        self, connection: Connection, table: str, evidence: SuggestionEvidence,
        source: SourceObjectRecord, receipt: SourceObjectWriteReceipt,
    ) -> None:
        if isinstance(self.documents, PgOfficeDocumentRepository):
            assert connection is not None
    ...
    def _acl_rows(self, connection: Connection, user: UserContext, object_id: str) -> list[tuple[Any, ...]]:
        ...
        return sorted(("user", actor, permission, 1) for (tenant, doc, actor), permission in self.documents.grants.items()
                      if tenant == user.tenant_id and doc == object_id)

    def _replay(
        self, connection: Connection, user: UserContext, document: OfficeDocumentRecord,
        suggestion: TextSuggestionRecord, decision: SuggestionDecisionRecord | None,
    ) -> SuggestionSnapshot:
        payload = read_suggestion_payload(self.documents.source_repository, document, suggestion, decision or suggestion)
        version = self._version(connection, user, document.object_id, decision.result_version_id) if decision and decision.result_version_id else None
        content = self.document_service._read_content(document, version) if version else None
        return SuggestionSnapshot(
            document=document, suggestion=suggestion, decision=decision, can_write=True,
            payload=payload, result_version=version, result_content=content, replayed=True,
        )
    ...
    def commit(
        ...
        with self._session(user, write=True) as connection:
            ...
            for table in TABLE_MODELS:
                rows = self._rows(connection, table, user, created_by=user.user_id, mutation_reference=command.mutation_reference)
                if rows:
                    ...
                    decision = SuggestionDecisionRecord.model_validate(rows[0]) if table == "text_suggestion_decisions" else None
            ...
            if isinstance(command, SuggestionCreateCommand):
                if document.current_version_id != command.anchor_version_id or document.current_version_id != command.expected_current_version_id:
                ...
                payload = {
                    "schema_version": SUGGESTION_SCHEMA, "suggestion_id": new_id, "document_id": object_id,
                    "anchor_version_id": version.version_id, "anchor_content_hash": version.content_hash,
                    "anchor": command.anchor.model_dump(by_alias=True), "quote": quote,
                    "replacement_text": command.replacement_text, "operation": "create",
                    "result_version_id": None, "result_content_hash": None,
                }
                evidence, source, receipt = _prepare_evidence(
                    user, document, new_id, f"office-suggestion-event-{uuid4().hex}", payload,
                    command_hash, command.mutation_reference, acl_rows,
                )
                suggestion = TextSuggestionRecord(
                    **evidence.model_dump(), anchor_version_id=version.version_id, anchor_content_hash=version.content_hash,
                    anchor_from=command.anchor.from_, anchor_to=command.anchor.to,
                )
            ...
            if command.operation == "accept":
                if document.current_version_id != suggestion.anchor_version_id or document.current_version_id != command.expected_current_version_id:
                    raise OfficeDocumentConflictError("Document has a newer saved version")
                ...
                if previous.content_hash != suggestion.anchor_content_hash or derive_review_quote(original_content, suggestion.anchor()) != payload["quote"]:
                    raise OfficeDocumentInvalidContentError("Suggestion anchor is invalid")
                result_content = replace_suggestion_text(original_content, suggestion.anchor(), payload["replacement_text"])
                save = OfficeDocumentSaveCommand(
                    title=document.title, document=result_content, expected_current_version_id=document.current_version_id,
                    mutation_reference=f"office-suggestion-accept:{decision_id}", human_confirmation=True,
                )
                document, result_version, doc_source, doc_receipt = _prepare_version(
                    user=user, document=document, previous_version_id=document.current_version_id, command=save,
                    command_hash=office_document_command_hash(user_context=user, object_id=object_id, command=save),
                    acl_rows=acl_rows,
                )
                if isinstance(self.documents, PgOfficeDocumentRepository):
                    assert connection is not None
                    self.documents._persist_prepared_version(connection, document, result_version, doc_source, doc_receipt)
                else:
                    ...
                    self.documents.saved_versions[(user.tenant_id, object_id, result_version.version_id)] = result_version
            payload = {**payload, "operation": command.operation,
            ...
            evidence, source, receipt = _prepare_evidence(
                user, document, suggestion.suggestion_id, decision_id, payload,
                command_hash, command.mutation_reference, acl_rows,
            )
            decision = SuggestionDecisionRecord(
                **evidence.model_dump(), decision_id=decision_id, operation=command.operation,
                result_version_id=result_version.version_id if result_version else None,
                result_content_hash=result_version.content_hash if result_version else None,
            )
            self._store(connection, "text_suggestion_decisions", decision, source, receipt)
            return SuggestionSnapshot(
                document=document, suggestion=suggestion, decision=decision, can_write=True, payload=payload,
                result_version=result_version, result_content=result_content,
            )
