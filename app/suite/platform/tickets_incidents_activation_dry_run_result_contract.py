...
TICKETS_INCIDENTS_ACTIVATION_DRY_RUN_RESULT_CONTRACT_SCHEMA_VERSION = (
    "tickets_incidents_activation_dry_run_result_contract.v1"
)
TICKETS_INCIDENTS_ACTIVATION_DRY_RUN_RESULT_CONTRACT_RESULT_CONTRACT = (
    "metadata_only_tickets_incidents_activation_dry_run_result_contract_no_execution"
)
TICKETS_INCIDENTS_ACTIVATION_DRY_RUN_RESULT_CONTRACT_ENDPOINT = (
...
TICKETS_INCIDENTS_ACTIVATION_DRY_RUN_RESULT_CONTRACT_STATEMENT = (
    "I prepare the Tickets & Incidents activation dry-run result contract without executing "
    "dry-run or tenant activation."
)
TICKETS_INCIDENTS_ACTIVATION_DRY_RUN_RESULT_CONTRACT_READY_NEXT_ACTION = (
    "prepare_tickets_incidents_activation_dry_run_execution_gate_without_execution"
)
TICKETS_INCIDENTS_ACTIVATION_DRY_RUN_RESULT_CONTRACT_RETRY_NEXT_ACTION = (
...
class TicketsIncidentsActivationDryRunResultContractCommand(BaseModel):
    ...
    activation_dry_run_result_contract_ref: str
    change_request_ref: str
    idempotency_key_ref: str
    prepared_at_utc: datetime
    audit_chain_ref: str
    activation_dry_run_result_contract_statement: str
    activation_dry_run_result_contract_requested: bool = True
    ...
    def require_sha256(cls, value: str) -> str:
        if not SHA256_PATTERN.fullmatch(value):
            raise ValueError("Tickets & Incidents activation dry-run result contract hashes must be sha256 references")
        return value

    @field_validator(
        "activation_dry_run_result_contract_ref",
        "change_request_ref",
        "idempotency_key_ref",
        "audit_chain_ref",
    )
    ...
    def require_ref(cls, value: str) -> str:
        if not REF_PATTERN.fullmatch(value.strip()):
            raise ValueError(
                "Tickets & Incidents activation dry-run result contract references need typed ref prefixes"
            )
        return value.strip()

    @field_validator("activation_dry_run_result_contract_statement")
    @classmethod
    def require_exact_statement(cls, value: str) -> str:
        normalized = value.strip()
        if normalized != TICKETS_INCIDENTS_ACTIVATION_DRY_RUN_RESULT_CONTRACT_STATEMENT:
            raise ValueError("Tickets & Incidents activation dry-run result contract requires the exact statement")
        return normalized
    ...
    def require_timezone_aware_timestamp(cls, value: datetime) -> datetime:
        if value.tzinfo is None or value.utcoffset() is None:
            raise ValueError("Tickets & Incidents activation dry-run result contract prepared_at_utc needs a timezone")
        return value


class TicketsIncidentsActivationDryRunResultContractSummary(BaseModel):
    model_config = ConfigDict(extra="forbid")

    activation_dry_run_result_contract_step_count: int
    required_activation_dry_run_result_contract_evidence_count: int
    blocking_reason_count: int


class TicketsIncidentsActivationDryRunResultContractResponse(BaseModel):
    model_config = ConfigDict(extra="forbid")

    schema_version: str = TICKETS_INCIDENTS_ACTIVATION_DRY_RUN_RESULT_CONTRACT_SCHEMA_VERSION
    tenant_id: str
    module_id: str = TICKETS_INCIDENTS_MODULE_ID
    endpoint: str = TICKETS_INCIDENTS_ACTIVATION_DRY_RUN_RESULT_CONTRACT_ENDPOINT
    result_contract: str = TICKETS_INCIDENTS_ACTIVATION_DRY_RUN_RESULT_CONTRACT_RESULT_CONTRACT
    ...
    activation_dry_run_result_contract_statement_hash: str
    activation_dry_run_result_contract_ref: str
    change_request_ref: str
    audit_chain_ref: str
    prepared_by: str
    ...
    activation_dry_run_result_contract_requested: bool
    activation_dry_run_result_contract_ready: bool
    ...
    activation_dry_run_result_contract_steps: tuple[str, ...]
    required_activation_dry_run_result_contract_evidence: tuple[str, ...]
    blocking_reasons: tuple[str, ...]
    summary: TicketsIncidentsActivationDryRunResultContractSummary
    ...
    def require_non_empty_text(cls, value: str) -> str:
        if not value.strip():
            raise ValueError("Tickets & Incidents activation dry-run result contract text fields must not be empty")
        return value
    ...
    @field_validator(
        ...
        "activation_dry_run_result_contract_statement_hash",
        "evidence_hash",
    )
    @classmethod
    ...
    @field_validator(
        "activation_dry_run_result_contract_steps",
        "required_activation_dry_run_result_contract_evidence",
        "blocking_reasons",
        "evidence_refs",
    )
    @classmethod
    def require_unique_lists(cls, value: tuple[str, ...]) -> tuple[str, ...]:
        if len(set(value)) != len(value):
            raise ValueError("Tickets & Incidents activation dry-run result contract lists must not contain duplicates")
        for item in value:
            if not item.strip():
                raise ValueError("Tickets & Incidents activation dry-run result contract list items must not be empty")
        return value

    @model_validator(mode="after")
    def require_non_executing_result_contract(self) -> TicketsIncidentsActivationDryRunResultContractResponse:
        if self.schema_version != TICKETS_INCIDENTS_ACTIVATION_DRY_RUN_RESULT_CONTRACT_SCHEMA_VERSION:
            raise ValueError("Tickets & Incidents activation dry-run result contract schema version is invalid")
        if self.endpoint != TICKETS_INCIDENTS_ACTIVATION_DRY_RUN_RESULT_CONTRACT_ENDPOINT:
            raise ValueError("Tickets & Incidents activation dry-run result contract endpoint is invalid")
        if self.result_contract != TICKETS_INCIDENTS_ACTIVATION_DRY_RUN_RESULT_CONTRACT_RESULT_CONTRACT:
            raise ValueError("Tickets & Incidents activation dry-run result contract result contract is invalid")
        if self.module_id != TICKETS_INCIDENTS_MODULE_ID:
            raise ValueError("Tickets & Incidents activation dry-run result contract only applies to tickets_incidents")
        if self.continuity_domain != TICKETS_INCIDENTS_CONTINUITY_DOMAIN:
            raise ValueError("Tickets & Incidents activation dry-run result contract continuity domain is invalid")
        expected_ready = (
            self.approval_gate_ready
            and self.human_approval_ready
            and self.preparer_role_allowed
            and self.activation_dry_run_result_contract_requested
            and not self.blocking_reasons
        )
        if self.activation_dry_run_result_contract_ready != expected_ready:
            raise ValueError("Tickets & Incidents activation dry-run result contract readiness is inconsistent")
        if not self.future_dry_run_execution_gate_required:
            raise ValueError(
                "Tickets & Incidents activation dry-run result contract must require a future execution gate"
            )
        if (
            ...
            raise ValueError("Tickets & Incidents activation dry-run result contract must remain metadata-only")
        if self.summary.activation_dry_run_result_contract_step_count != len(
            self.activation_dry_run_result_contract_steps
        ):
            raise ValueError("Tickets & Incidents activation dry-run result contract step count mismatch")
        if self.summary.required_activation_dry_run_result_contract_evidence_count != len(
            self.required_activation_dry_run_result_contract_evidence
        ):
            raise ValueError("Tickets & Incidents activation dry-run result contract evidence count mismatch")
        if self.summary.blocking_reason_count != len(self.blocking_reasons):
            raise ValueError("Tickets & Incidents activation dry-run result contract blocking count mismatch")
        return self


def build_tickets_incidents_activation_dry_run_result_contract_response(
    *,
    command: TicketsIncidentsActivationDryRunResultContractCommand,
...
) -> TicketsIncidentsActivationDryRunResultContractResponse:
    ...
    command_hash = build_tickets_incidents_activation_dry_run_result_contract_command_hash(command)
    idempotency_key_hash = stable_hash(
        canonical_json(
            {
                "schema_version": "tickets_incidents_activation_dry_run_result_contract_idempotency_key.v1",
                "tenant_id": user_context.tenant_id,
    ...
    statement_hash = stable_hash(command.activation_dry_run_result_contract_statement)
    preparer_role_allowed = bool({"tenant-admin", "tenant_admin"} & user_context.role_ids)
    blocking_reasons = _activation_dry_run_result_contract_blocking_reasons(
        command=command,
    ...
    contract_ready = not blocking_reasons
    contract_steps = (
        ...
        "define_activation_dry_run_result_receipt_schema_version",
        "define_no_write_check_status_fields",
        "define_blocking_reason_and_warning_fields",
        "define_restore_and_approval_hash_fields",
        "define_no_persistence_worker_business_api_and_tenant_state_flags",
    ...
    required_evidence = (
        ...
        "tickets_restore_drill_evidence_hash",
        "exact_activation_dry_run_result_contract_statement_hash",
        "change_request_ref",
        "idempotency_key_hash",
        "audit_chain_ref",
        "future_activation_dry_run_execution_gate_required",
        "activation_dry_run_result_receipt_schema_version",
        "activation_dry_run_result_no_write_fields",
        "activation_dry_run_result_blocking_reason_fields",
        "activation_dry_run_result_restore_and_approval_hash_fields",
    ...
    draft = TicketsIncidentsActivationDryRunResultContractResponse(
        tenant_id=user_context.tenant_id,
        approval_gate_ready=approval_gate.approval_gate_ready,
        ...
        activation_dry_run_result_contract_statement_hash=statement_hash,
        activation_dry_run_result_contract_ref=command.activation_dry_run_result_contract_ref,
        ...
        preparer_role_allowed=preparer_role_allowed,
        activation_dry_run_result_contract_requested=command.activation_dry_run_result_contract_requested,
        activation_dry_run_result_contract_ready=contract_ready,
        activation_dry_run_result_contract_steps=contract_steps,
        required_activation_dry_run_result_contract_evidence=required_evidence,
        blocking_reasons=blocking_reasons,
        summary=TicketsIncidentsActivationDryRunResultContractSummary(
            activation_dry_run_result_contract_step_count=len(contract_steps),
            required_activation_dry_run_result_contract_evidence_count=len(required_evidence),
        ...
        next_action=(
            TICKETS_INCIDENTS_ACTIVATION_DRY_RUN_RESULT_CONTRACT_READY_NEXT_ACTION
            if contract_ready
            else TICKETS_INCIDENTS_ACTIVATION_DRY_RUN_RESULT_CONTRACT_RETRY_NEXT_ACTION
        ),
    )
    return draft.model_copy(
        update={"evidence_hash": build_tickets_incidents_activation_dry_run_result_contract_hash(draft)}
    )


def build_tickets_incidents_activation_dry_run_result_contract_command_hash(
    command: TicketsIncidentsActivationDryRunResultContractCommand,
) -> str:
    payload = command.model_dump(mode="json", exclude={"activation_dry_run_result_contract_statement"})
    payload["activation_dry_run_result_contract_statement_hash"] = stable_hash(
        command.activation_dry_run_result_contract_statement
    )
    return stable_hash(canonical_json(payload))


def build_tickets_incidents_activation_dry_run_result_contract_hash(
    response: TicketsIncidentsActivationDryRunResultContractResponse,
) -> str:
    return stable_hash(canonical_json(response.model_dump(mode="json", exclude={"evidence_hash"})))


def _activation_dry_run_result_contract_blocking_reasons(
    *,
    command: TicketsIncidentsActivationDryRunResultContractCommand,
    approval_gate_ready: bool,
    ...
    if not command.activation_dry_run_result_contract_requested:
        reasons.append("activation_dry_run_result_contract_not_requested")
