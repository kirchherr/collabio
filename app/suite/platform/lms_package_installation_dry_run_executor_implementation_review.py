...
LMS_PACKAGE_INSTALLATION_DRY_RUN_EXECUTOR_IMPLEMENTATION_REVIEW_SCHEMA_VERSION = (
    "lms_package_installation_dry_run_executor_implementation_review.v1"
)
...
LMS_PACKAGE_INSTALLATION_DRY_RUN_EXECUTOR_IMPLEMENTATION_REVIEW_STATEMENT = (
    "I review the LMS package installation dry-run executor implementation "
    "without executing dry-run or tenant activation."
)
LMS_PACKAGE_INSTALLATION_DRY_RUN_EXECUTOR_IMPLEMENTATION_REVIEW_READY_NEXT_ACTION = (
    "prepare_lms_package_installation_dry_run_result_contract_without_execution"
)
LMS_PACKAGE_INSTALLATION_DRY_RUN_EXECUTOR_IMPLEMENTATION_REVIEW_RETRY_NEXT_ACTION = (
...
class LmsPackageInstallationDryRunExecutorImplementationReviewCommand(BaseModel):
    ...
    dry_run_executor_implementation_review_ref: str
    change_request_ref: str
    idempotency_key_ref: str
    reviewed_at_utc: datetime
    audit_chain_ref: str
    dry_run_executor_implementation_review_statement: str
    dry_run_executor_implementation_review_requested: bool = True
    ...
    def require_sha256(cls, value: str) -> str:
        if not SHA256_PATTERN.fullmatch(value):
            raise ValueError("LMS dry-run executor implementation review hashes must be sha256 references")
        return value

    @field_validator(
        "dry_run_executor_implementation_review_ref", "change_request_ref", "idempotency_key_ref", "audit_chain_ref"
    )
    @classmethod
    def require_ref(cls, value: str) -> str:
        if not REF_PATTERN.fullmatch(value.strip()):
            raise ValueError("LMS dry-run executor implementation review references must use a typed ref prefix")
        return value.strip()

    @field_validator("dry_run_executor_implementation_review_statement")
    @classmethod
    def require_exact_review_statement(cls, value: str) -> str:
        normalized = value.strip()
        if normalized != LMS_PACKAGE_INSTALLATION_DRY_RUN_EXECUTOR_IMPLEMENTATION_REVIEW_STATEMENT:
            raise ValueError("LMS dry-run executor implementation review requires the exact review statement")
        return normalized
    ...
    def require_timezone_aware_timestamp(cls, value: datetime) -> datetime:
        if value.tzinfo is None or value.utcoffset() is None:
            raise ValueError("LMS dry-run executor implementation review reviewed_at_utc must include a timezone")
        return value


class LmsPackageInstallationDryRunExecutorImplementationReviewSummary(BaseModel):
    model_config = ConfigDict(extra="forbid")

    dry_run_executor_implementation_review_step_count: int
    required_dry_run_executor_implementation_review_evidence_count: int
    blocking_reason_count: int


class LmsPackageInstallationDryRunExecutorImplementationReviewResponse(BaseModel):
    model_config = ConfigDict(extra="forbid")

    schema_version: str = LMS_PACKAGE_INSTALLATION_DRY_RUN_EXECUTOR_IMPLEMENTATION_REVIEW_SCHEMA_VERSION
    tenant_id: str
    module_id: str = LMS_MODULE_ID
    endpoint: str = LMS_PACKAGE_INSTALLATION_DRY_RUN_EXECUTOR_IMPLEMENTATION_REVIEW_ENDPOINT
    result_contract: str = LMS_PACKAGE_INSTALLATION_DRY_RUN_EXECUTOR_IMPLEMENTATION_REVIEW_RESULT_CONTRACT
    ...
    dry_run_executor_implementation_review_statement_hash: str
    dry_run_executor_implementation_review_ref: str
    change_request_ref: str
    audit_chain_ref: str
    reviewed_by: str
    ...
    dry_run_executor_implementation_review_requested: bool
    dry_run_executor_implementation_review_ready: bool
    package_installation_dry_run_executor_implementation_required: bool = True
    dry_run_result_contract_required: bool = True
    ...
    dry_run_executor_implementation_review_steps: tuple[str, ...]
    required_dry_run_executor_implementation_review_evidence: tuple[str, ...]
    blocking_reasons: tuple[str, ...]
    summary: LmsPackageInstallationDryRunExecutorImplementationReviewSummary
    ...
    @field_validator(
        ...
        "result_contract",
        "continuity_domain",
        "dry_run_executor_implementation_review_ref",
        "change_request_ref",
        "audit_chain_ref",
        "reviewed_by",
        "next_action",
    ...
    def require_non_empty_text(cls, value: str) -> str:
        if not value.strip():
            raise ValueError("LMS dry-run executor implementation review text fields must not be empty")
        return value
    ...
    @field_validator(
        "dry_run_executor_implementation_review_steps",
        "required_dry_run_executor_implementation_review_evidence",
        "blocking_reasons",
        "evidence_refs",
    )
    @classmethod
    def require_unique_lists(cls, value: tuple[str, ...]) -> tuple[str, ...]:
        if len(set(value)) != len(value):
            raise ValueError("LMS dry-run executor implementation review lists must not contain duplicates")
        for item in value:
            if not item.strip():
                raise ValueError("LMS dry-run executor implementation review list items must not be empty")
        return value
    ...
    def require_non_executing_skeleton_contract(
        ...
        if self.schema_version != LMS_PACKAGE_INSTALLATION_DRY_RUN_EXECUTOR_IMPLEMENTATION_REVIEW_SCHEMA_VERSION:
            raise ValueError("LMS dry-run executor implementation review schema version is invalid")
        if self.endpoint != LMS_PACKAGE_INSTALLATION_DRY_RUN_EXECUTOR_IMPLEMENTATION_REVIEW_ENDPOINT:
            raise ValueError("LMS dry-run executor implementation review endpoint is invalid")
        if self.result_contract != LMS_PACKAGE_INSTALLATION_DRY_RUN_EXECUTOR_IMPLEMENTATION_REVIEW_RESULT_CONTRACT:
            raise ValueError("LMS dry-run executor implementation review result contract is invalid")
        if self.module_id != LMS_MODULE_ID:
            raise ValueError("LMS dry-run executor implementation review only applies to lms")
        if self.continuity_domain != LMS_CONTINUITY_DOMAIN:
            raise ValueError("LMS dry-run executor implementation review continuity domain is invalid")
        expected_ready = (
            self.package_installation_ready
            and self.reviewer_role_allowed
            and self.dry_run_executor_implementation_review_requested
            and not self.blocking_reasons
        )
        if self.dry_run_executor_implementation_review_ready != expected_ready:
            raise ValueError("LMS dry-run executor implementation review readiness must match prerequisites")
        if not self.package_installation_dry_run_executor_implementation_required:
            raise ValueError(
                "LMS dry-run executor implementation review must require a future dry-run executor implementation"
            )
        if not self.dry_run_result_contract_required:
            raise ValueError("LMS dry-run executor implementation review must require a future dry-run result contract")
        if (
            ...
            raise ValueError("LMS dry-run executor implementation review must remain metadata-only and non-executing")
        if self.summary.dry_run_executor_implementation_review_step_count != len(
            self.dry_run_executor_implementation_review_steps
        ):
            raise ValueError("LMS dry-run executor implementation review step count must match skeleton steps")
        if self.summary.required_dry_run_executor_implementation_review_evidence_count != len(
            self.required_dry_run_executor_implementation_review_evidence
        ):
            raise ValueError("LMS dry-run executor implementation review evidence count must match evidence list")
        if self.summary.blocking_reason_count != len(self.blocking_reasons):
            raise ValueError("LMS dry-run executor implementation review blocking count must match blocking reasons")
        return self


def build_lms_package_installation_dry_run_executor_implementation_review_response(
    *,
    ...
    command_hash = build_lms_package_installation_dry_run_executor_implementation_review_command_hash(command)
    idempotency_key_hash = stable_hash(
        canonical_json(
            {
                "schema_version": "lms_package_installation_dry_run_executor_implementation_review_idempotency_key.v1",
    ...
    review_statement_hash = stable_hash(command.dry_run_executor_implementation_review_statement)
    reviewer_role_allowed = bool({"tenant-admin", "tenant_admin"} & user_context.role_ids)
    blocking_reasons = _dry_run_executor_implementation_review_blocking_reasons(
        command=command,
    ...
    implementation_review_ready = not blocking_reasons
    review_steps = (
        ...
        "plan_read_only_catalog_and_metadata_schema_checks",
        "plan_idempotent_no_write_dry_run_receipt",
    ...
    required_evidence = (
        ...
        "exact_dry_run_executor_implementation_review_statement_hash",
        "change_request_ref",
        "idempotency_key_hash",
        "audit_chain_ref",
        "future_dry_run_executor_implementation_required",
        "future_dry_run_result_contract_required",
        "no_lms_dry_run_execution_confirmation",
        "no_dry_run_result_persistence_confirmation",
    )
    draft = LmsPackageInstallationDryRunExecutorImplementationReviewResponse(
        tenant_id=user_context.tenant_id,
        package_installation_ready=readiness.package_installation_ready,
        ...
        dry_run_executor_implementation_review_statement_hash=review_statement_hash,
        dry_run_executor_implementation_review_ref=command.dry_run_executor_implementation_review_ref,
        ...
        reviewer_role_allowed=reviewer_role_allowed,
        dry_run_executor_implementation_review_requested=command.dry_run_executor_implementation_review_requested,
        dry_run_executor_implementation_review_ready=implementation_review_ready,
        dry_run_executor_implementation_review_steps=review_steps,
        required_dry_run_executor_implementation_review_evidence=required_evidence,
        blocking_reasons=blocking_reasons,
        summary=LmsPackageInstallationDryRunExecutorImplementationReviewSummary(
            dry_run_executor_implementation_review_step_count=len(review_steps),
            required_dry_run_executor_implementation_review_evidence_count=len(required_evidence),
        ...
        next_action=(
            LMS_PACKAGE_INSTALLATION_DRY_RUN_EXECUTOR_IMPLEMENTATION_REVIEW_READY_NEXT_ACTION
            if implementation_review_ready
            else LMS_PACKAGE_INSTALLATION_DRY_RUN_EXECUTOR_IMPLEMENTATION_REVIEW_RETRY_NEXT_ACTION
        ),
    )
    return draft.model_copy(
        update={"evidence_hash": build_lms_package_installation_dry_run_executor_implementation_review_hash(draft)}
    )


def build_lms_package_installation_dry_run_executor_implementation_review_command_hash(
    command: LmsPackageInstallationDryRunExecutorImplementationReviewCommand,
) -> str:
    payload = command.model_dump(mode="json", exclude={"dry_run_executor_implementation_review_statement"})
    payload["dry_run_executor_implementation_review_statement_hash"] = stable_hash(
        command.dry_run_executor_implementation_review_statement
    )
    return stable_hash(canonical_json(payload))


def build_lms_package_installation_dry_run_executor_implementation_review_hash(
    response: LmsPackageInstallationDryRunExecutorImplementationReviewResponse,
) -> str:
    return stable_hash(canonical_json(response.model_dump(mode="json", exclude={"evidence_hash"})))


def _dry_run_executor_implementation_review_blocking_reasons(
    *,
    command: LmsPackageInstallationDryRunExecutorImplementationReviewCommand,
    package_installation_ready: bool,
    ...
    if not command.dry_run_executor_implementation_review_requested:
        reasons.append("dry_run_executor_implementation_review_not_requested")
