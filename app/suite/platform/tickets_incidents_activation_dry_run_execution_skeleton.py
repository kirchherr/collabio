...
TICKETS_INCIDENTS_ACTIVATION_DRY_RUN_EXECUTION_SKELETON_SCHEMA_VERSION = (
    "tickets_incidents_activation_dry_run_execution_skeleton.v1"
)
...
TICKETS_INCIDENTS_ACTIVATION_DRY_RUN_EXECUTION_SKELETON_PREPARATION_STATEMENT = (
    "I prepare the Tickets & Incidents activation dry-run execution skeleton without executing "
    "dry-run or tenant activation."
)
TICKETS_INCIDENTS_ACTIVATION_DRY_RUN_EXECUTION_SKELETON_READY_NEXT_ACTION = (
    "review_tickets_incidents_activation_dry_run_executor_implementation_without_tenant_activation"
)
TICKETS_INCIDENTS_ACTIVATION_DRY_RUN_EXECUTION_SKELETON_RETRY_NEXT_ACTION = (
...
class TicketsIncidentsActivationDryRunExecutionSkeletonCommand(BaseModel):
    ...
    activation_dry_run_execution_skeleton_ref: str
    change_request_ref: str
    idempotency_key_ref: str
    prepared_at_utc: datetime
    audit_chain_ref: str
    activation_dry_run_execution_skeleton_preparation_statement: str
    activation_dry_run_execution_skeleton_requested: bool = True
    ...
    def require_sha256(cls, value: str) -> str:
        if not SHA256_PATTERN.fullmatch(value):
            raise ValueError(
                "Tickets & Incidents activation dry-run execution skeleton hashes must be sha256 references"
            )
        return value
    ...
    def require_ref(cls, value: str) -> str:
        if not REF_PATTERN.fullmatch(value.strip()):
            raise ValueError(
                "Tickets & Incidents activation dry-run execution skeleton references need typed ref prefixes"
            )
        return value.strip()

    @field_validator("activation_dry_run_execution_skeleton_preparation_statement")
    @classmethod
    def require_exact_preparation_statement(cls, value: str) -> str:
        normalized = value.strip()
        if normalized != TICKETS_INCIDENTS_ACTIVATION_DRY_RUN_EXECUTION_SKELETON_PREPARATION_STATEMENT:
            raise ValueError("Tickets & Incidents activation dry-run execution skeleton requires the exact statement")
        return normalized
    ...
    def require_timezone_aware_timestamp(cls, value: datetime) -> datetime:
        if value.tzinfo is None or value.utcoffset() is None:
            raise ValueError(
                "Tickets & Incidents activation dry-run execution skeleton prepared_at_utc needs a timezone"
            )
        return value


class TicketsIncidentsActivationDryRunExecutionSkeletonSummary(BaseModel):
    model_config = ConfigDict(extra="forbid")
...
class TicketsIncidentsActivationDryRunExecutionSkeletonResponse(BaseModel):
    model_config = ConfigDict(extra="forbid")

    schema_version: str = TICKETS_INCIDENTS_ACTIVATION_DRY_RUN_EXECUTION_SKELETON_SCHEMA_VERSION
    tenant_id: str
    module_id: str = TICKETS_INCIDENTS_MODULE_ID
    endpoint: str = TICKETS_INCIDENTS_ACTIVATION_DRY_RUN_EXECUTION_SKELETON_ENDPOINT
    result_contract: str = TICKETS_INCIDENTS_ACTIVATION_DRY_RUN_EXECUTION_SKELETON_RESULT_CONTRACT
    ...
    activation_dry_run_execution_skeleton_preparation_statement_hash: str
    activation_dry_run_execution_skeleton_ref: str
    change_request_ref: str
    audit_chain_ref: str
    prepared_by: str
    ...
    activation_dry_run_execution_skeleton_requested: bool
    activation_dry_run_execution_skeleton_prepared: bool
    ...
    summary: TicketsIncidentsActivationDryRunExecutionSkeletonSummary
    ...
    def require_non_empty_text(cls, value: str) -> str:
        if not value.strip():
            raise ValueError("Tickets & Incidents activation dry-run execution skeleton text fields must not be empty")
        return value
    ...
    @field_validator(
        ...
        "activation_dry_run_execution_skeleton_preparation_statement_hash",
        "evidence_hash",
    )
    @classmethod
    ...
    def require_unique_lists(cls, value: tuple[str, ...]) -> tuple[str, ...]:
        if len(set(value)) != len(value):
            raise ValueError(
                "Tickets & Incidents activation dry-run execution skeleton lists must not contain duplicates"
            )
        for item in value:
            if not item.strip():
                raise ValueError(
                    "Tickets & Incidents activation dry-run execution skeleton list items must not be empty"
                )
        return value

    @model_validator(mode="after")
    def require_non_executing_skeleton_contract(self) -> TicketsIncidentsActivationDryRunExecutionSkeletonResponse:
        if self.schema_version != TICKETS_INCIDENTS_ACTIVATION_DRY_RUN_EXECUTION_SKELETON_SCHEMA_VERSION:
            raise ValueError("Tickets & Incidents activation dry-run execution skeleton schema version is invalid")
        if self.endpoint != TICKETS_INCIDENTS_ACTIVATION_DRY_RUN_EXECUTION_SKELETON_ENDPOINT:
            raise ValueError("Tickets & Incidents activation dry-run execution skeleton endpoint is invalid")
        if self.result_contract != TICKETS_INCIDENTS_ACTIVATION_DRY_RUN_EXECUTION_SKELETON_RESULT_CONTRACT:
            raise ValueError("Tickets & Incidents activation dry-run execution skeleton result contract is invalid")
        if self.module_id != TICKETS_INCIDENTS_MODULE_ID:
            raise ValueError(
                "Tickets & Incidents activation dry-run execution skeleton only applies to tickets_incidents"
            )
        if self.continuity_domain != TICKETS_INCIDENTS_CONTINUITY_DOMAIN:
            raise ValueError("Tickets & Incidents activation dry-run execution skeleton continuity domain is invalid")
        expected_prepared = (
            self.approval_gate_ready
            and self.human_approval_ready
            and self.preparer_role_allowed
            and self.activation_dry_run_execution_skeleton_requested
            and not self.blocking_reasons
        )
        if self.activation_dry_run_execution_skeleton_prepared != expected_prepared:
            raise ValueError("Tickets & Incidents activation dry-run execution skeleton readiness is inconsistent")
        if not self.activation_dry_run_executor_implementation_required:
            raise ValueError(
                "Tickets & Incidents activation dry-run execution skeleton must require future executor review"
            )
        if not self.dry_run_result_contract_required:
            raise ValueError("Tickets & Incidents activation dry-run execution skeleton must require result contract")
        if (
            ...
            raise ValueError("Tickets & Incidents activation dry-run execution skeleton must remain metadata-only")
        if self.summary.dry_run_execution_skeleton_step_count != len(self.dry_run_execution_skeleton_steps):
            raise ValueError("Tickets & Incidents activation dry-run execution skeleton step count mismatch")
        if self.summary.required_dry_run_execution_skeleton_evidence_count != len(
            self.required_dry_run_execution_skeleton_evidence
        ):
            raise ValueError("Tickets & Incidents activation dry-run execution skeleton evidence count mismatch")
        if self.summary.blocking_reason_count != len(self.blocking_reasons):
            raise ValueError("Tickets & Incidents activation dry-run execution skeleton blocking count mismatch")
        return self


def build_tickets_incidents_activation_dry_run_execution_skeleton_response(
    *,
    command: TicketsIncidentsActivationDryRunExecutionSkeletonCommand,
...
) -> TicketsIncidentsActivationDryRunExecutionSkeletonResponse:
    ...
    command_hash = build_tickets_incidents_activation_dry_run_execution_skeleton_command_hash(command)
    idempotency_key_hash = stable_hash(
        canonical_json(
            {
                "schema_version": "tickets_incidents_activation_dry_run_execution_skeleton_idempotency_key.v1",
                "tenant_id": user_context.tenant_id,
    ...
    preparation_statement_hash = stable_hash(command.activation_dry_run_execution_skeleton_preparation_statement)
    preparer_role_allowed = bool({"tenant-admin", "tenant_admin"} & user_context.role_ids)
    blocking_reasons = _activation_dry_run_execution_skeleton_blocking_reasons(
        command=command,
    ...
    skeleton_steps = (
        ...
        "plan_read_only_ticket_metadata_schema_checks",
    ...
    required_evidence = (
        ...
        "tickets_restore_drill_evidence_hash",
        "exact_activation_dry_run_execution_skeleton_preparation_statement_hash",
        "change_request_ref",
        "idempotency_key_hash",
        "audit_chain_ref",
    ...
    draft = TicketsIncidentsActivationDryRunExecutionSkeletonResponse(
        tenant_id=user_context.tenant_id,
        approval_gate_ready=approval_gate.approval_gate_ready,
        ...
        activation_dry_run_execution_skeleton_preparation_statement_hash=preparation_statement_hash,
        activation_dry_run_execution_skeleton_ref=command.activation_dry_run_execution_skeleton_ref,
        ...
        preparer_role_allowed=preparer_role_allowed,
        activation_dry_run_execution_skeleton_requested=command.activation_dry_run_execution_skeleton_requested,
        activation_dry_run_execution_skeleton_prepared=skeleton_prepared,
        ...
        next_action=(
            ...
            else TICKETS_INCIDENTS_ACTIVATION_DRY_RUN_EXECUTION_SKELETON_RETRY_NEXT_ACTION
        ),
    )
    return draft.model_copy(
        update={"evidence_hash": build_tickets_incidents_activation_dry_run_execution_skeleton_hash(draft)}
    )


def build_tickets_incidents_activation_dry_run_execution_skeleton_command_hash(
    command: TicketsIncidentsActivationDryRunExecutionSkeletonCommand,
) -> str:
    payload = command.model_dump(
        mode="json",
        exclude={"activation_dry_run_execution_skeleton_preparation_statement"},
    )
    payload["activation_dry_run_execution_skeleton_preparation_statement_hash"] = stable_hash(
        command.activation_dry_run_execution_skeleton_preparation_statement
    )
    return stable_hash(canonical_json(payload))


def build_tickets_incidents_activation_dry_run_execution_skeleton_hash(
    response: TicketsIncidentsActivationDryRunExecutionSkeletonResponse,
) -> str:
    return stable_hash(canonical_json(response.model_dump(mode="json", exclude={"evidence_hash"})))


def _activation_dry_run_execution_skeleton_blocking_reasons(
    *,
    command: TicketsIncidentsActivationDryRunExecutionSkeletonCommand,
    approval_gate_ready: bool,
    ...
    if not command.activation_dry_run_execution_skeleton_requested:
        reasons.append("activation_dry_run_execution_skeleton_not_requested")
