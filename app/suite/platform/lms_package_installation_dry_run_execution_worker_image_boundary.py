...
from suite.platform.lms_package_installation_dry_run_execution_scheduler_boundary import (
    LMS_PACKAGE_INSTALLATION_DRY_RUN_EXECUTION_SCHEDULER_BOUNDARY_ENDPOINT,
)
...
LMS_PACKAGE_INSTALLATION_DRY_RUN_EXECUTION_WORKER_IMAGE_BOUNDARY_RESULT_CONTRACT = (
    "metadata_only_lms_package_installation_dry_run_execution_worker_image_boundary_no_resolution"
)
...
LMS_PACKAGE_INSTALLATION_DRY_RUN_EXECUTION_WORKER_IMAGE_BOUNDARY_STATEMENT = (
    "I prepare the LMS package installation dry-run execution worker image boundary without resolving worker images, "
    "pulling images, looking up image digests, enqueuing workers, starting dry-run execution, persisting dry-run "
    "results, or executing package installation."
)
LMS_PACKAGE_INSTALLATION_DRY_RUN_EXECUTION_WORKER_IMAGE_BOUNDARY_READY_NEXT_ACTION = (
    "prepare_lms_dry_run_execution_dispatch_boundary_without_dispatch"
)
LMS_PACKAGE_INSTALLATION_DRY_RUN_EXECUTION_WORKER_IMAGE_BOUNDARY_RETRY_NEXT_ACTION = (
...
class LmsPackageInstallationDryRunExecutionWorkerImageBoundaryCommand(BaseModel):
    model_config = ConfigDict(extra="forbid")

    dry_run_execution_scheduler_boundary_evidence_hash: str
    dry_run_execution_plan_review_evidence_hash: str
    ...
    dry_run_execution_worker_image_boundary_ref: str
    dry_run_execution_plan_ref: str
    execution_window_ref: str
    resource_budget_ref: str
    scheduler_policy_ref: str
    worker_image_policy_ref: str
    worker_image_catalog_ref: str
    worker_image_resolution_boundary_ref: str
    idempotency_key_ref: str
    change_request_ref: str
    prepared_at_utc: datetime
    audit_chain_ref: str
    dry_run_execution_worker_image_boundary_statement: str
    dry_run_execution_worker_image_boundary_requested: bool = True
    ...
    @field_validator(
        ...
        "dry_run_execution_plan_review_evidence_hash",
    ...
    def require_sha256(cls, value: str) -> str:
        if not SHA256_PATTERN.fullmatch(value):
            raise ValueError("LMS dry-run execution worker image boundary hashes must be sha256 references")
        return value

    @field_validator(
        "dry_run_execution_worker_image_boundary_ref",
        "dry_run_execution_plan_ref",
        "execution_window_ref",
        "resource_budget_ref",
        "scheduler_policy_ref",
        "worker_image_policy_ref",
        "worker_image_catalog_ref",
        "worker_image_resolution_boundary_ref",
        "idempotency_key_ref",
        "change_request_ref",
        "audit_chain_ref",
    )
    ...
    def require_ref(cls, value: str) -> str:
        if not REF_PATTERN.fullmatch(value.strip()):
            raise ValueError("LMS dry-run execution worker image boundary references must use a typed ref prefix")
        return value.strip()

    @field_validator("dry_run_execution_worker_image_boundary_statement")
    @classmethod
    def require_exact_statement(cls, value: str) -> str:
        normalized = value.strip()
        if normalized != LMS_PACKAGE_INSTALLATION_DRY_RUN_EXECUTION_WORKER_IMAGE_BOUNDARY_STATEMENT:
            raise ValueError("LMS dry-run execution worker image boundary requires the exact preparation statement")
        return normalized
    ...
    def require_timezone_aware_timestamp(cls, value: datetime) -> datetime:
        if value.tzinfo is None or value.utcoffset() is None:
            raise ValueError("LMS dry-run execution worker image boundary prepared_at_utc must include a timezone")
        return value


class LmsPackageInstallationDryRunExecutionWorkerImageBoundarySummary(BaseModel):
    model_config = ConfigDict(extra="forbid")

    dry_run_execution_worker_image_boundary_step_count: int
    required_dry_run_execution_worker_image_boundary_evidence_count: int
    blocking_reason_count: int


class LmsPackageInstallationDryRunExecutionWorkerImageBoundaryResponse(BaseModel):
    model_config = ConfigDict(extra="forbid")

    schema_version: str = LMS_PACKAGE_INSTALLATION_DRY_RUN_EXECUTION_WORKER_IMAGE_BOUNDARY_SCHEMA_VERSION
    tenant_id: str
    module_id: str = LMS_MODULE_ID
    endpoint: str = LMS_PACKAGE_INSTALLATION_DRY_RUN_EXECUTION_WORKER_IMAGE_BOUNDARY_ENDPOINT
    result_contract: str = LMS_PACKAGE_INSTALLATION_DRY_RUN_EXECUTION_WORKER_IMAGE_BOUNDARY_RESULT_CONTRACT
    ...
    dry_run_execution_scheduler_boundary_endpoint: str = (
        LMS_PACKAGE_INSTALLATION_DRY_RUN_EXECUTION_SCHEDULER_BOUNDARY_ENDPOINT
    )
    dry_run_execution_scheduler_boundary_evidence_hash: str
    dry_run_execution_plan_review_evidence_hash: str
    ...
    dry_run_execution_worker_image_boundary_statement_hash: str
    dry_run_execution_worker_image_boundary_ref: str
    dry_run_execution_plan_ref: str
    execution_window_ref: str
    resource_budget_ref: str
    ...
    dry_run_execution_worker_image_boundary_requested: bool
    dry_run_execution_worker_image_boundary_ready: bool
    explicit_human_execution_approval_present: bool
    approval_record_tenant_match: bool
    approval_record_hash_match: bool
    future_dispatch_boundary_required: bool = True
    ...
    dry_run_execution_worker_image_boundary_steps: tuple[str, ...]
    required_dry_run_execution_worker_image_boundary_evidence: tuple[str, ...]
    blocking_reasons: tuple[str, ...]
    summary: LmsPackageInstallationDryRunExecutionWorkerImageBoundarySummary
    ...
    @field_validator(
        ...
        "result_contract",
        "continuity_domain",
        "dry_run_execution_scheduler_boundary_endpoint",
        "dry_run_execution_worker_image_boundary_ref",
        "dry_run_execution_plan_ref",
        "execution_window_ref",
        "resource_budget_ref",
    ...
    def require_non_empty_text(cls, value: str) -> str:
        if not value.strip():
            raise ValueError("LMS dry-run execution worker image boundary text fields must not be empty")
        return value
    ...
    @field_validator(
        ...
        "dry_run_execution_worker_image_boundary_statement_hash",
        "evidence_hash",
    )
    @classmethod
    ...
    @field_validator(
        "dry_run_execution_worker_image_boundary_steps",
        "required_dry_run_execution_worker_image_boundary_evidence",
        "blocking_reasons",
        "evidence_refs",
    )
    @classmethod
    def require_unique_lists(cls, value: tuple[str, ...]) -> tuple[str, ...]:
        if len(set(value)) != len(value):
            raise ValueError("LMS dry-run execution worker image boundary lists must not contain duplicates")
        for item in value:
            if not item.strip():
                raise ValueError("LMS dry-run execution worker image boundary list items must not be empty")
        return value

    @model_validator(mode="after")
    def require_non_resolving_worker_image_boundary(
        self,
    ) -> LmsPackageInstallationDryRunExecutionWorkerImageBoundaryResponse:
        if self.schema_version != LMS_PACKAGE_INSTALLATION_DRY_RUN_EXECUTION_WORKER_IMAGE_BOUNDARY_SCHEMA_VERSION:
            raise ValueError("LMS dry-run execution worker image boundary schema version is invalid")
        if self.endpoint != LMS_PACKAGE_INSTALLATION_DRY_RUN_EXECUTION_WORKER_IMAGE_BOUNDARY_ENDPOINT:
            raise ValueError("LMS dry-run execution worker image boundary endpoint is invalid")
        if self.result_contract != LMS_PACKAGE_INSTALLATION_DRY_RUN_EXECUTION_WORKER_IMAGE_BOUNDARY_RESULT_CONTRACT:
            raise ValueError("LMS dry-run execution worker image boundary result contract is invalid")
        if self.module_id != LMS_MODULE_ID:
            raise ValueError("LMS dry-run execution worker image boundary only applies to lms")
        if self.continuity_domain != LMS_CONTINUITY_DOMAIN:
            raise ValueError("LMS dry-run execution worker image boundary continuity domain is invalid")
        if self.dry_run_execution_scheduler_boundary_endpoint != (
            LMS_PACKAGE_INSTALLATION_DRY_RUN_EXECUTION_SCHEDULER_BOUNDARY_ENDPOINT
        ):
            raise ValueError("LMS dry-run execution worker image boundary scheduler endpoint is invalid")
        if self.explicit_human_execution_approval_present != (
            self.approval_record_tenant_match and self.approval_record_hash_match
        ):
            raise ValueError("LMS dry-run execution worker image boundary approval flags must match stored record")
        expected_ready = (
            self.package_installation_ready
            and self.preparer_role_allowed
            and self.dry_run_execution_worker_image_boundary_requested
            and self.explicit_human_execution_approval_present
            and not self.blocking_reasons
        )
        if self.dry_run_execution_worker_image_boundary_ready != expected_ready:
            raise ValueError("LMS dry-run execution worker image boundary readiness must match prerequisites")
        if not self.future_dispatch_boundary_required:
            raise ValueError("LMS dry-run execution worker image boundary must require a future dispatch boundary")
        if (
            ...
            raise ValueError("LMS dry-run execution worker image boundary must remain metadata-only and non-resolving")
        if self.summary.dry_run_execution_worker_image_boundary_step_count != len(
            self.dry_run_execution_worker_image_boundary_steps
        ):
            raise ValueError("LMS dry-run execution worker image boundary step count must match boundary steps")
        if self.summary.required_dry_run_execution_worker_image_boundary_evidence_count != len(
            self.required_dry_run_execution_worker_image_boundary_evidence
        ):
            raise ValueError("LMS dry-run execution worker image boundary evidence count must match evidence list")
        if self.summary.blocking_reason_count != len(self.blocking_reasons):
            raise ValueError("LMS dry-run execution worker image boundary blocking count must match blocking reasons")
        return self


def build_lms_package_installation_dry_run_execution_worker_image_boundary_response(
    *,
    command: LmsPackageInstallationDryRunExecutionWorkerImageBoundaryCommand,
    ...
    command_hash = build_lms_package_installation_dry_run_execution_worker_image_boundary_command_hash(command)
    idempotency_key_hash = stable_hash(
        canonical_json(
            {
                "schema_version": "lms_package_installation_dry_run_execution_worker_image_boundary_idempotency_key.v1",
                "tenant_id": user_context.tenant_id,
                "dry_run_execution_scheduler_boundary_evidence_hash": (
                    command.dry_run_execution_scheduler_boundary_evidence_hash
                ),
                "dry_run_execution_plan_review_evidence_hash": (command.dry_run_execution_plan_review_evidence_hash),
                ...
                "worker_image_policy_ref": command.worker_image_policy_ref,
                "worker_image_catalog_ref": command.worker_image_catalog_ref,
                "worker_image_resolution_boundary_ref": command.worker_image_resolution_boundary_ref,
                "idempotency_key_ref": command.idempotency_key_ref,
            }
        )
    )
    statement_hash = stable_hash(command.dry_run_execution_worker_image_boundary_statement)
    preparer_role_allowed = bool({"tenant-admin", "tenant_admin"} & user_context.role_ids)
    blocking_reasons = _dry_run_execution_worker_image_boundary_blocking_reasons(
        command=command,
    ...
    worker_image_boundary_ready = not blocking_reasons
    boundary_steps = (
        "verify_lms_catalog_status_not_installed",
        "bind_lms_package_installation_dry_run_execution_scheduler_boundary_hash",
        "bind_lms_package_installation_dry_run_execution_plan_review_hash",
        ...
        "bind_execution_window_ref",
        "bind_resource_budget_ref",
        "bind_scheduler_policy_ref_without_activation",
        "bind_worker_image_policy_ref_without_resolution",
        "bind_worker_image_catalog_ref_without_lookup",
        "bind_worker_image_resolution_boundary_ref_as_future_gate",
        "define_worker_image_boundary_idempotency_and_hash_closure",
        "define_worker_image_boundary_as_non_resolving_artifact",
        "require_future_dispatch_boundary_before_enqueue_or_dispatch",
        "confirm_scheduler_not_activated",
        "confirm_scheduler_job_not_created",
        "confirm_worker_image_not_resolved",
        "confirm_worker_image_not_pulled",
        "confirm_worker_image_digest_not_looked_up",
    ...
    required_evidence = (
        ...
        "package_installation_dry_run_execution_scheduler_boundary_hash",
        "package_installation_dry_run_execution_plan_review_hash",
        ...
        "exact_dry_run_execution_worker_image_boundary_statement_hash",
        "execution_window_ref",
        "resource_budget_ref",
        "scheduler_policy_ref",
        "worker_image_policy_ref",
        ...
        "idempotency_key_hash",
        "audit_chain_ref",
        "dry_run_execution_worker_image_boundary_schema_version",
        "future_dispatch_boundary_required",
        "scheduler_not_activated",
        "scheduler_job_not_created",
    ...
    draft = LmsPackageInstallationDryRunExecutionWorkerImageBoundaryResponse(
        tenant_id=user_context.tenant_id,
        package_installation_ready=readiness.package_installation_ready,
        ...
        dry_run_execution_worker_image_boundary_statement_hash=statement_hash,
        dry_run_execution_worker_image_boundary_ref=command.dry_run_execution_worker_image_boundary_ref,
        ...
        preparer_role_allowed=preparer_role_allowed,
        dry_run_execution_worker_image_boundary_requested=command.dry_run_execution_worker_image_boundary_requested,
        dry_run_execution_worker_image_boundary_ready=worker_image_boundary_ready,
        ...
        approval_record_hash_match=approval_record_hash_match,
        dry_run_execution_worker_image_boundary_steps=boundary_steps,
        required_dry_run_execution_worker_image_boundary_evidence=required_evidence,
        blocking_reasons=blocking_reasons,
        summary=LmsPackageInstallationDryRunExecutionWorkerImageBoundarySummary(
            dry_run_execution_worker_image_boundary_step_count=len(boundary_steps),
            required_dry_run_execution_worker_image_boundary_evidence_count=len(required_evidence),
        ...
        next_action=(
            LMS_PACKAGE_INSTALLATION_DRY_RUN_EXECUTION_WORKER_IMAGE_BOUNDARY_READY_NEXT_ACTION
            if worker_image_boundary_ready
            else LMS_PACKAGE_INSTALLATION_DRY_RUN_EXECUTION_WORKER_IMAGE_BOUNDARY_RETRY_NEXT_ACTION
        ),
    )
    return draft.model_copy(
        update={"evidence_hash": build_lms_package_installation_dry_run_execution_worker_image_boundary_hash(draft)}
    )


def build_lms_package_installation_dry_run_execution_worker_image_boundary_command_hash(
    command: LmsPackageInstallationDryRunExecutionWorkerImageBoundaryCommand,
) -> str:
    payload = command.model_dump(mode="json", exclude={"dry_run_execution_worker_image_boundary_statement"})
    payload["dry_run_execution_worker_image_boundary_statement_hash"] = stable_hash(
        command.dry_run_execution_worker_image_boundary_statement
    )
    return stable_hash(canonical_json(payload))


def build_lms_package_installation_dry_run_execution_worker_image_boundary_hash(
    response: LmsPackageInstallationDryRunExecutionWorkerImageBoundaryResponse,
) -> str:
    return stable_hash(canonical_json(response.model_dump(mode="json", exclude={"evidence_hash"})))


def _dry_run_execution_worker_image_boundary_blocking_reasons(
    *,
    command: LmsPackageInstallationDryRunExecutionWorkerImageBoundaryCommand,
    package_installation_ready: bool,
    ...
    if command.dry_run_execution_scheduler_boundary_evidence_hash == ZERO_SHA256:
        reasons.append("package_installation_dry_run_execution_scheduler_boundary_hash_missing")
    if command.dry_run_execution_plan_review_evidence_hash == ZERO_SHA256:
        reasons.append("package_installation_dry_run_execution_plan_review_hash_missing")
    ...
    if not command.dry_run_execution_worker_image_boundary_requested:
        reasons.append("dry_run_execution_worker_image_boundary_not_requested")
