...
LMS_PACKAGE_INSTALLATION_DRY_RUN_EXECUTION_RUNBOOK_SCHEMA_VERSION = (
    "lms_package_installation_dry_run_execution_runbook.v1"
)
LMS_PACKAGE_INSTALLATION_DRY_RUN_EXECUTION_RUNBOOK_RESULT_CONTRACT = (
    "metadata_only_lms_package_installation_dry_run_execution_runbook_no_execution"
)
...
LMS_PACKAGE_INSTALLATION_DRY_RUN_EXECUTION_RUNBOOK_STATEMENT = (
    "I prepare the LMS package installation dry-run execution runbook without enqueuing workers, "
    "starting dry-run execution, persisting dry-run results, or executing package installation."
)
LMS_PACKAGE_INSTALLATION_DRY_RUN_EXECUTION_RUNBOOK_READY_NEXT_ACTION = (
    "prepare_lms_dry_run_execution_plan_without_execution"
)
LMS_PACKAGE_INSTALLATION_DRY_RUN_EXECUTION_RUNBOOK_RETRY_NEXT_ACTION = (
...
class LmsPackageInstallationDryRunExecutionRunbookCommand(BaseModel):
    ...
    dry_run_execution_runbook_ref: str
    ...
    audit_chain_ref: str
    dry_run_execution_runbook_statement: str
    dry_run_execution_runbook_requested: bool = True
    ...
    def require_sha256(cls, value: str) -> str:
        if not SHA256_PATTERN.fullmatch(value):
            raise ValueError("LMS dry-run execution runbook hashes must be sha256 references")
        return value

    @field_validator(
        "dry_run_execution_runbook_ref",
    ...
    def require_ref(cls, value: str) -> str:
        if not REF_PATTERN.fullmatch(value.strip()):
            raise ValueError("LMS dry-run execution runbook references must use a typed ref prefix")
        return value.strip()

    @field_validator("dry_run_execution_runbook_statement")
    @classmethod
    def require_exact_statement(cls, value: str) -> str:
        normalized = value.strip()
        if normalized != LMS_PACKAGE_INSTALLATION_DRY_RUN_EXECUTION_RUNBOOK_STATEMENT:
            raise ValueError("LMS dry-run execution runbook requires the exact preparation statement")
        return normalized
    ...
    def require_timezone_aware_timestamp(cls, value: datetime) -> datetime:
        if value.tzinfo is None or value.utcoffset() is None:
            raise ValueError("LMS dry-run execution runbook prepared_at_utc must include a timezone")
        return value


class LmsPackageInstallationDryRunExecutionRunbookSummary(BaseModel):
    model_config = ConfigDict(extra="forbid")

    dry_run_execution_runbook_step_count: int
    required_dry_run_execution_runbook_evidence_count: int
    blocking_reason_count: int


class LmsPackageInstallationDryRunExecutionRunbookResponse(BaseModel):
    model_config = ConfigDict(extra="forbid")

    schema_version: str = LMS_PACKAGE_INSTALLATION_DRY_RUN_EXECUTION_RUNBOOK_SCHEMA_VERSION
    tenant_id: str
    module_id: str = LMS_MODULE_ID
    endpoint: str = LMS_PACKAGE_INSTALLATION_DRY_RUN_EXECUTION_RUNBOOK_ENDPOINT
    result_contract: str = LMS_PACKAGE_INSTALLATION_DRY_RUN_EXECUTION_RUNBOOK_RESULT_CONTRACT
    ...
    dry_run_execution_admission_gate_endpoint: str = LMS_PACKAGE_INSTALLATION_DRY_RUN_EXECUTION_ADMISSION_GATE_ENDPOINT
    dry_run_execution_admission_gate_evidence_hash: str
    dry_run_execution_approval_boundary_evidence_hash: str
    dry_run_execution_approval_record_hash: str
    ...
    dry_run_execution_runbook_statement_hash: str
    dry_run_execution_runbook_ref: str
    backup_restore_runbook_ref: str
    rollback_runbook_ref: str
    operator_handoff_ref: str
    ...
    dry_run_execution_runbook_requested: bool
    dry_run_execution_runbook_ready: bool
    explicit_human_execution_approval_present: bool
    approval_record_tenant_match: bool
    approval_record_hash_match: bool
    future_dry_run_execution_plan_required: bool = True
    ...
    dry_run_execution_runbook_steps: tuple[str, ...]
    required_dry_run_execution_runbook_evidence: tuple[str, ...]
    blocking_reasons: tuple[str, ...]
    summary: LmsPackageInstallationDryRunExecutionRunbookSummary
    ...
    @field_validator(
        ...
        "result_contract",
        "continuity_domain",
        "dry_run_execution_admission_gate_endpoint",
        "dry_run_execution_runbook_ref",
        "backup_restore_runbook_ref",
        "rollback_runbook_ref",
        "operator_handoff_ref",
    ...
    def require_non_empty_text(cls, value: str) -> str:
        if not value.strip():
            raise ValueError("LMS dry-run execution runbook text fields must not be empty")
        return value
    ...
    @field_validator(
        ...
        "dry_run_execution_runbook_statement_hash",
        "evidence_hash",
    )
    @classmethod
    ...
    @field_validator(
        "dry_run_execution_runbook_steps",
        "required_dry_run_execution_runbook_evidence",
        "blocking_reasons",
        "evidence_refs",
    )
    @classmethod
    def require_unique_lists(cls, value: tuple[str, ...]) -> tuple[str, ...]:
        if len(set(value)) != len(value):
            raise ValueError("LMS dry-run execution runbook lists must not contain duplicates")
        for item in value:
            if not item.strip():
                raise ValueError("LMS dry-run execution runbook list items must not be empty")
        return value

    @model_validator(mode="after")
    def require_non_executing_runbook(self) -> LmsPackageInstallationDryRunExecutionRunbookResponse:
        if self.schema_version != LMS_PACKAGE_INSTALLATION_DRY_RUN_EXECUTION_RUNBOOK_SCHEMA_VERSION:
            raise ValueError("LMS dry-run execution runbook schema version is invalid")
        if self.endpoint != LMS_PACKAGE_INSTALLATION_DRY_RUN_EXECUTION_RUNBOOK_ENDPOINT:
            raise ValueError("LMS dry-run execution runbook endpoint is invalid")
        if self.result_contract != LMS_PACKAGE_INSTALLATION_DRY_RUN_EXECUTION_RUNBOOK_RESULT_CONTRACT:
            raise ValueError("LMS dry-run execution runbook result contract is invalid")
        if self.module_id != LMS_MODULE_ID:
            raise ValueError("LMS dry-run execution runbook only applies to lms")
        if self.continuity_domain != LMS_CONTINUITY_DOMAIN:
            raise ValueError("LMS dry-run execution runbook continuity domain is invalid")
        if self.explicit_human_execution_approval_present != (
            self.approval_record_tenant_match and self.approval_record_hash_match
        ):
            raise ValueError("LMS dry-run execution runbook approval flags must match stored approval record")
        expected_ready = (
            self.package_installation_ready
            and self.preparer_role_allowed
            and self.dry_run_execution_runbook_requested
            and self.explicit_human_execution_approval_present
            and not self.blocking_reasons
        )
        if self.dry_run_execution_runbook_ready != expected_ready:
            raise ValueError("LMS dry-run execution runbook readiness must match prerequisites")
        if not self.future_dry_run_execution_plan_required:
            raise ValueError("LMS dry-run execution runbook must require a future execution plan")
        if (
            ...
            raise ValueError("LMS dry-run execution runbook must remain metadata-only and non-executing")
        if self.summary.dry_run_execution_runbook_step_count != len(self.dry_run_execution_runbook_steps):
            raise ValueError("LMS dry-run execution runbook step count must match runbook steps")
        if self.summary.required_dry_run_execution_runbook_evidence_count != len(
            self.required_dry_run_execution_runbook_evidence
        ):
            raise ValueError("LMS dry-run execution runbook evidence count must match evidence list")
        if self.summary.blocking_reason_count != len(self.blocking_reasons):
            raise ValueError("LMS dry-run execution runbook blocking count must match blocking reasons")
        return self


def build_lms_package_installation_dry_run_execution_runbook_response(
    *,
    command: LmsPackageInstallationDryRunExecutionRunbookCommand,
...
) -> LmsPackageInstallationDryRunExecutionRunbookResponse:
    ...
    command_hash = build_lms_package_installation_dry_run_execution_runbook_command_hash(command)
    idempotency_key_hash = stable_hash(
        canonical_json(
            {
                "schema_version": "lms_package_installation_dry_run_execution_runbook_idempotency_key.v1",
                "tenant_id": user_context.tenant_id,
                "dry_run_execution_admission_gate_evidence_hash": (
    ...
    statement_hash = stable_hash(command.dry_run_execution_runbook_statement)
    preparer_role_allowed = bool({"tenant-admin", "tenant_admin"} & user_context.role_ids)
    blocking_reasons = _dry_run_execution_runbook_blocking_reasons(
        command=command,
    ...
    runbook_ready = not blocking_reasons
    runbook_steps = (
        ...
        "bind_backup_restore_runbook_ref",
        "bind_rollback_runbook_ref",
        "bind_operator_handoff_ref",
        "define_runbook_idempotency_and_hash_closure",
        "define_runbook_as_non_executing_operator_preparation",
        "require_future_dry_run_execution_plan_before_worker_or_scheduler_design",
    ...
    required_evidence = (
        ...
        "exact_dry_run_execution_runbook_statement_hash",
        "backup_restore_runbook_ref",
        "rollback_runbook_ref",
        "operator_handoff_ref",
        "change_request_ref",
        "idempotency_key_hash",
        "audit_chain_ref",
        "dry_run_execution_runbook_schema_version",
        "dry_run_execution_admission_gate_schema_version",
        "future_dry_run_execution_plan_required",
    ...
    draft = LmsPackageInstallationDryRunExecutionRunbookResponse(
        tenant_id=user_context.tenant_id,
        package_installation_ready=readiness.package_installation_ready,
        ...
        dry_run_execution_runbook_statement_hash=statement_hash,
        dry_run_execution_runbook_ref=command.dry_run_execution_runbook_ref,
        ...
        preparer_role_allowed=preparer_role_allowed,
        dry_run_execution_runbook_requested=command.dry_run_execution_runbook_requested,
        dry_run_execution_runbook_ready=runbook_ready,
        ...
        approval_record_hash_match=approval_record_hash_match,
        dry_run_execution_runbook_steps=runbook_steps,
        required_dry_run_execution_runbook_evidence=required_evidence,
        blocking_reasons=blocking_reasons,
        summary=LmsPackageInstallationDryRunExecutionRunbookSummary(
            dry_run_execution_runbook_step_count=len(runbook_steps),
            required_dry_run_execution_runbook_evidence_count=len(required_evidence),
        ...
        next_action=(
            LMS_PACKAGE_INSTALLATION_DRY_RUN_EXECUTION_RUNBOOK_READY_NEXT_ACTION
            if runbook_ready
            else LMS_PACKAGE_INSTALLATION_DRY_RUN_EXECUTION_RUNBOOK_RETRY_NEXT_ACTION
        ),
    )
    return draft.model_copy(
        update={"evidence_hash": build_lms_package_installation_dry_run_execution_runbook_hash(draft)}
    )


def build_lms_package_installation_dry_run_execution_runbook_command_hash(
    command: LmsPackageInstallationDryRunExecutionRunbookCommand,
) -> str:
    payload = command.model_dump(mode="json", exclude={"dry_run_execution_runbook_statement"})
    payload["dry_run_execution_runbook_statement_hash"] = stable_hash(command.dry_run_execution_runbook_statement)
    return stable_hash(canonical_json(payload))


def build_lms_package_installation_dry_run_execution_runbook_hash(
    response: LmsPackageInstallationDryRunExecutionRunbookResponse,
) -> str:
    return stable_hash(canonical_json(response.model_dump(mode="json", exclude={"evidence_hash"})))


def _dry_run_execution_runbook_blocking_reasons(
    *,
    command: LmsPackageInstallationDryRunExecutionRunbookCommand,
    package_installation_ready: bool,
    ...
    if not command.dry_run_execution_runbook_requested:
        reasons.append("dry_run_execution_runbook_not_requested")
