...
LMS_PACKAGE_INSTALLATION_DRY_RUN_EXECUTION_SKELETON_SCHEMA_VERSION = (
    "lms_package_installation_dry_run_execution_skeleton.v1"
)
LMS_PACKAGE_INSTALLATION_DRY_RUN_EXECUTION_SKELETON_RESULT_CONTRACT = (
    "metadata_only_lms_package_installation_dry_run_execution_skeleton_no_execution"
)
LMS_PACKAGE_INSTALLATION_DRY_RUN_EXECUTION_SKELETON_ENDPOINT = (
...
LMS_PACKAGE_INSTALLATION_DRY_RUN_EXECUTION_SKELETON_PREPARATION_STATEMENT = (
    "I prepare the LMS package installation dry-run execution skeleton without executing dry-run or tenant activation."
)
LMS_PACKAGE_INSTALLATION_DRY_RUN_EXECUTION_SKELETON_READY_NEXT_ACTION = (
    "review_lms_package_installation_dry_run_executor_implementation_without_tenant_activation"
)
LMS_PACKAGE_INSTALLATION_DRY_RUN_EXECUTION_SKELETON_RETRY_NEXT_ACTION = (
    "prepare_lms_package_installation_dry_run_execution_skeleton_without_tenant_activation"
)
...
class LmsPackageInstallationDryRunExecutionSkeletonCommand(BaseModel):
    ...
    dry_run_execution_skeleton_ref: str
    change_request_ref: str
    idempotency_key_ref: str
    prepared_at_utc: datetime
    audit_chain_ref: str
    dry_run_execution_skeleton_preparation_statement: str
    dry_run_execution_skeleton_requested: bool = True
    ...
    def require_sha256(cls, value: str) -> str:
        if not SHA256_PATTERN.fullmatch(value):
            raise ValueError("LMS dry-run execution skeleton hashes must be sha256 references")
        return value

    @field_validator("dry_run_execution_skeleton_ref", "change_request_ref", "idempotency_key_ref", "audit_chain_ref")
    @classmethod
    def require_ref(cls, value: str) -> str:
        if not REF_PATTERN.fullmatch(value.strip()):
            raise ValueError("LMS dry-run execution skeleton references must use a typed ref prefix")
        return value.strip()

    @field_validator("dry_run_execution_skeleton_preparation_statement")
    @classmethod
    def require_exact_preparation_statement(cls, value: str) -> str:
        normalized = value.strip()
        if normalized != LMS_PACKAGE_INSTALLATION_DRY_RUN_EXECUTION_SKELETON_PREPARATION_STATEMENT:
            raise ValueError("LMS dry-run execution skeleton requires the exact preparation statement")
        return normalized
    ...
    def require_timezone_aware_timestamp(cls, value: datetime) -> datetime:
        if value.tzinfo is None or value.utcoffset() is None:
            raise ValueError("LMS dry-run execution skeleton prepared_at_utc must include a timezone")
        return value


class LmsPackageInstallationDryRunExecutionSkeletonSummary(BaseModel):
    model_config = ConfigDict(extra="forbid")

    dry_run_execution_skeleton_step_count: int
    required_dry_run_execution_skeleton_evidence_count: int
    blocking_reason_count: int


class LmsPackageInstallationDryRunExecutionSkeletonResponse(BaseModel):
    model_config = ConfigDict(extra="forbid")

    schema_version: str = LMS_PACKAGE_INSTALLATION_DRY_RUN_EXECUTION_SKELETON_SCHEMA_VERSION
    tenant_id: str
    module_id: str = LMS_MODULE_ID
    endpoint: str = LMS_PACKAGE_INSTALLATION_DRY_RUN_EXECUTION_SKELETON_ENDPOINT
    result_contract: str = LMS_PACKAGE_INSTALLATION_DRY_RUN_EXECUTION_SKELETON_RESULT_CONTRACT
    ...
    dry_run_execution_skeleton_preparation_statement_hash: str
    dry_run_execution_skeleton_ref: str
    change_request_ref: str
    audit_chain_ref: str
    prepared_by: str
    ...
    dry_run_execution_skeleton_requested: bool
    dry_run_execution_skeleton_prepared: bool
    ...
    dry_run_execution_skeleton_steps: tuple[str, ...]
    required_dry_run_execution_skeleton_evidence: tuple[str, ...]
    blocking_reasons: tuple[str, ...]
    summary: LmsPackageInstallationDryRunExecutionSkeletonSummary
    ...
    @field_validator(
        ...
        "result_contract",
        "continuity_domain",
        "dry_run_execution_skeleton_ref",
        "change_request_ref",
        "audit_chain_ref",
        "prepared_by",
        "next_action",
    ...
    def require_non_empty_text(cls, value: str) -> str:
        if not value.strip():
            raise ValueError("LMS dry-run execution skeleton text fields must not be empty")
        return value
    ...
    @field_validator(
        ...
        "dry_run_execution_skeleton_preparation_statement_hash",
        "evidence_hash",
    )
    @classmethod
    ...
    @field_validator(
        "dry_run_execution_skeleton_steps",
        "required_dry_run_execution_skeleton_evidence",
        "blocking_reasons",
        "evidence_refs",
    )
    @classmethod
    def require_unique_lists(cls, value: tuple[str, ...]) -> tuple[str, ...]:
        if len(set(value)) != len(value):
            raise ValueError("LMS dry-run execution skeleton lists must not contain duplicates")
        for item in value:
            if not item.strip():
                raise ValueError("LMS dry-run execution skeleton list items must not be empty")
        return value

    @model_validator(mode="after")
    def require_non_executing_skeleton_contract(self) -> LmsPackageInstallationDryRunExecutionSkeletonResponse:
        if self.schema_version != LMS_PACKAGE_INSTALLATION_DRY_RUN_EXECUTION_SKELETON_SCHEMA_VERSION:
            raise ValueError("LMS dry-run execution skeleton schema version is invalid")
        if self.endpoint != LMS_PACKAGE_INSTALLATION_DRY_RUN_EXECUTION_SKELETON_ENDPOINT:
            raise ValueError("LMS dry-run execution skeleton endpoint is invalid")
        if self.result_contract != LMS_PACKAGE_INSTALLATION_DRY_RUN_EXECUTION_SKELETON_RESULT_CONTRACT:
            raise ValueError("LMS dry-run execution skeleton result contract is invalid")
        if self.module_id != LMS_MODULE_ID:
            raise ValueError("LMS dry-run execution skeleton only applies to lms")
        if self.continuity_domain != LMS_CONTINUITY_DOMAIN:
            raise ValueError("LMS dry-run execution skeleton continuity domain is invalid")
        expected_prepared = (
            self.package_installation_ready
            and self.preparer_role_allowed
            and self.dry_run_execution_skeleton_requested
            and not self.blocking_reasons
        )
        if self.dry_run_execution_skeleton_prepared != expected_prepared:
            raise ValueError("LMS dry-run execution skeleton readiness must match prerequisites")
        if not self.package_installation_dry_run_executor_implementation_required:
            raise ValueError("LMS dry-run execution skeleton must require a future dry-run executor implementation")
        if not self.dry_run_result_contract_required:
            raise ValueError("LMS dry-run execution skeleton must require a future dry-run result contract")
        if (
            ...
            raise ValueError("LMS dry-run execution skeleton must remain metadata-only and non-executing")
        if self.summary.dry_run_execution_skeleton_step_count != len(self.dry_run_execution_skeleton_steps):
            raise ValueError("LMS dry-run execution skeleton step count must match skeleton steps")
        if self.summary.required_dry_run_execution_skeleton_evidence_count != len(
            self.required_dry_run_execution_skeleton_evidence
        ):
            raise ValueError("LMS dry-run execution skeleton evidence count must match evidence list")
        if self.summary.blocking_reason_count != len(self.blocking_reasons):
            raise ValueError("LMS dry-run execution skeleton blocking count must match blocking reasons")
        return self


def build_lms_package_installation_dry_run_execution_skeleton_response(
    *,
    command: LmsPackageInstallationDryRunExecutionSkeletonCommand,
    ...
    command_hash = build_lms_package_installation_dry_run_execution_skeleton_command_hash(command)
    idempotency_key_hash = stable_hash(
        canonical_json(
            {
                "schema_version": "lms_package_installation_dry_run_execution_skeleton_idempotency_key.v1",
    ...
    preparation_statement_hash = stable_hash(command.dry_run_execution_skeleton_preparation_statement)
    preparer_role_allowed = bool({"tenant-admin", "tenant_admin"} & user_context.role_ids)
    blocking_reasons = _dry_run_execution_skeleton_blocking_reasons(
        command=command,
    ...
    skeleton_prepared = not blocking_reasons
    skeleton_steps = (
    ...
    required_evidence = (
        ...
        "exact_dry_run_execution_skeleton_preparation_statement_hash",
        "change_request_ref",
        "idempotency_key_hash",
        "audit_chain_ref",
    ...
    draft = LmsPackageInstallationDryRunExecutionSkeletonResponse(
        tenant_id=user_context.tenant_id,
        package_installation_ready=readiness.package_installation_ready,
        ...
        dry_run_execution_skeleton_preparation_statement_hash=preparation_statement_hash,
        dry_run_execution_skeleton_ref=command.dry_run_execution_skeleton_ref,
        ...
        preparer_role_allowed=preparer_role_allowed,
        dry_run_execution_skeleton_requested=command.dry_run_execution_skeleton_requested,
        dry_run_execution_skeleton_prepared=skeleton_prepared,
        dry_run_execution_skeleton_steps=skeleton_steps,
        required_dry_run_execution_skeleton_evidence=required_evidence,
        blocking_reasons=blocking_reasons,
        summary=LmsPackageInstallationDryRunExecutionSkeletonSummary(
            dry_run_execution_skeleton_step_count=len(skeleton_steps),
            required_dry_run_execution_skeleton_evidence_count=len(required_evidence),
        ...
        next_action=(
            LMS_PACKAGE_INSTALLATION_DRY_RUN_EXECUTION_SKELETON_READY_NEXT_ACTION
            if skeleton_prepared
            else LMS_PACKAGE_INSTALLATION_DRY_RUN_EXECUTION_SKELETON_RETRY_NEXT_ACTION
        ),
    )
    return draft.model_copy(
        update={"evidence_hash": build_lms_package_installation_dry_run_execution_skeleton_hash(draft)}
    )


def build_lms_package_installation_dry_run_execution_skeleton_command_hash(
    command: LmsPackageInstallationDryRunExecutionSkeletonCommand,
) -> str:
    payload = command.model_dump(mode="json", exclude={"dry_run_execution_skeleton_preparation_statement"})
    payload["dry_run_execution_skeleton_preparation_statement_hash"] = stable_hash(
        command.dry_run_execution_skeleton_preparation_statement
    )
    return stable_hash(canonical_json(payload))


def build_lms_package_installation_dry_run_execution_skeleton_hash(
    response: LmsPackageInstallationDryRunExecutionSkeletonResponse,
) -> str:
    return stable_hash(canonical_json(response.model_dump(mode="json", exclude={"evidence_hash"})))


def _dry_run_execution_skeleton_blocking_reasons(
    *,
    command: LmsPackageInstallationDryRunExecutionSkeletonCommand,
    package_installation_ready: bool,
    ...
    if not command.dry_run_execution_skeleton_requested:
        reasons.append("dry_run_execution_skeleton_not_requested")
