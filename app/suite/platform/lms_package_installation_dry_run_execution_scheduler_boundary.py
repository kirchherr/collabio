...
LMS_PACKAGE_INSTALLATION_DRY_RUN_EXECUTION_SCHEDULER_BOUNDARY_SCHEMA_VERSION = (
    "lms_package_installation_dry_run_execution_scheduler_boundary.v1"
)
LMS_PACKAGE_INSTALLATION_DRY_RUN_EXECUTION_SCHEDULER_BOUNDARY_RESULT_CONTRACT = (
    "metadata_only_lms_package_installation_dry_run_execution_scheduler_boundary_no_activation"
)
...
LMS_PACKAGE_INSTALLATION_DRY_RUN_EXECUTION_SCHEDULER_BOUNDARY_STATEMENT = (
    "I prepare the LMS package installation dry-run execution scheduler boundary without activating schedulers, "
...
LMS_PACKAGE_INSTALLATION_DRY_RUN_EXECUTION_SCHEDULER_BOUNDARY_READY_NEXT_ACTION = (
    "prepare_lms_dry_run_execution_worker_image_boundary_without_resolution"
)
LMS_PACKAGE_INSTALLATION_DRY_RUN_EXECUTION_SCHEDULER_BOUNDARY_RETRY_NEXT_ACTION = (
...
class LmsPackageInstallationDryRunExecutionSchedulerBoundaryCommand(BaseModel):
    ...
    dry_run_execution_scheduler_boundary_ref: str
    dry_run_execution_plan_ref: str
    execution_window_ref: str
    resource_budget_ref: str
    scheduler_policy_ref: str
    scheduler_activation_boundary_ref: str
    idempotency_key_ref: str
    change_request_ref: str
    prepared_at_utc: datetime
    audit_chain_ref: str
    dry_run_execution_scheduler_boundary_statement: str
    dry_run_execution_scheduler_boundary_requested: bool = True
    ...
    def require_sha256(cls, value: str) -> str:
        if not SHA256_PATTERN.fullmatch(value):
            raise ValueError("LMS dry-run execution scheduler boundary hashes must be sha256 references")
        return value

    @field_validator(
        "dry_run_execution_scheduler_boundary_ref",
        "dry_run_execution_plan_ref",
        "execution_window_ref",
        "resource_budget_ref",
        "scheduler_policy_ref",
        "scheduler_activation_boundary_ref",
        "idempotency_key_ref",
        "change_request_ref",
        "audit_chain_ref",
    )
    ...
    def require_ref(cls, value: str) -> str:
        if not REF_PATTERN.fullmatch(value.strip()):
            raise ValueError("LMS dry-run execution scheduler boundary references must use a typed ref prefix")
        return value.strip()

    @field_validator("dry_run_execution_scheduler_boundary_statement")
    @classmethod
    def require_exact_statement(cls, value: str) -> str:
        normalized = value.strip()
        if normalized != LMS_PACKAGE_INSTALLATION_DRY_RUN_EXECUTION_SCHEDULER_BOUNDARY_STATEMENT:
            raise ValueError("LMS dry-run execution scheduler boundary requires the exact preparation statement")
        return normalized
    ...
    def require_timezone_aware_timestamp(cls, value: datetime) -> datetime:
        if value.tzinfo is None or value.utcoffset() is None:
            raise ValueError("LMS dry-run execution scheduler boundary prepared_at_utc must include a timezone")
        return value


class LmsPackageInstallationDryRunExecutionSchedulerBoundarySummary(BaseModel):
    model_config = ConfigDict(extra="forbid")

    dry_run_execution_scheduler_boundary_step_count: int
    required_dry_run_execution_scheduler_boundary_evidence_count: int
    blocking_reason_count: int


class LmsPackageInstallationDryRunExecutionSchedulerBoundaryResponse(BaseModel):
    model_config = ConfigDict(extra="forbid")

    schema_version: str = LMS_PACKAGE_INSTALLATION_DRY_RUN_EXECUTION_SCHEDULER_BOUNDARY_SCHEMA_VERSION
    tenant_id: str
    module_id: str = LMS_MODULE_ID
    endpoint: str = LMS_PACKAGE_INSTALLATION_DRY_RUN_EXECUTION_SCHEDULER_BOUNDARY_ENDPOINT
    result_contract: str = LMS_PACKAGE_INSTALLATION_DRY_RUN_EXECUTION_SCHEDULER_BOUNDARY_RESULT_CONTRACT
    ...
    dry_run_execution_plan_review_endpoint: str = LMS_PACKAGE_INSTALLATION_DRY_RUN_EXECUTION_PLAN_REVIEW_ENDPOINT
    dry_run_execution_plan_review_evidence_hash: str
    dry_run_execution_plan_evidence_hash: str
    dry_run_execution_runbook_evidence_hash: str
    dry_run_execution_admission_gate_evidence_hash: str
    ...
    dry_run_execution_scheduler_boundary_statement_hash: str
    dry_run_execution_scheduler_boundary_ref: str
    dry_run_execution_plan_ref: str
    execution_window_ref: str
    resource_budget_ref: str
    ...
    dry_run_execution_scheduler_boundary_requested: bool
    dry_run_execution_scheduler_boundary_ready: bool
    explicit_human_execution_approval_present: bool
    approval_record_tenant_match: bool
    approval_record_hash_match: bool
    future_worker_image_boundary_required: bool = True
    ...
    dry_run_execution_scheduler_boundary_steps: tuple[str, ...]
    required_dry_run_execution_scheduler_boundary_evidence: tuple[str, ...]
    blocking_reasons: tuple[str, ...]
    summary: LmsPackageInstallationDryRunExecutionSchedulerBoundarySummary
    ...
    @field_validator(
        ...
        "result_contract",
        "continuity_domain",
        "dry_run_execution_plan_review_endpoint",
        "dry_run_execution_scheduler_boundary_ref",
        "dry_run_execution_plan_ref",
        "execution_window_ref",
        "resource_budget_ref",
    ...
    def require_non_empty_text(cls, value: str) -> str:
        if not value.strip():
            raise ValueError("LMS dry-run execution scheduler boundary text fields must not be empty")
        return value
    ...
    @field_validator(
        ...
        "dry_run_execution_scheduler_boundary_statement_hash",
        "evidence_hash",
    )
    @classmethod
    ...
    @field_validator(
        "dry_run_execution_scheduler_boundary_steps",
        "required_dry_run_execution_scheduler_boundary_evidence",
        "blocking_reasons",
        "evidence_refs",
    )
    @classmethod
    def require_unique_lists(cls, value: tuple[str, ...]) -> tuple[str, ...]:
        if len(set(value)) != len(value):
            raise ValueError("LMS dry-run execution scheduler boundary lists must not contain duplicates")
        for item in value:
            if not item.strip():
                raise ValueError("LMS dry-run execution scheduler boundary list items must not be empty")
        return value

    @model_validator(mode="after")
    def require_non_activating_scheduler_boundary(
        self,
    ) -> LmsPackageInstallationDryRunExecutionSchedulerBoundaryResponse:
        if self.schema_version != LMS_PACKAGE_INSTALLATION_DRY_RUN_EXECUTION_SCHEDULER_BOUNDARY_SCHEMA_VERSION:
            raise ValueError("LMS dry-run execution scheduler boundary schema version is invalid")
        if self.endpoint != LMS_PACKAGE_INSTALLATION_DRY_RUN_EXECUTION_SCHEDULER_BOUNDARY_ENDPOINT:
            raise ValueError("LMS dry-run execution scheduler boundary endpoint is invalid")
        if self.result_contract != LMS_PACKAGE_INSTALLATION_DRY_RUN_EXECUTION_SCHEDULER_BOUNDARY_RESULT_CONTRACT:
            raise ValueError("LMS dry-run execution scheduler boundary result contract is invalid")
        if self.module_id != LMS_MODULE_ID:
            raise ValueError("LMS dry-run execution scheduler boundary only applies to lms")
        if self.continuity_domain != LMS_CONTINUITY_DOMAIN:
            raise ValueError("LMS dry-run execution scheduler boundary continuity domain is invalid")
        if self.explicit_human_execution_approval_present != (
            self.approval_record_tenant_match and self.approval_record_hash_match
        ):
            raise ValueError("LMS dry-run execution scheduler boundary approval flags must match stored record")
        expected_ready = (
            self.package_installation_ready
            and self.preparer_role_allowed
            and self.dry_run_execution_scheduler_boundary_requested
            and self.explicit_human_execution_approval_present
            and not self.blocking_reasons
        )
        if self.dry_run_execution_scheduler_boundary_ready != expected_ready:
            raise ValueError("LMS dry-run execution scheduler boundary readiness must match prerequisites")
        if not self.future_worker_image_boundary_required:
            raise ValueError("LMS dry-run execution scheduler boundary must require a future worker image boundary")
        if (
            ...
            raise ValueError("LMS dry-run execution scheduler boundary must remain metadata-only and non-activating")
        if self.summary.dry_run_execution_scheduler_boundary_step_count != len(
            self.dry_run_execution_scheduler_boundary_steps
        ):
            raise ValueError("LMS dry-run execution scheduler boundary step count must match boundary steps")
        if self.summary.required_dry_run_execution_scheduler_boundary_evidence_count != len(
            self.required_dry_run_execution_scheduler_boundary_evidence
        ):
            raise ValueError("LMS dry-run execution scheduler boundary evidence count must match evidence list")
        if self.summary.blocking_reason_count != len(self.blocking_reasons):
            raise ValueError("LMS dry-run execution scheduler boundary blocking count must match blocking reasons")
        return self


def build_lms_package_installation_dry_run_execution_scheduler_boundary_response(
    *,
    command: LmsPackageInstallationDryRunExecutionSchedulerBoundaryCommand,
    ...
    command_hash = build_lms_package_installation_dry_run_execution_scheduler_boundary_command_hash(command)
    idempotency_key_hash = stable_hash(
        canonical_json(
            {
                "schema_version": "lms_package_installation_dry_run_execution_scheduler_boundary_idempotency_key.v1",
                "tenant_id": user_context.tenant_id,
                ...
                "scheduler_activation_boundary_ref": command.scheduler_activation_boundary_ref,
                "idempotency_key_ref": command.idempotency_key_ref,
            }
        )
    )
    statement_hash = stable_hash(command.dry_run_execution_scheduler_boundary_statement)
    preparer_role_allowed = bool({"tenant-admin", "tenant_admin"} & user_context.role_ids)
    blocking_reasons = _dry_run_execution_scheduler_boundary_blocking_reasons(
        command=command,
    ...
    scheduler_boundary_ready = not blocking_reasons
    boundary_steps = (
        ...
        "bind_scheduler_activation_boundary_ref_as_future_gate",
        "define_scheduler_boundary_idempotency_and_hash_closure",
        "define_scheduler_boundary_as_non_activating_artifact",
        "require_future_worker_image_boundary_before_resolution",
        "confirm_scheduler_not_activated",
        "confirm_scheduler_job_not_created",
        "confirm_worker_image_resolution_disabled",
    ...
    required_evidence = (
        ...
        "exact_dry_run_execution_scheduler_boundary_statement_hash",
        "execution_window_ref",
        "resource_budget_ref",
        "scheduler_policy_ref",
        "scheduler_activation_boundary_ref",
        ...
        "idempotency_key_hash",
        "audit_chain_ref",
        "dry_run_execution_scheduler_boundary_schema_version",
        "future_worker_image_boundary_required",
    ...
    draft = LmsPackageInstallationDryRunExecutionSchedulerBoundaryResponse(
        tenant_id=user_context.tenant_id,
        package_installation_ready=readiness.package_installation_ready,
        ...
        dry_run_execution_scheduler_boundary_statement_hash=statement_hash,
        dry_run_execution_scheduler_boundary_ref=command.dry_run_execution_scheduler_boundary_ref,
        ...
        preparer_role_allowed=preparer_role_allowed,
        dry_run_execution_scheduler_boundary_requested=command.dry_run_execution_scheduler_boundary_requested,
        dry_run_execution_scheduler_boundary_ready=scheduler_boundary_ready,
        ...
        approval_record_hash_match=approval_record_hash_match,
        dry_run_execution_scheduler_boundary_steps=boundary_steps,
        required_dry_run_execution_scheduler_boundary_evidence=required_evidence,
        blocking_reasons=blocking_reasons,
        summary=LmsPackageInstallationDryRunExecutionSchedulerBoundarySummary(
            dry_run_execution_scheduler_boundary_step_count=len(boundary_steps),
            required_dry_run_execution_scheduler_boundary_evidence_count=len(required_evidence),
        ...
        next_action=(
            LMS_PACKAGE_INSTALLATION_DRY_RUN_EXECUTION_SCHEDULER_BOUNDARY_READY_NEXT_ACTION
            if scheduler_boundary_ready
            else LMS_PACKAGE_INSTALLATION_DRY_RUN_EXECUTION_SCHEDULER_BOUNDARY_RETRY_NEXT_ACTION
        ),
    )
    return draft.model_copy(
        update={"evidence_hash": build_lms_package_installation_dry_run_execution_scheduler_boundary_hash(draft)}
    )


def build_lms_package_installation_dry_run_execution_scheduler_boundary_command_hash(
    command: LmsPackageInstallationDryRunExecutionSchedulerBoundaryCommand,
) -> str:
    payload = command.model_dump(mode="json", exclude={"dry_run_execution_scheduler_boundary_statement"})
    payload["dry_run_execution_scheduler_boundary_statement_hash"] = stable_hash(
        command.dry_run_execution_scheduler_boundary_statement
    )
    return stable_hash(canonical_json(payload))


def build_lms_package_installation_dry_run_execution_scheduler_boundary_hash(
    response: LmsPackageInstallationDryRunExecutionSchedulerBoundaryResponse,
) -> str:
    return stable_hash(canonical_json(response.model_dump(mode="json", exclude={"evidence_hash"})))


def _dry_run_execution_scheduler_boundary_blocking_reasons(
    *,
    command: LmsPackageInstallationDryRunExecutionSchedulerBoundaryCommand,
    package_installation_ready: bool,
    ...
    if not command.dry_run_execution_scheduler_boundary_requested:
        reasons.append("dry_run_execution_scheduler_boundary_not_requested")
