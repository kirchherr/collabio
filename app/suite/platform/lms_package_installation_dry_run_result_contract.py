...
LMS_PACKAGE_INSTALLATION_DRY_RUN_RESULT_CONTRACT_SCHEMA_VERSION = "lms_package_installation_dry_run_result_contract.v1"
LMS_PACKAGE_INSTALLATION_DRY_RUN_RESULT_CONTRACT_RESULT_CONTRACT = (
    "metadata_only_lms_package_installation_dry_run_result_contract_no_execution"
)
LMS_PACKAGE_INSTALLATION_DRY_RUN_RESULT_CONTRACT_ENDPOINT = (
...
LMS_PACKAGE_INSTALLATION_DRY_RUN_RESULT_CONTRACT_STATEMENT = (
    "I prepare the LMS package installation dry-run result contract without executing dry-run or tenant activation."
)
LMS_PACKAGE_INSTALLATION_DRY_RUN_RESULT_CONTRACT_READY_NEXT_ACTION = (
    "prepare_lms_package_installation_dry_run_execution_gate_without_execution"
)
LMS_PACKAGE_INSTALLATION_DRY_RUN_RESULT_CONTRACT_RETRY_NEXT_ACTION = (
...
class LmsPackageInstallationDryRunResultContractCommand(BaseModel):
    ...
    dry_run_result_contract_ref: str
    change_request_ref: str
    idempotency_key_ref: str
    prepared_at_utc: datetime
    audit_chain_ref: str
    dry_run_result_contract_statement: str
    dry_run_result_contract_requested: bool = True
    ...
    def require_sha256(cls, value: str) -> str:
        if not SHA256_PATTERN.fullmatch(value):
            raise ValueError("LMS dry-run result contract hashes must be sha256 references")
        return value

    @field_validator("dry_run_result_contract_ref", "change_request_ref", "idempotency_key_ref", "audit_chain_ref")
    @classmethod
    def require_ref(cls, value: str) -> str:
        if not REF_PATTERN.fullmatch(value.strip()):
            raise ValueError("LMS dry-run result contract references must use a typed ref prefix")
        return value.strip()

    @field_validator("dry_run_result_contract_statement")
    @classmethod
    def require_exact_statement(cls, value: str) -> str:
        normalized = value.strip()
        if normalized != LMS_PACKAGE_INSTALLATION_DRY_RUN_RESULT_CONTRACT_STATEMENT:
            raise ValueError("LMS dry-run result contract requires the exact preparation statement")
        return normalized
    ...
    def require_timezone_aware_timestamp(cls, value: datetime) -> datetime:
        if value.tzinfo is None or value.utcoffset() is None:
            raise ValueError("LMS dry-run result contract prepared_at_utc must include a timezone")
        return value


class LmsPackageInstallationDryRunResultContractSummary(BaseModel):
    model_config = ConfigDict(extra="forbid")

    dry_run_result_contract_step_count: int
    required_dry_run_result_contract_evidence_count: int
    blocking_reason_count: int


class LmsPackageInstallationDryRunResultContractResponse(BaseModel):
    model_config = ConfigDict(extra="forbid")

    schema_version: str = LMS_PACKAGE_INSTALLATION_DRY_RUN_RESULT_CONTRACT_SCHEMA_VERSION
    tenant_id: str
    module_id: str = LMS_MODULE_ID
    endpoint: str = LMS_PACKAGE_INSTALLATION_DRY_RUN_RESULT_CONTRACT_ENDPOINT
    result_contract: str = LMS_PACKAGE_INSTALLATION_DRY_RUN_RESULT_CONTRACT_RESULT_CONTRACT
    ...
    dry_run_result_contract_statement_hash: str
    dry_run_result_contract_ref: str
    change_request_ref: str
    audit_chain_ref: str
    prepared_by: str
    ...
    dry_run_result_contract_requested: bool
    dry_run_result_contract_ready: bool
    future_dry_run_execution_gate_required: bool = True
    ...
    dry_run_result_contract_steps: tuple[str, ...]
    required_dry_run_result_contract_evidence: tuple[str, ...]
    blocking_reasons: tuple[str, ...]
    summary: LmsPackageInstallationDryRunResultContractSummary
    ...
    def require_non_empty_text(cls, value: str) -> str:
        if not value.strip():
            raise ValueError("LMS dry-run result contract text fields must not be empty")
        return value
    ...
    @field_validator(
        ...
        "dry_run_result_contract_statement_hash",
        "evidence_hash",
    )
    @classmethod
    ...
    @field_validator(
        "dry_run_result_contract_steps",
        "required_dry_run_result_contract_evidence",
        "blocking_reasons",
        "evidence_refs",
    )
    @classmethod
    def require_unique_lists(cls, value: tuple[str, ...]) -> tuple[str, ...]:
        if len(set(value)) != len(value):
            raise ValueError("LMS dry-run result contract lists must not contain duplicates")
        for item in value:
            if not item.strip():
                raise ValueError("LMS dry-run result contract list items must not be empty")
        return value

    @model_validator(mode="after")
    def require_non_executing_skeleton_contract(
        self,
    ) -> LmsPackageInstallationDryRunResultContractResponse:
        if self.schema_version != LMS_PACKAGE_INSTALLATION_DRY_RUN_RESULT_CONTRACT_SCHEMA_VERSION:
            raise ValueError("LMS dry-run result contract schema version is invalid")
        if self.endpoint != LMS_PACKAGE_INSTALLATION_DRY_RUN_RESULT_CONTRACT_ENDPOINT:
            raise ValueError("LMS dry-run result contract endpoint is invalid")
        if self.result_contract != LMS_PACKAGE_INSTALLATION_DRY_RUN_RESULT_CONTRACT_RESULT_CONTRACT:
            raise ValueError("LMS dry-run result contract result contract is invalid")
        if self.module_id != LMS_MODULE_ID:
            raise ValueError("LMS dry-run result contract only applies to lms")
        if self.continuity_domain != LMS_CONTINUITY_DOMAIN:
            raise ValueError("LMS dry-run result contract continuity domain is invalid")
        expected_ready = (
            self.package_installation_ready
            and self.preparer_role_allowed
            and self.dry_run_result_contract_requested
            and not self.blocking_reasons
        )
        if self.dry_run_result_contract_ready != expected_ready:
            raise ValueError("LMS dry-run result contract readiness must match prerequisites")
        if not self.future_dry_run_execution_gate_required:
            raise ValueError("LMS dry-run result contract must require a future dry-run execution gate")
        if (
            ...
            raise ValueError("LMS dry-run result contract must remain metadata-only and non-executing")
        if self.summary.dry_run_result_contract_step_count != len(self.dry_run_result_contract_steps):
            raise ValueError("LMS dry-run result contract step count must match skeleton steps")
        if self.summary.required_dry_run_result_contract_evidence_count != len(
            self.required_dry_run_result_contract_evidence
        ):
            raise ValueError("LMS dry-run result contract evidence count must match evidence list")
        if self.summary.blocking_reason_count != len(self.blocking_reasons):
            raise ValueError("LMS dry-run result contract blocking count must match blocking reasons")
        return self


def build_lms_package_installation_dry_run_result_contract_response(
    *,
    command: LmsPackageInstallationDryRunResultContractCommand,
    ...
    command_hash = build_lms_package_installation_dry_run_result_contract_command_hash(command)
    idempotency_key_hash = stable_hash(
        canonical_json(
            {
                "schema_version": "lms_package_installation_dry_run_result_contract_idempotency_key.v1",
    ...
    statement_hash = stable_hash(command.dry_run_result_contract_statement)
    preparer_role_allowed = bool({"tenant-admin", "tenant_admin"} & user_context.role_ids)
    blocking_reasons = _dry_run_result_contract_blocking_reasons(
        command=command,
    ...
    contract_fields = (
        ...
        "define_dry_run_result_receipt_schema_version",
        "define_no_write_check_status_fields",
        "define_blocking_reason_and_warning_fields",
        "define_restore_and_approval_hash_fields",
        "define_no_persistence_and_no_tenant_state_flags",
    ...
    required_evidence = (
        ...
        "exact_dry_run_result_contract_statement_hash",
        "change_request_ref",
        "idempotency_key_hash",
        "audit_chain_ref",
        "future_dry_run_execution_gate_required",
        "dry_run_result_receipt_schema_version",
        "dry_run_result_no_write_fields",
        "dry_run_result_blocking_reason_fields",
        "dry_run_result_restore_and_approval_hash_fields",
        "no_lms_dry_run_execution_confirmation",
        "no_dry_run_result_persistence_confirmation",
    )
    draft = LmsPackageInstallationDryRunResultContractResponse(
        tenant_id=user_context.tenant_id,
        package_installation_ready=readiness.package_installation_ready,
        ...
        dry_run_result_contract_statement_hash=statement_hash,
        dry_run_result_contract_ref=command.dry_run_result_contract_ref,
        ...
        preparer_role_allowed=preparer_role_allowed,
        dry_run_result_contract_requested=command.dry_run_result_contract_requested,
        dry_run_result_contract_ready=implementation_ready,
        dry_run_result_contract_steps=contract_fields,
        required_dry_run_result_contract_evidence=required_evidence,
        blocking_reasons=blocking_reasons,
        summary=LmsPackageInstallationDryRunResultContractSummary(
            dry_run_result_contract_step_count=len(contract_fields),
            required_dry_run_result_contract_evidence_count=len(required_evidence),
        ...
        next_action=(
            ...
            else LMS_PACKAGE_INSTALLATION_DRY_RUN_RESULT_CONTRACT_RETRY_NEXT_ACTION
        ),
    )
    return draft.model_copy(
        update={"evidence_hash": build_lms_package_installation_dry_run_result_contract_hash(draft)}
    )


def build_lms_package_installation_dry_run_result_contract_command_hash(
    command: LmsPackageInstallationDryRunResultContractCommand,
) -> str:
    payload = command.model_dump(mode="json", exclude={"dry_run_result_contract_statement"})
    payload["dry_run_result_contract_statement_hash"] = stable_hash(command.dry_run_result_contract_statement)
    return stable_hash(canonical_json(payload))


def build_lms_package_installation_dry_run_result_contract_hash(
    response: LmsPackageInstallationDryRunResultContractResponse,
) -> str:
    return stable_hash(canonical_json(response.model_dump(mode="json", exclude={"evidence_hash"})))


def _dry_run_result_contract_blocking_reasons(
    *,
    command: LmsPackageInstallationDryRunResultContractCommand,
    package_installation_ready: bool,
    ...
    if not command.dry_run_result_contract_requested:
        reasons.append("dry_run_result_contract_not_requested")
