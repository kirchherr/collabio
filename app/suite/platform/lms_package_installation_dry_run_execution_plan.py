...
LMS_PACKAGE_INSTALLATION_DRY_RUN_EXECUTION_PLAN_SCHEMA_VERSION = "lms_package_installation_dry_run_execution_plan.v1"
LMS_PACKAGE_INSTALLATION_DRY_RUN_EXECUTION_PLAN_RESULT_CONTRACT = (
    "metadata_only_lms_package_installation_dry_run_execution_plan_no_execution"
)
...
LMS_PACKAGE_INSTALLATION_DRY_RUN_EXECUTION_PLAN_STATEMENT = (
    "I prepare the LMS package installation dry-run execution plan without activating schedulers, "
...
LMS_PACKAGE_INSTALLATION_DRY_RUN_EXECUTION_PLAN_READY_NEXT_ACTION = (
    "prepare_lms_dry_run_execution_plan_review_without_execution"
)
LMS_PACKAGE_INSTALLATION_DRY_RUN_EXECUTION_PLAN_RETRY_NEXT_ACTION = (
...
class LmsPackageInstallationDryRunExecutionPlanCommand(BaseModel):
    ...
    audit_chain_ref: str
    dry_run_execution_plan_statement: str
    dry_run_execution_plan_requested: bool = True
    ...
    def require_sha256(cls, value: str) -> str:
        if not SHA256_PATTERN.fullmatch(value):
            raise ValueError("LMS dry-run execution plan hashes must be sha256 references")
        return value
    ...
    def require_ref(cls, value: str) -> str:
        if not REF_PATTERN.fullmatch(value.strip()):
            raise ValueError("LMS dry-run execution plan references must use a typed ref prefix")
        return value.strip()

    @field_validator("dry_run_execution_plan_statement")
    @classmethod
    def require_exact_statement(cls, value: str) -> str:
        normalized = value.strip()
        if normalized != LMS_PACKAGE_INSTALLATION_DRY_RUN_EXECUTION_PLAN_STATEMENT:
            raise ValueError("LMS dry-run execution plan requires the exact preparation statement")
        return normalized
    ...
    def require_timezone_aware_timestamp(cls, value: datetime) -> datetime:
        if value.tzinfo is None or value.utcoffset() is None:
            raise ValueError("LMS dry-run execution plan prepared_at_utc must include a timezone")
        return value


class LmsPackageInstallationDryRunExecutionPlanSummary(BaseModel):
    model_config = ConfigDict(extra="forbid")

    dry_run_execution_plan_step_count: int
    required_dry_run_execution_plan_evidence_count: int
    blocking_reason_count: int


class LmsPackageInstallationDryRunExecutionPlanResponse(BaseModel):
    model_config = ConfigDict(extra="forbid")

    schema_version: str = LMS_PACKAGE_INSTALLATION_DRY_RUN_EXECUTION_PLAN_SCHEMA_VERSION
    tenant_id: str
    module_id: str = LMS_MODULE_ID
    endpoint: str = LMS_PACKAGE_INSTALLATION_DRY_RUN_EXECUTION_PLAN_ENDPOINT
    result_contract: str = LMS_PACKAGE_INSTALLATION_DRY_RUN_EXECUTION_PLAN_RESULT_CONTRACT
    ...
    dry_run_execution_runbook_endpoint: str = LMS_PACKAGE_INSTALLATION_DRY_RUN_EXECUTION_RUNBOOK_ENDPOINT
    dry_run_execution_runbook_evidence_hash: str
    dry_run_execution_admission_gate_evidence_hash: str
    dry_run_execution_approval_boundary_evidence_hash: str
    dry_run_execution_approval_record_hash: str
    ...
    dry_run_execution_plan_statement_hash: str
    dry_run_execution_plan_ref: str
    backup_restore_runbook_ref: str
    rollback_runbook_ref: str
    operator_handoff_ref: str
    ...
    dry_run_execution_plan_requested: bool
    dry_run_execution_plan_ready: bool
    explicit_human_execution_approval_present: bool
    approval_record_tenant_match: bool
    approval_record_hash_match: bool
    future_dry_run_execution_plan_review_required: bool = True
    ...
    dry_run_execution_plan_steps: tuple[str, ...]
    required_dry_run_execution_plan_evidence: tuple[str, ...]
    blocking_reasons: tuple[str, ...]
    summary: LmsPackageInstallationDryRunExecutionPlanSummary
    ...
    @field_validator(
        ...
        "result_contract",
        "continuity_domain",
        "dry_run_execution_runbook_endpoint",
        "dry_run_execution_plan_ref",
        "backup_restore_runbook_ref",
        "rollback_runbook_ref",
        "operator_handoff_ref",
    ...
    def require_non_empty_text(cls, value: str) -> str:
        if not value.strip():
            raise ValueError("LMS dry-run execution plan text fields must not be empty")
        return value
    ...
    @field_validator(
        ...
        "dry_run_execution_plan_statement_hash",
        "evidence_hash",
    )
    @classmethod
    ...
    @field_validator(
        "dry_run_execution_plan_steps",
        "required_dry_run_execution_plan_evidence",
        "blocking_reasons",
        "evidence_refs",
    )
    @classmethod
    def require_unique_lists(cls, value: tuple[str, ...]) -> tuple[str, ...]:
        if len(set(value)) != len(value):
            raise ValueError("LMS dry-run execution plan lists must not contain duplicates")
        for item in value:
            if not item.strip():
                raise ValueError("LMS dry-run execution plan list items must not be empty")
        return value

    @model_validator(mode="after")
    def require_non_executing_plan(self) -> LmsPackageInstallationDryRunExecutionPlanResponse:
        if self.schema_version != LMS_PACKAGE_INSTALLATION_DRY_RUN_EXECUTION_PLAN_SCHEMA_VERSION:
            raise ValueError("LMS dry-run execution plan schema version is invalid")
        if self.endpoint != LMS_PACKAGE_INSTALLATION_DRY_RUN_EXECUTION_PLAN_ENDPOINT:
            raise ValueError("LMS dry-run execution plan endpoint is invalid")
        if self.result_contract != LMS_PACKAGE_INSTALLATION_DRY_RUN_EXECUTION_PLAN_RESULT_CONTRACT:
            raise ValueError("LMS dry-run execution plan result contract is invalid")
        if self.module_id != LMS_MODULE_ID:
            raise ValueError("LMS dry-run execution plan only applies to lms")
        if self.continuity_domain != LMS_CONTINUITY_DOMAIN:
            raise ValueError("LMS dry-run execution plan continuity domain is invalid")
        if self.explicit_human_execution_approval_present != (
            self.approval_record_tenant_match and self.approval_record_hash_match
        ):
            raise ValueError("LMS dry-run execution plan approval flags must match stored approval record")
        expected_ready = (
            self.package_installation_ready
            and self.preparer_role_allowed
            and self.dry_run_execution_plan_requested
            and self.explicit_human_execution_approval_present
            and not self.blocking_reasons
        )
        if self.dry_run_execution_plan_ready != expected_ready:
            raise ValueError("LMS dry-run execution plan readiness must match prerequisites")
        if not self.future_dry_run_execution_plan_review_required:
            raise ValueError("LMS dry-run execution plan must require a future plan review")
        if (
            ...
            raise ValueError("LMS dry-run execution plan must remain metadata-only and non-executing")
        if self.summary.dry_run_execution_plan_step_count != len(self.dry_run_execution_plan_steps):
            raise ValueError("LMS dry-run execution plan step count must match plan steps")
        if self.summary.required_dry_run_execution_plan_evidence_count != len(
            self.required_dry_run_execution_plan_evidence
        ):
            raise ValueError("LMS dry-run execution plan evidence count must match evidence list")
        if self.summary.blocking_reason_count != len(self.blocking_reasons):
            raise ValueError("LMS dry-run execution plan blocking count must match blocking reasons")
        return self


def build_lms_package_installation_dry_run_execution_plan_response(
    *,
    command: LmsPackageInstallationDryRunExecutionPlanCommand,
...
) -> LmsPackageInstallationDryRunExecutionPlanResponse:
    ...
    command_hash = build_lms_package_installation_dry_run_execution_plan_command_hash(command)
    idempotency_key_hash = stable_hash(
        canonical_json(
            {
                "schema_version": "lms_package_installation_dry_run_execution_plan_idempotency_key.v1",
                "tenant_id": user_context.tenant_id,
    ...
    statement_hash = stable_hash(command.dry_run_execution_plan_statement)
    preparer_role_allowed = bool({"tenant-admin", "tenant_admin"} & user_context.role_ids)
    blocking_reasons = _dry_run_execution_plan_blocking_reasons(
        command=command,
    ...
    plan_ready = not blocking_reasons
    plan_steps = (
        ...
        "define_execution_plan_idempotency_and_hash_closure",
        "define_execution_plan_as_non_executing_plan_artifact",
        "require_future_dry_run_execution_plan_review_before_scheduler_or_worker_design",
    ...
    required_evidence = (
        ...
        "exact_dry_run_execution_plan_statement_hash",
        "backup_restore_runbook_ref",
        "rollback_runbook_ref",
        "operator_handoff_ref",
        "execution_window_ref",
        ...
        "idempotency_key_hash",
        "audit_chain_ref",
        "dry_run_execution_plan_schema_version",
        "dry_run_execution_runbook_schema_version",
        "future_dry_run_execution_plan_review_required",
    ...
    draft = LmsPackageInstallationDryRunExecutionPlanResponse(
        tenant_id=user_context.tenant_id,
        package_installation_ready=readiness.package_installation_ready,
        ...
        dry_run_execution_plan_statement_hash=statement_hash,
        ...
        preparer_role_allowed=preparer_role_allowed,
        dry_run_execution_plan_requested=command.dry_run_execution_plan_requested,
        dry_run_execution_plan_ready=plan_ready,
        ...
        approval_record_hash_match=approval_record_hash_match,
        dry_run_execution_plan_steps=plan_steps,
        required_dry_run_execution_plan_evidence=required_evidence,
        blocking_reasons=blocking_reasons,
        summary=LmsPackageInstallationDryRunExecutionPlanSummary(
            dry_run_execution_plan_step_count=len(plan_steps),
            required_dry_run_execution_plan_evidence_count=len(required_evidence),
        ...
        next_action=(
            LMS_PACKAGE_INSTALLATION_DRY_RUN_EXECUTION_PLAN_READY_NEXT_ACTION
            if plan_ready
            else LMS_PACKAGE_INSTALLATION_DRY_RUN_EXECUTION_PLAN_RETRY_NEXT_ACTION
        ),
    )
    return draft.model_copy(update={"evidence_hash": build_lms_package_installation_dry_run_execution_plan_hash(draft)})


def build_lms_package_installation_dry_run_execution_plan_command_hash(
    command: LmsPackageInstallationDryRunExecutionPlanCommand,
) -> str:
    payload = command.model_dump(mode="json", exclude={"dry_run_execution_plan_statement"})
    payload["dry_run_execution_plan_statement_hash"] = stable_hash(command.dry_run_execution_plan_statement)
    return stable_hash(canonical_json(payload))


def build_lms_package_installation_dry_run_execution_plan_hash(
    response: LmsPackageInstallationDryRunExecutionPlanResponse,
) -> str:
    return stable_hash(canonical_json(response.model_dump(mode="json", exclude={"evidence_hash"})))


def _dry_run_execution_plan_blocking_reasons(
    *,
    command: LmsPackageInstallationDryRunExecutionPlanCommand,
    package_installation_ready: bool,
    ...
    if not command.dry_run_execution_plan_requested:
        reasons.append("dry_run_execution_plan_not_requested")
