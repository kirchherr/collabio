...
TICKETS_INCIDENTS_ACTIVATION_DRY_RUN_EXECUTOR_IMPLEMENTATION_REVIEW_SCHEMA_VERSION = (
    "tickets_incidents_activation_dry_run_executor_implementation_review.v1"
)
TICKETS_INCIDENTS_ACTIVATION_DRY_RUN_EXECUTOR_IMPLEMENTATION_REVIEW_RESULT_CONTRACT = (
    "metadata_only_tickets_incidents_activation_dry_run_executor_implementation_review_no_execution"
)
TICKETS_INCIDENTS_ACTIVATION_DRY_RUN_EXECUTOR_IMPLEMENTATION_REVIEW_ENDPOINT = (
...
TICKETS_INCIDENTS_ACTIVATION_DRY_RUN_EXECUTOR_IMPLEMENTATION_REVIEW_STATEMENT = (
    "I review the Tickets & Incidents activation dry-run executor implementation without executing "
    "dry-run or tenant activation."
)
TICKETS_INCIDENTS_ACTIVATION_DRY_RUN_EXECUTOR_IMPLEMENTATION_REVIEW_READY_NEXT_ACTION = (
    "prepare_tickets_incidents_activation_dry_run_result_contract_without_execution"
)
TICKETS_INCIDENTS_ACTIVATION_DRY_RUN_EXECUTOR_IMPLEMENTATION_REVIEW_RETRY_NEXT_ACTION = (
...
class TicketsIncidentsActivationDryRunExecutorImplementationReviewCommand(BaseModel):
    ...
    activation_dry_run_executor_implementation_review_ref: str
    change_request_ref: str
    idempotency_key_ref: str
    reviewed_at_utc: datetime
    audit_chain_ref: str
    activation_dry_run_executor_implementation_review_statement: str
    activation_dry_run_executor_implementation_review_requested: bool = True
    ...
    def require_sha256(cls, value: str) -> str:
        if not SHA256_PATTERN.fullmatch(value):
            raise ValueError(
                "Tickets & Incidents activation dry-run executor implementation review hashes must be sha256 references"
            )
        return value
    ...
    def require_ref(cls, value: str) -> str:
        if not REF_PATTERN.fullmatch(value.strip()):
            raise ValueError(
                "Tickets & Incidents activation dry-run executor implementation review references "
                "need typed ref prefixes"
            )
        return value.strip()

    @field_validator("activation_dry_run_executor_implementation_review_statement")
    @classmethod
    def require_exact_review_statement(cls, value: str) -> str:
        normalized = value.strip()
        if normalized != TICKETS_INCIDENTS_ACTIVATION_DRY_RUN_EXECUTOR_IMPLEMENTATION_REVIEW_STATEMENT:
            raise ValueError(
                "Tickets & Incidents activation dry-run executor implementation review requires the exact statement"
            )
        return normalized
    ...
    def require_timezone_aware_timestamp(cls, value: datetime) -> datetime:
        if value.tzinfo is None or value.utcoffset() is None:
            raise ValueError(
                "Tickets & Incidents activation dry-run executor implementation review reviewed_at_utc needs a timezone"
            )
        return value


class TicketsIncidentsActivationDryRunExecutorImplementationReviewSummary(BaseModel):
    model_config = ConfigDict(extra="forbid")

    activation_dry_run_executor_implementation_review_step_count: int
    required_activation_dry_run_executor_implementation_review_evidence_count: int
    blocking_reason_count: int


class TicketsIncidentsActivationDryRunExecutorImplementationReviewResponse(BaseModel):
    model_config = ConfigDict(extra="forbid")

    schema_version: str = TICKETS_INCIDENTS_ACTIVATION_DRY_RUN_EXECUTOR_IMPLEMENTATION_REVIEW_SCHEMA_VERSION
    tenant_id: str
    module_id: str = TICKETS_INCIDENTS_MODULE_ID
    endpoint: str = TICKETS_INCIDENTS_ACTIVATION_DRY_RUN_EXECUTOR_IMPLEMENTATION_REVIEW_ENDPOINT
    result_contract: str = TICKETS_INCIDENTS_ACTIVATION_DRY_RUN_EXECUTOR_IMPLEMENTATION_REVIEW_RESULT_CONTRACT
    ...
    activation_dry_run_executor_implementation_review_statement_hash: str
    activation_dry_run_executor_implementation_review_ref: str
    change_request_ref: str
    audit_chain_ref: str
    reviewed_by: str
    ...
    activation_dry_run_executor_implementation_review_requested: bool
    activation_dry_run_executor_implementation_review_ready: bool
    activation_dry_run_executor_implementation_required: bool = True
    ...
    activation_dry_run_executor_implementation_review_steps: tuple[str, ...]
    required_activation_dry_run_executor_implementation_review_evidence: tuple[str, ...]
    blocking_reasons: tuple[str, ...]
    summary: TicketsIncidentsActivationDryRunExecutorImplementationReviewSummary
    ...
    def require_non_empty_text(cls, value: str) -> str:
        if not value.strip():
            raise ValueError(
                "Tickets & Incidents activation dry-run executor implementation review text fields must not be empty"
            )
        return value
    ...
    @field_validator(
        ...
        "activation_dry_run_executor_implementation_review_statement_hash",
        "evidence_hash",
    )
    @classmethod
    ...
    @field_validator(
        "activation_dry_run_executor_implementation_review_steps",
        "required_activation_dry_run_executor_implementation_review_evidence",
        "blocking_reasons",
        "evidence_refs",
    )
    @classmethod
    def require_unique_lists(cls, value: tuple[str, ...]) -> tuple[str, ...]:
        if len(set(value)) != len(value):
            raise ValueError(
                "Tickets & Incidents activation dry-run executor implementation review lists "
                "must not contain duplicates"
            )
        for item in value:
            if not item.strip():
                raise ValueError(
                    "Tickets & Incidents activation dry-run executor implementation review list items must not be empty"
                )
        return value

    @model_validator(mode="after")
    def require_non_executing_review_contract(
        self,
    ) -> TicketsIncidentsActivationDryRunExecutorImplementationReviewResponse:
        if self.schema_version != TICKETS_INCIDENTS_ACTIVATION_DRY_RUN_EXECUTOR_IMPLEMENTATION_REVIEW_SCHEMA_VERSION:
            raise ValueError(
                "Tickets & Incidents activation dry-run executor implementation review schema version is invalid"
            )
        if self.endpoint != TICKETS_INCIDENTS_ACTIVATION_DRY_RUN_EXECUTOR_IMPLEMENTATION_REVIEW_ENDPOINT:
            raise ValueError(
                "Tickets & Incidents activation dry-run executor implementation review endpoint is invalid"
            )
        if self.result_contract != TICKETS_INCIDENTS_ACTIVATION_DRY_RUN_EXECUTOR_IMPLEMENTATION_REVIEW_RESULT_CONTRACT:
            raise ValueError(
                "Tickets & Incidents activation dry-run executor implementation review result contract is invalid"
            )
        if self.module_id != TICKETS_INCIDENTS_MODULE_ID:
            raise ValueError(
                "Tickets & Incidents activation dry-run executor implementation review only applies to "
                "tickets_incidents"
            )
        if self.continuity_domain != TICKETS_INCIDENTS_CONTINUITY_DOMAIN:
            raise ValueError(
                "Tickets & Incidents activation dry-run executor implementation review continuity domain is invalid"
            )
        expected_ready = (
            self.approval_gate_ready
            and self.human_approval_ready
            and self.reviewer_role_allowed
            and self.activation_dry_run_executor_implementation_review_requested
            and not self.blocking_reasons
        )
        if self.activation_dry_run_executor_implementation_review_ready != expected_ready:
            raise ValueError(
                "Tickets & Incidents activation dry-run executor implementation review readiness is inconsistent"
            )
        if not self.activation_dry_run_executor_implementation_required:
            raise ValueError(
                "Tickets & Incidents activation dry-run executor implementation review must require future executor "
                "implementation"
            )
        if not self.dry_run_result_contract_required:
            raise ValueError(
                "Tickets & Incidents activation dry-run executor implementation review must require result contract"
            )
        if (
            ...
            raise ValueError(
                "Tickets & Incidents activation dry-run executor implementation review must remain metadata-only"
            )
        if self.summary.activation_dry_run_executor_implementation_review_step_count != len(
            self.activation_dry_run_executor_implementation_review_steps
        ):
            raise ValueError(
                "Tickets & Incidents activation dry-run executor implementation review step count mismatch"
            )
        if self.summary.required_activation_dry_run_executor_implementation_review_evidence_count != len(
            self.required_activation_dry_run_executor_implementation_review_evidence
        ):
            raise ValueError(
                "Tickets & Incidents activation dry-run executor implementation review evidence count mismatch"
            )
        if self.summary.blocking_reason_count != len(self.blocking_reasons):
            raise ValueError(
                "Tickets & Incidents activation dry-run executor implementation review blocking count mismatch"
            )
        return self


def build_tickets_incidents_activation_dry_run_executor_implementation_review_response(
    *,
    command: TicketsIncidentsActivationDryRunExecutorImplementationReviewCommand,
    ...
    command_hash = build_tickets_incidents_activation_dry_run_executor_implementation_review_command_hash(command)
    idempotency_key_hash = stable_hash(
        canonical_json(
            {
                "schema_version": (
                    "tickets_incidents_activation_dry_run_executor_implementation_review_idempotency_key.v1"
                ),
                "tenant_id": user_context.tenant_id,
    ...
    review_statement_hash = stable_hash(command.activation_dry_run_executor_implementation_review_statement)
    reviewer_role_allowed = bool({"tenant-admin", "tenant_admin"} & user_context.role_ids)
    blocking_reasons = _activation_dry_run_executor_implementation_review_blocking_reasons(
        command=command,
    ...
    review_ready = not blocking_reasons
    review_steps = (
        ...
        "review_read_only_ticket_metadata_schema_checks",
        "review_idempotent_no_write_dry_run_receipt_shape",
        "defer_dry_run_result_contract",
    ...
    required_evidence = (
        ...
        "tickets_restore_drill_evidence_hash",
        "exact_activation_dry_run_executor_implementation_review_statement_hash",
        "change_request_ref",
        "idempotency_key_hash",
        "audit_chain_ref",
    ...
    draft = TicketsIncidentsActivationDryRunExecutorImplementationReviewResponse(
        tenant_id=user_context.tenant_id,
        approval_gate_ready=approval_gate.approval_gate_ready,
        ...
        activation_dry_run_executor_implementation_review_statement_hash=review_statement_hash,
        activation_dry_run_executor_implementation_review_ref=(
            command.activation_dry_run_executor_implementation_review_ref
        ),
        ...
        reviewer_role_allowed=reviewer_role_allowed,
        activation_dry_run_executor_implementation_review_requested=(
            command.activation_dry_run_executor_implementation_review_requested
        ),
        activation_dry_run_executor_implementation_review_ready=review_ready,
        activation_dry_run_executor_implementation_review_steps=review_steps,
        required_activation_dry_run_executor_implementation_review_evidence=required_evidence,
        blocking_reasons=blocking_reasons,
        summary=TicketsIncidentsActivationDryRunExecutorImplementationReviewSummary(
            activation_dry_run_executor_implementation_review_step_count=len(review_steps),
            required_activation_dry_run_executor_implementation_review_evidence_count=len(required_evidence),
        ...
        next_action=(
            TICKETS_INCIDENTS_ACTIVATION_DRY_RUN_EXECUTOR_IMPLEMENTATION_REVIEW_READY_NEXT_ACTION
            if review_ready
            else TICKETS_INCIDENTS_ACTIVATION_DRY_RUN_EXECUTOR_IMPLEMENTATION_REVIEW_RETRY_NEXT_ACTION
        ),
    )
    return draft.model_copy(
        update={"evidence_hash": build_tickets_incidents_activation_dry_run_executor_implementation_review_hash(draft)}
    )


def build_tickets_incidents_activation_dry_run_executor_implementation_review_command_hash(
    command: TicketsIncidentsActivationDryRunExecutorImplementationReviewCommand,
) -> str:
    payload = command.model_dump(
        mode="json",
        exclude={"activation_dry_run_executor_implementation_review_statement"},
    )
    payload["activation_dry_run_executor_implementation_review_statement_hash"] = stable_hash(
        command.activation_dry_run_executor_implementation_review_statement
    )
    return stable_hash(canonical_json(payload))


def build_tickets_incidents_activation_dry_run_executor_implementation_review_hash(
    response: TicketsIncidentsActivationDryRunExecutorImplementationReviewResponse,
) -> str:
    return stable_hash(canonical_json(response.model_dump(mode="json", exclude={"evidence_hash"})))


def _activation_dry_run_executor_implementation_review_blocking_reasons(
    *,
    command: TicketsIncidentsActivationDryRunExecutorImplementationReviewCommand,
    approval_gate_ready: bool,
    ...
    if not command.activation_dry_run_executor_implementation_review_requested:
        reasons.append("activation_dry_run_executor_implementation_review_not_requested")
