...
LMS_PACKAGE_INSTALLATION_DRY_RUN_EXECUTION_GATE_SCHEMA_VERSION = "lms_package_installation_dry_run_execution_gate.v1"
LMS_PACKAGE_INSTALLATION_DRY_RUN_EXECUTION_GATE_RESULT_CONTRACT = (
    "metadata_only_lms_package_installation_dry_run_execution_gate_no_execution"
)
LMS_PACKAGE_INSTALLATION_DRY_RUN_EXECUTION_GATE_ENDPOINT = (
...
LMS_PACKAGE_INSTALLATION_DRY_RUN_EXECUTION_GATE_STATEMENT = (
    "I prepare the LMS package installation dry-run execution gate without executing dry-run or tenant activation."
)
LMS_PACKAGE_INSTALLATION_DRY_RUN_EXECUTION_GATE_READY_NEXT_ACTION = (
    "prepare_lms_package_installation_dry_run_execution_request_boundary_without_execution"
)
LMS_PACKAGE_INSTALLATION_DRY_RUN_EXECUTION_GATE_RETRY_NEXT_ACTION = (
    "prepare_lms_package_installation_dry_run_execution_gate_without_execution"
)
...
class LmsPackageInstallationDryRunExecutionGateCommand(BaseModel):
    ...
    dry_run_execution_gate_ref: str
    change_request_ref: str
    idempotency_key_ref: str
    prepared_at_utc: datetime
    audit_chain_ref: str
    dry_run_execution_gate_statement: str
    dry_run_execution_gate_requested: bool = True
    ...
    def require_sha256(cls, value: str) -> str:
        if not SHA256_PATTERN.fullmatch(value):
            raise ValueError("LMS dry-run execution gate hashes must be sha256 references")
        return value

    @field_validator("dry_run_execution_gate_ref", "change_request_ref", "idempotency_key_ref", "audit_chain_ref")
    @classmethod
    def require_ref(cls, value: str) -> str:
        if not REF_PATTERN.fullmatch(value.strip()):
            raise ValueError("LMS dry-run execution gate references must use a typed ref prefix")
        return value.strip()

    @field_validator("dry_run_execution_gate_statement")
    @classmethod
    def require_exact_statement(cls, value: str) -> str:
        normalized = value.strip()
        if normalized != LMS_PACKAGE_INSTALLATION_DRY_RUN_EXECUTION_GATE_STATEMENT:
            raise ValueError("LMS dry-run execution gate requires the exact preparation statement")
        return normalized
    ...
    def require_timezone_aware_timestamp(cls, value: datetime) -> datetime:
        if value.tzinfo is None or value.utcoffset() is None:
            raise ValueError("LMS dry-run execution gate prepared_at_utc must include a timezone")
        return value


class LmsPackageInstallationDryRunExecutionGateSummary(BaseModel):
    model_config = ConfigDict(extra="forbid")

    dry_run_execution_gate_step_count: int
    required_dry_run_execution_gate_evidence_count: int
    blocking_reason_count: int


class LmsPackageInstallationDryRunExecutionGateResponse(BaseModel):
    model_config = ConfigDict(extra="forbid")

    schema_version: str = LMS_PACKAGE_INSTALLATION_DRY_RUN_EXECUTION_GATE_SCHEMA_VERSION
    tenant_id: str
    module_id: str = LMS_MODULE_ID
    endpoint: str = LMS_PACKAGE_INSTALLATION_DRY_RUN_EXECUTION_GATE_ENDPOINT
    result_contract: str = LMS_PACKAGE_INSTALLATION_DRY_RUN_EXECUTION_GATE_RESULT_CONTRACT
    ...
    dry_run_execution_gate_statement_hash: str
    dry_run_execution_gate_ref: str
    change_request_ref: str
    audit_chain_ref: str
    prepared_by: str
    ...
    dry_run_execution_gate_requested: bool
    dry_run_execution_gate_ready: bool
    future_dry_run_execution_request_boundary_required: bool = True
    ...
    dry_run_execution_gate_steps: tuple[str, ...]
    required_dry_run_execution_gate_evidence: tuple[str, ...]
    blocking_reasons: tuple[str, ...]
    summary: LmsPackageInstallationDryRunExecutionGateSummary
    ...
    def require_non_empty_text(cls, value: str) -> str:
        if not value.strip():
            raise ValueError("LMS dry-run execution gate text fields must not be empty")
        return value
    ...
    @field_validator(
        ...
        "dry_run_execution_gate_statement_hash",
        "evidence_hash",
    )
    @classmethod
    ...
    @field_validator(
        "dry_run_execution_gate_steps",
        "required_dry_run_execution_gate_evidence",
        "blocking_reasons",
        "evidence_refs",
    )
    @classmethod
    def require_unique_lists(cls, value: tuple[str, ...]) -> tuple[str, ...]:
        if len(set(value)) != len(value):
            raise ValueError("LMS dry-run execution gate lists must not contain duplicates")
        for item in value:
            if not item.strip():
                raise ValueError("LMS dry-run execution gate list items must not be empty")
        return value
    ...
    def require_non_executing_skeleton_contract(
        ...
        if self.schema_version != LMS_PACKAGE_INSTALLATION_DRY_RUN_EXECUTION_GATE_SCHEMA_VERSION:
            raise ValueError("LMS dry-run execution gate schema version is invalid")
        if self.endpoint != LMS_PACKAGE_INSTALLATION_DRY_RUN_EXECUTION_GATE_ENDPOINT:
            raise ValueError("LMS dry-run execution gate endpoint is invalid")
        if self.result_contract != LMS_PACKAGE_INSTALLATION_DRY_RUN_EXECUTION_GATE_RESULT_CONTRACT:
            raise ValueError("LMS dry-run execution gate execution gate is invalid")
        if self.module_id != LMS_MODULE_ID:
            raise ValueError("LMS dry-run execution gate only applies to lms")
        if self.continuity_domain != LMS_CONTINUITY_DOMAIN:
            raise ValueError("LMS dry-run execution gate continuity domain is invalid")
        expected_ready = (
            self.package_installation_ready
            and self.preparer_role_allowed
            and self.dry_run_execution_gate_requested
            and not self.blocking_reasons
        )
        if self.dry_run_execution_gate_ready != expected_ready:
            raise ValueError("LMS dry-run execution gate readiness must match prerequisites")
        if not self.future_dry_run_execution_request_boundary_required:
            raise ValueError("LMS dry-run execution gate must require a future dry-run execution request boundary")
        if (
            ...
            raise ValueError("LMS dry-run execution gate must remain metadata-only and non-executing")
        if self.summary.dry_run_execution_gate_step_count != len(self.dry_run_execution_gate_steps):
            raise ValueError("LMS dry-run execution gate step count must match skeleton steps")
        if self.summary.required_dry_run_execution_gate_evidence_count != len(
            self.required_dry_run_execution_gate_evidence
        ):
            raise ValueError("LMS dry-run execution gate evidence count must match evidence list")
        if self.summary.blocking_reason_count != len(self.blocking_reasons):
            raise ValueError("LMS dry-run execution gate blocking count must match blocking reasons")
        return self


def build_lms_package_installation_dry_run_execution_gate_response(
    *,
    command: LmsPackageInstallationDryRunExecutionGateCommand,
    ...
    command_hash = build_lms_package_installation_dry_run_execution_gate_command_hash(command)
    idempotency_key_hash = stable_hash(
        canonical_json(
            {
                "schema_version": "lms_package_installation_dry_run_execution_gate_idempotency_key.v1",
    ...
    statement_hash = stable_hash(command.dry_run_execution_gate_statement)
    preparer_role_allowed = bool({"tenant-admin", "tenant_admin"} & user_context.role_ids)
    blocking_reasons = _dry_run_execution_gate_blocking_reasons(
        command=command,
    ...
    contract_fields = (
        ...
        "verify_dry_run_result_contract_schema_bound",
        "define_dry_run_execution_request_boundary_ref",
        "define_pre_execution_readiness_checks",
        "define_no_write_execution_gate_flags",
        "define_human_confirmation_required_for_future_execution",
    ...
    required_evidence = (
        ...
        "exact_dry_run_execution_gate_statement_hash",
        "change_request_ref",
        "idempotency_key_hash",
        "audit_chain_ref",
        "future_dry_run_execution_request_boundary_required",
        "dry_run_execution_gate_schema_version",
        "dry_run_execution_precondition_fields",
        "dry_run_execution_no_write_fields",
        "future_human_confirmation_for_dry_run_execution",
        "no_lms_dry_run_execution_confirmation",
        "no_dry_run_result_persistence_confirmation",
    )
    draft = LmsPackageInstallationDryRunExecutionGateResponse(
        tenant_id=user_context.tenant_id,
        package_installation_ready=readiness.package_installation_ready,
        ...
        dry_run_execution_gate_statement_hash=statement_hash,
        dry_run_execution_gate_ref=command.dry_run_execution_gate_ref,
        ...
        preparer_role_allowed=preparer_role_allowed,
        dry_run_execution_gate_requested=command.dry_run_execution_gate_requested,
        dry_run_execution_gate_ready=implementation_ready,
        dry_run_execution_gate_steps=contract_fields,
        required_dry_run_execution_gate_evidence=required_evidence,
        blocking_reasons=blocking_reasons,
        summary=LmsPackageInstallationDryRunExecutionGateSummary(
            dry_run_execution_gate_step_count=len(contract_fields),
            required_dry_run_execution_gate_evidence_count=len(required_evidence),
        ...
        next_action=(
            ...
            else LMS_PACKAGE_INSTALLATION_DRY_RUN_EXECUTION_GATE_RETRY_NEXT_ACTION
        ),
    )
    return draft.model_copy(update={"evidence_hash": build_lms_package_installation_dry_run_execution_gate_hash(draft)})


def build_lms_package_installation_dry_run_execution_gate_command_hash(
    command: LmsPackageInstallationDryRunExecutionGateCommand,
) -> str:
    payload = command.model_dump(mode="json", exclude={"dry_run_execution_gate_statement"})
    payload["dry_run_execution_gate_statement_hash"] = stable_hash(command.dry_run_execution_gate_statement)
    return stable_hash(canonical_json(payload))


def build_lms_package_installation_dry_run_execution_gate_hash(
    response: LmsPackageInstallationDryRunExecutionGateResponse,
) -> str:
    return stable_hash(canonical_json(response.model_dump(mode="json", exclude={"evidence_hash"})))


def _dry_run_execution_gate_blocking_reasons(
    *,
    command: LmsPackageInstallationDryRunExecutionGateCommand,
    package_installation_ready: bool,
    ...
    if not command.dry_run_execution_gate_requested:
        reasons.append("dry_run_execution_gate_not_requested")
