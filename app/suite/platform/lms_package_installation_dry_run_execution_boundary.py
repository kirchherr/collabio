...
LMS_PACKAGE_INSTALLATION_DRY_RUN_EXECUTION_BOUNDARY_SCHEMA_VERSION = (
    "lms_package_installation_dry_run_execution_boundary.v1"
)
LMS_PACKAGE_INSTALLATION_DRY_RUN_EXECUTION_BOUNDARY_RESULT_CONTRACT = (
    "metadata_only_lms_package_installation_dry_run_execution_boundary_no_execution"
)
LMS_PACKAGE_INSTALLATION_DRY_RUN_EXECUTION_BOUNDARY_ENDPOINT = (
...
LMS_PACKAGE_INSTALLATION_DRY_RUN_EXECUTION_BOUNDARY_REVIEW_STATEMENT = (
    "I review the LMS package installation dry-run execution boundary without executing dry-run or tenant activation."
)
LMS_PACKAGE_INSTALLATION_DRY_RUN_EXECUTION_BOUNDARY_READY_NEXT_ACTION = (
    "prepare_lms_package_installation_dry_run_execution_skeleton_without_tenant_activation"
)
LMS_PACKAGE_INSTALLATION_DRY_RUN_EXECUTION_BOUNDARY_RETRY_NEXT_ACTION = (
    "review_lms_package_installation_dry_run_execution_boundary"
)
...
class LmsPackageInstallationDryRunExecutionBoundaryCommand(BaseModel):
    ...
    dry_run_execution_boundary_ref: str
    change_request_ref: str
    idempotency_key_ref: str
    reviewed_at_utc: datetime
    audit_chain_ref: str
    dry_run_execution_boundary_review_statement: str
    dry_run_execution_boundary_review_requested: bool = True
    ...
    def require_sha256(cls, value: str) -> str:
        if not SHA256_PATTERN.fullmatch(value):
            raise ValueError("LMS dry-run execution boundary hashes must be sha256 references")
        return value

    @field_validator(
        "dry_run_execution_boundary_ref",
        "change_request_ref",
        "idempotency_key_ref",
        "audit_chain_ref",
    )
    ...
    def require_ref(cls, value: str) -> str:
        if not REF_PATTERN.fullmatch(value.strip()):
            raise ValueError("LMS dry-run execution boundary references must use a typed ref prefix")
        return value.strip()

    @field_validator("dry_run_execution_boundary_review_statement")
    @classmethod
    def require_exact_review_statement(cls, value: str) -> str:
        normalized = value.strip()
        if normalized != LMS_PACKAGE_INSTALLATION_DRY_RUN_EXECUTION_BOUNDARY_REVIEW_STATEMENT:
            raise ValueError("LMS dry-run execution boundary requires the exact review statement")
        return normalized
    ...
    def require_timezone_aware_timestamp(cls, value: datetime) -> datetime:
        if value.tzinfo is None or value.utcoffset() is None:
            raise ValueError("LMS dry-run execution boundary reviewed_at_utc must include a timezone")
        return value


class LmsPackageInstallationDryRunExecutionBoundarySummary(BaseModel):
    model_config = ConfigDict(extra="forbid")
...
class LmsPackageInstallationDryRunExecutionBoundaryResponse(BaseModel):
    model_config = ConfigDict(extra="forbid")

    schema_version: str = LMS_PACKAGE_INSTALLATION_DRY_RUN_EXECUTION_BOUNDARY_SCHEMA_VERSION
    tenant_id: str
    module_id: str = LMS_MODULE_ID
    endpoint: str = LMS_PACKAGE_INSTALLATION_DRY_RUN_EXECUTION_BOUNDARY_ENDPOINT
    result_contract: str = LMS_PACKAGE_INSTALLATION_DRY_RUN_EXECUTION_BOUNDARY_RESULT_CONTRACT
    ...
    dry_run_execution_boundary_review_statement_hash: str
    dry_run_execution_boundary_ref: str
    change_request_ref: str
    audit_chain_ref: str
    reviewed_by: str
    ...
    dry_run_execution_boundary_review_requested: bool
    dry_run_execution_boundary_review_ready: bool
    package_installation_dry_run_execution_boundary_ready: bool
    future_dry_run_execution_skeleton_required: bool = True
    ...
    summary: LmsPackageInstallationDryRunExecutionBoundarySummary
    ...
    def require_non_empty_text(cls, value: str) -> str:
        if not value.strip():
            raise ValueError("LMS dry-run execution boundary text fields must not be empty")
        return value
    ...
    @field_validator(
        ...
        "dry_run_execution_boundary_review_statement_hash",
        "evidence_hash",
    )
    @classmethod
    ...
    def require_unique_lists(cls, value: tuple[str, ...]) -> tuple[str, ...]:
        if len(set(value)) != len(value):
            raise ValueError("LMS dry-run execution boundary lists must not contain duplicates")
        for item in value:
            if not item.strip():
                raise ValueError("LMS dry-run execution boundary list items must not be empty")
        return value

    @model_validator(mode="after")
    def require_non_executing_boundary_contract(self) -> LmsPackageInstallationDryRunExecutionBoundaryResponse:
        if self.schema_version != LMS_PACKAGE_INSTALLATION_DRY_RUN_EXECUTION_BOUNDARY_SCHEMA_VERSION:
            raise ValueError("LMS dry-run execution boundary schema version is invalid")
        if self.endpoint != LMS_PACKAGE_INSTALLATION_DRY_RUN_EXECUTION_BOUNDARY_ENDPOINT:
            raise ValueError("LMS dry-run execution boundary endpoint is invalid")
        if self.result_contract != LMS_PACKAGE_INSTALLATION_DRY_RUN_EXECUTION_BOUNDARY_RESULT_CONTRACT:
            raise ValueError("LMS dry-run execution boundary result contract is invalid")
        if self.module_id != LMS_MODULE_ID:
            raise ValueError("LMS dry-run execution boundary only applies to lms")
        if self.continuity_domain != LMS_CONTINUITY_DOMAIN:
            raise ValueError("LMS dry-run execution boundary continuity domain is invalid")
        expected_ready = (
            self.package_installation_ready
            and self.reviewer_role_allowed
            and self.dry_run_execution_boundary_review_requested
            and not self.blocking_reasons
        )
        if self.dry_run_execution_boundary_review_ready != expected_ready:
            raise ValueError("LMS dry-run execution boundary review readiness must match prerequisites")
        if self.package_installation_dry_run_execution_boundary_ready != expected_ready:
            raise ValueError("LMS dry-run execution boundary readiness must match prerequisites")
        if not self.future_dry_run_execution_skeleton_required:
            raise ValueError("LMS dry-run execution boundary must require a future execution skeleton")
        if (
            ...
            raise ValueError("LMS dry-run execution boundary must remain metadata-only and non-executing")
        if self.summary.required_dry_run_execution_boundary_evidence_count != len(
            self.required_dry_run_execution_boundary_evidence
        ):
            raise ValueError("LMS dry-run execution boundary evidence count must match evidence list")
        if self.summary.blocking_reason_count != len(self.blocking_reasons):
            raise ValueError("LMS dry-run execution boundary blocking count must match blocking reasons")
        return self


def build_lms_package_installation_dry_run_execution_boundary_response(
    *,
    command: LmsPackageInstallationDryRunExecutionBoundaryCommand,
    ...
    command_hash = build_lms_package_installation_dry_run_execution_boundary_command_hash(command)
    idempotency_key_hash = stable_hash(
        canonical_json(
            {
                "schema_version": "lms_package_installation_dry_run_execution_boundary_idempotency_key.v1",
    ...
    review_statement_hash = stable_hash(command.dry_run_execution_boundary_review_statement)
    reviewer_role_allowed = bool({"tenant-admin", "tenant_admin"} & user_context.role_ids)
    blocking_reasons = _dry_run_execution_boundary_blocking_reasons(
        command=command,
    ...
    required_evidence = (
        ...
        "exact_dry_run_execution_boundary_review_statement_hash",
        "change_request_ref",
        "idempotency_key_hash",
        "audit_chain_ref",
        "future_dry_run_execution_skeleton_required",
        "no_lms_dry_run_execution_confirmation",
    )
    draft = LmsPackageInstallationDryRunExecutionBoundaryResponse(
        tenant_id=user_context.tenant_id,
        package_installation_ready=readiness.package_installation_ready,
        ...
        dry_run_execution_boundary_review_statement_hash=review_statement_hash,
        dry_run_execution_boundary_ref=command.dry_run_execution_boundary_ref,
        ...
        reviewer_role_allowed=reviewer_role_allowed,
        dry_run_execution_boundary_review_requested=command.dry_run_execution_boundary_review_requested,
        dry_run_execution_boundary_review_ready=boundary_ready,
        package_installation_dry_run_execution_boundary_ready=boundary_ready,
        ...
        next_action=(
            LMS_PACKAGE_INSTALLATION_DRY_RUN_EXECUTION_BOUNDARY_READY_NEXT_ACTION
            if boundary_ready
            else LMS_PACKAGE_INSTALLATION_DRY_RUN_EXECUTION_BOUNDARY_RETRY_NEXT_ACTION
        ),
    )
    return draft.model_copy(
        update={"evidence_hash": build_lms_package_installation_dry_run_execution_boundary_hash(draft)}
    )


def build_lms_package_installation_dry_run_execution_boundary_command_hash(
    command: LmsPackageInstallationDryRunExecutionBoundaryCommand,
) -> str:
    payload = command.model_dump(mode="json", exclude={"dry_run_execution_boundary_review_statement"})
    payload["dry_run_execution_boundary_review_statement_hash"] = stable_hash(
        command.dry_run_execution_boundary_review_statement
    )
    return stable_hash(canonical_json(payload))


def build_lms_package_installation_dry_run_execution_boundary_hash(
    response: LmsPackageInstallationDryRunExecutionBoundaryResponse,
) -> str:
    return stable_hash(canonical_json(response.model_dump(mode="json", exclude={"evidence_hash"})))


def _dry_run_execution_boundary_blocking_reasons(
    *,
    command: LmsPackageInstallationDryRunExecutionBoundaryCommand,
    package_installation_ready: bool,
    ...
    if not command.dry_run_execution_boundary_review_requested:
        reasons.append("dry_run_execution_boundary_review_not_requested")
