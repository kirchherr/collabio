...
| CM-012 | Human oversight | Critical actions require approval | Approval engine, Knowledge Base write/edit approval dry-run, command hashing, append-only write-approval evidence ledger, approval-transition lineage, source-object write guard, metadata-only restore/source evidence refresh preview, explicit-human-confirmation write-execution skeleton, and guarded edit execution before article/source persistence | `tests/test_knowledge_base.py`, `tests/test_api.py`, `tests/test_knowledge_base_write_approval_ledger.py`, `tests/test_pgvector_migration.py`, approval workflow tests | partial |
